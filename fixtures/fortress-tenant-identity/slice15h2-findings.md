# FORTRESS Slice 15H2 — Meta WhatsApp ingress authority with transactional wa_message_id claim (B02)
...
**Deferred 15H:** `fortress/slice-15h-meta-authority-enforcement` @ `72fb041` — do not merge / do not modify (session `pg_advisory_lock` approach superseded)

## Outcome

Implements the frozen 15G2 acceptance boundary with a corrected global `wa_message_id`
claim. When `META_WHATSAPP_INGRESS_AUTHORITY` is explicitly enabled **and** channel
routing config is present, a known `phone_number_id` makes the resolver’s
...
## Delta vs deferred FORTRESS-15H

Deferred 15H used **session** advisory locks (`pg_advisory_lock` /
`pg_advisory_unlock`) around the global find-or-insert claim. 15H2 replaces
that with a **single transaction**:

`BEGIN` → `SELECT pg_advisory_xact_lock(hashtext(ns), hashtext(wa_message_id))`
→ global lookup by `wa_message_id` → conditional insert → `COMMIT`

`ROLLBACK` on every failure (lookup error, insert error, or any thrown error).
Never `pg_advisory_lock` / `pg_advisory_unlock`. Never depends on a new
`UNIQUE(wa_message_id)` schema constraint.
...
- Historical replay looks up candidates by `wa_message_id` only (does not trust the
  ...
  event is inserted
- Cross-tenant `wa_message_id` claim uses PostgreSQL **transactional** advisory locks
  (`pg_advisory_xact_lock`) inside one `BEGIN`/`COMMIT` so concurrent cross-slug
  inserts cannot both win without requiring an unapplied `UNIQUE(wa_message_id)`
  schema constraint at code rollout
...
15H2 does **not** enable the policy on any runtime and does **not** set up a live
...
the policy is off. Do not merge deferred 15H (session lock); use 15H2 instead.
