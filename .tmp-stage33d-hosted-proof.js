'use strict';
/** Stage 33d — deploy 4628dc4 pending service attach DB proof. Temp — do not commit. */
...
const COMMIT = '4628dc4';
const IMAGE_TAG = `${COMMIT}-stage33d-pending-service-attach-proof`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 'stage33d-svc-attach';
...
const TESTS = [
  {
    ...
    checks: (ctx) => ({
      ...
      plan_status_ready: ctx.depositPlanStatus === 'ready',
      ...
      yoga_record_source: ctx.serviceRecords?.some((r) => r.service_type === 'yoga' && r.source === 'luna_guest_pending'),
  ...
  {
    id: 'B_meals_pending_attach',
    name: 'Meals deferred manual service attach after hold',
    messages: ['Malibu July 10 to July 17 for 1', 'just the stay', 'Can I book dinners?', "I'll decide later", 'deposit'],
    cleanupWindows: [PROOF_WINDOW],
    checkIn: PROOF_WINDOW.check_in,
    checkOut: PROOF_WINDOW.check_out,
    checks: (ctx) => ({
      meals_status_interested: ['interested', 'requested', 'needs_staff_confirmation'].includes(ctx.depositObs?.meals_status),
      services_pending_manual: Array.isArray(ctx.depositObs?.services_pending_manual)
        && ctx.depositObs.services_pending_manual.includes('meals'),
      hold_created: !!ctx.booking,
      payment_draft_created: !!ctx.payment,
      stripe_link_sent: ctx.stripeLinkSends >= 1,
      meals_record_exists: ctx.serviceRecords?.some((r) => r.service_type === 'meal'),
      meals_record_source: ctx.serviceRecords?.some((r) => r.service_type === 'meal' && r.source === 'luna_guest_pending'),
      meals_needs_scheduling: ctx.serviceRecords?.some((r) => r.service_type === 'meal' && r.metadata?.needs_scheduling === true),
      meals_no_fake_date: ctx.serviceRecords?.every((r) => r.service_type !== 'meal' || r.service_date == null),
      no_confirmation: !ctx.confirmationSent,
    }),
  },
];
...
function buildMetaPayload(text) {
  const wamid = `wamid.stage33d.${Date.now()}.${crypto.randomBytes(4).toString('hex')}`;
  return {
    ...
    entry: [{
      changes: [{
        value: {
          ...
          contacts: [{ profile: { name: 'Stage33d Guest' }, wa_id: PROOF_PHONE_RAW }],
          messages: [{
...
(async () => {
  ...
  try {
    ...
    if (cmd === 'tests' || cmd === 'all') {
      ...
      sleep(20000);
      report.gates_during = envPick(GATE_NAMES);
      const testFilter = process.env.STAGE33D_TEST || '';
      ...
      for (const testDef of suite) {
        ...
        if (testDef.id === 'A_yoga_pending_attach' && !report.tests[report.tests.length - 1].pass) {
          console.error('[stage33d] Test A failed — skipping Test B');
          break;
        }
      }
    }
