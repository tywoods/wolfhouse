// RADAR Slice 16H — standalone staging-only Staff API metric alerts (one RG).
// Source module only — supersedes deferred 16F/16G. No deployment wrapper.
//
// Declares ONLY:
...
// network, budgets, or action groups.
//
// Operator deployment (Incremental) and all live proof remain OPEN.
// This template cannot enforce ARM deployment mode — mode is external to ARM
// evaluation. Prefer Incremental when an approved operator deploy occurs.
// Live apply is out of scope for this slice (source + offline gates only).
...
// Documentary only — ARM templates cannot enforce deployment mode.
