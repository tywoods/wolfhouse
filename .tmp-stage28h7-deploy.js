'use strict';
/** Stage 28h.7 — deploy 7015061 intake continuation fix. Temp — do not commit. */
const { execSync } = require('child_process');

const COMMIT = '7015061';
const IMAGE_TAG = `${COMMIT}-stage28h7-intake-continuation`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 's28h7-intake-continuation';
...
try {
  ...
  console.log(JSON.stringify({
    ...
    healthz: healthz(),
    image: IMAGE,
    retest: ['Hello', 'Yes I want to book a stay', 'July 1-5', '1'],
    prep: 'Use Fresh Start on +491726422307 before retest if prior handoff context remains',
  }, null, 2));
} catch (e) {
