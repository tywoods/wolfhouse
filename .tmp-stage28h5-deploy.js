'use strict';
/** Stage 28h.5 — deploy 62f0178 compact ordinal dates. Temp — do not commit. */
const { execSync } = require('child_process');

const COMMIT = '62f0178';
const IMAGE_TAG = `${COMMIT}-stage28h5-compact-ordinal-dates`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 's28h5-compact-ordinal-dates';
...
try {
  ...
  const rev = deploy();
  const gates = envPick([...Object.keys(PLAYGROUND_ON_ENV), 'LUNA_CONFIRMATION_LIVE_SEND_ALLOWLIST', 'NODE_ENV']);
  console.log(JSON.stringify({
    ...
    healthz: healthz(),
    image: IMAGE,
    meta_callback: `https://${STAFF_HOST}/staff/meta/whatsapp/webhook`,
    retest_turn1: 'Hello, I want to create a new booking please',
    retest_turn2: 'July 1st to 5th. just me',
  }, null, 2));
} catch (e) {
