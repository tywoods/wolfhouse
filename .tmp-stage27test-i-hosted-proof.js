'use strict';
/** Stage 27test-i — deploy proof. Temp — do not commit. */
const https = require('https');
const { execSync, spawnSync } = require('child_process');

const HOST = 'staff-staging.lunafrontdesk.com';
const COMMIT = 'e79ee4b';
const IMAGE_TAG = `${COMMIT}-stage27test-i-batch-edge`;
...
function parseGolden(out) {
  ...
  const resultMatch = out.match(/Golden result:\s*(PASS|FAIL)/i);
  const failures = [];
  for (const line of out.split('\n')) {
...
(async () => {
  ...
  const goldenRun = spawnSync(
    process.execPath,
    ['scripts/run-luna-guest-golden-tests.js', '--base-url', `https://${HOST}`, '--limit', '50'],
    {
      cwd: process.cwd(),
      env: { ...process.env, LUNA_BOT_INTERNAL_TOKEN: token },
      encoding: 'utf8',
  ...
  const bannedHits = bannedTerms.filter((t) => new RegExp(t, 'i').test(batchOut));
  ...
  const safetyOk = out.safety.review_dry_run
    && out.safety.review_sends_whatsapp
    && out.safety.review_live_blocked
    && out.safety.no_stripe_in_batch
