'use strict';
/** Stage 28c.4 — deploy 8fd0d39 + Meta→Staff API handset proof. Temp — do not commit. */
...
function summarizeTurn(event, snap) {
  ...
  return {
    ...
    guest_count: odr.guest_count,
    package_code: odr.package_code,
    check_in: odr.check_in,
    check_out: odr.check_out,
    payment_choice_ready: odr.payment_choice_ready,
    payment_choice_needed: odr.payment_choice_needed,
...
(async () => {
  ...
  const out = {
    stage: '28c.4-meta-staffapi-handset',
    commit: COMMIT,
    image_tag: IMAGE_TAG,
  ...
  try {
    ...
    console.error('\n=== PROOF WINDOW OPEN — Ty send 3 NEW messages now ===');
    ...
    out.checks = {
      ...
      turn1_malibu: String(out.turn1.package_code || '').toLowerCase().includes('malibu'),
