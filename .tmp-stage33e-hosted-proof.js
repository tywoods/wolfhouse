'use strict';
/** Stage 33e — deploy 6e78c63 yoga pending attach DB proof. Temp — do not commit. */
...
const COMMIT = '6e78c63';
const IMAGE_TAG = `${COMMIT}-stage33e-yoga-pending-attach-proof`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 'stage33e-yoga-attach';
...
function buildMetaPayload(text) {
  const wamid = `wamid.stage33e.${Date.now()}.${crypto.randomBytes(4).toString('hex')}`;
  return {
...
(async () => {
  ...
  const outPath = path.join(__dirname, '.tmp-stage33e-proof-report.json');
