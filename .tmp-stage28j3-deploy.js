'use strict';
/** Stage 28j.3 — deploy a9bfe7d brain arbitration fix to live staging. Temp — do not commit. */
const https = require('https');
const { Client } = require('pg');
const { execSync } = require('child_process');
const { resetLunaConversationContext } = require('./scripts/lib/staff-conversation-writes');

const COMMIT = 'a9bfe7d';
const IMAGE_TAG = `${COMMIT}-stage28j3-brain-arbitration`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 's28j3-brain-arbitration';
...
async function inspectRetest(sinceIso) {
  ...
  for (const ev of events.rows) {
    ...
    turns.push({
      ...
      brain_source: brain?.source || brain?.brain_source || null,
  ...
  const asksDepositFull = (t) => /would you prefer to pay|deposit or the full amount/i.test(t.outbound || '');
  const isGenericHandoff = (t) => /passing this to our team|follow up soon.*team/i.test(t.outbound || '');
  ...
  const checks = {
    ...
    accommodation_only: turns.some((t) => /no add nothing/i.test(t.inbound || '')
      ...
      && !asksDepositFull(t)
      && !isGenericHandoff(t)),
    deposit_handled: turns.some((t) => /^deposit$/i.test((t.inbound || '').trim())
      && !asksDepositFull(t)
      && /team|confirm|accommodation/i.test(t.outbound || '')
      && !isGenericHandoff(t)),
    when_handled: turns.some((t) => /when will you/i.test(t.inbound || '')
      && !isGenericHandoff(t)
      && /team|confirm|accommodation|short stay/i.test(t.outbound || '')),
    no_intro_repeat: turns.filter((t) => !/^hi$/i.test((t.inbound || '').trim()))
      .every((t) => !/(?:Hi|Hey)[!,.]?\s+I'?m\s+Luna\s+from\s+Wolfhouse/i.test(t.outbound || '')),
    brain_observability: turns.some((t) => t.brain_enabled != null && t.brain_source != null),
    no_brain_override: !turns.some((t) => t.final_reply_overrode_brain === true),
  ...
  return {
    ...
    llm_called_any: turns.some((t) => t.brain_source === 'llm' || (t.llm_enabled && t.model_used)),
    models_used: [...new Set(turns.map((t) => t.model_used).filter(Boolean))],
  };
}
...
(async () => {
  try {
    ...
    if (cmd === 'fresh-start') {
      ...
      console.log(JSON.stringify({ phase: 'fresh-start', ...fs }, null, 2));
    }
    if (cmd === 'ready') {
      const pf = await preflight();
      const fs = await freshStart();
      console.log(JSON.stringify({
        ...
        pass: pf.pass,
        fresh_start: fs,
        message: `Ty: send these 7 messages from ${PROOF_PHONE} to demo WhatsApp ${DEMO_WA}:`,
