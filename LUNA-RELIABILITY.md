...
**Status bar:** `LR3.2 | Status Active | Golden offline PASS; #953 landed; post-953 live retest still empty_tool_calls; offline adapter repair proves terminal function_call recovery + wire tools/tool_choice normalization (provider_empty_with_wire_ok remains fail-closed); 03/08 HOLD | Next gate: Chief-gated live retest after adapter deploy`.

| Field | Value |
|---|---|
| Family | `Closed-fixture contract / abort observability mismatch` |
| Severity | Major — case-scoped QA blocker |
| Status | Partially cleared: #944 closed isolated harness is merged and deployed, #943 fixtures landed, #945/#946/#947 docs/verifier follow-ups are merged, #948 hardened the LR2 messy-pack receipt contract, #949 added metadata-first capture instrumentation at master tip `0ee15c4d`, and #953 merged the Responses adapter `tool_choice` wire plus `empty_tool_calls` classification at master tip `0deceb93`. The #953 live retest observed a healthy deploy, but both the auto path and named-read path still ended with `completion_category=empty_tool_calls`. Offline LR3.2 adapter repair now proves (1) Chat→flat tools/tool_choice on the wire, (2) synthetic `function_call` → executable descriptor without dispatch, and (3) June-pin done-only stream assembly recovers terminal `response.output` function_call items. When wire is correct and native output is genuinely empty, classify `provider_empty_with_wire_ok` (fail-closed; not a classification-only PASS). Golden offline remains PASS; live case-09 remains BLOCKED pending Chief-gated live retest after adapter deploy; 03/08 live execution stays on HOLD. |
| Evidence tips | #943 fixtures; #944 harness merged+deployed; #945/#946/#947 docs/verifier follow-ups; #948 messy-pack verifier; #949 metadata-first capture instrumentation `0ee15c4d`; #953 Responses adapter landing `0deceb93`; post-#953 live retest classification `completion_category=empty_tool_calls` for auto and named-read; offline LR3.2 adapter repair (terminal function_call recovery + `provider_empty_with_wire_ok`) |
...
This family records the split result honestly: the closed isolated harness and map stub shipped, the offline golden path passes against the merged fixture/doc verifier work, the metadata-first capture hook needed for zero-tool diagnosis has landed, the #953 Responses adapter wire/classification fix is on master, and the offline LR3.2 adapter extraction/serialization repair is proven red→green. It is **not** a full LR3.2 PASS: the healthy-deploy live retest still produced empty tool calls under both auto and named-read modes, with `completion_category=empty_tool_calls`, and live case-09 remains BLOCKED until a Chief-gated live retest after adapter deploy. Keep 03/08 live checks on HOLD until that unblock is explicitly cleared.
