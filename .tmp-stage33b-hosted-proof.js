'use strict';
/** Stage 33b — deploy 5e6a641 package add-ons + yoga attach staging proof. Temp — do not commit. */
...
const COMMIT = '5e6a641';
const IMAGE_TAG = `${COMMIT}-stage33b-package-addons-proof`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 'stage33b-package-addons-proof';
...
const TESTS = [
  {
    id: 'A_package_one_shot_addons',
    name: 'Package one-shot add-ons before deposit',
    messages: ['Malibu July 10 to July 17 for 1', 'just the stay', 'deposit'],
    cleanupWindows: [{ check_in: '2026-07-10', check_out: '2026-07-17' }],
    checkIn: '2026-07-10',
    checkOut: '2026-07-17',
    checks: (ctx) => ({
      composer_owned_quote: ctx.turns.some((t) => /malibu|€299|299/i.test(t.outbound || '')
        && /wetsuit|surfboard|lessons/i.test(t.outbound || '')),
      no_legacy_quote_source: !ctx.turns.some((t) => t.final_reply_source === 'quote'),
      addons_before_deposit: (() => {
        const addonIdx = ctx.turns.findIndex((t) => /wetsuit|lessons/i.test(t.outbound || ''));
        const depIdx = ctx.turns.findIndex((t) => /^deposit$/i.test((t.inbound || '').trim()));
        return addonIdx >= 0 && (depIdx < 0 || addonIdx < depIdx);
      })(),
      malibu_preserved: /malibu/i.test(ctx.booking?.package_code || '')
        || ctx.turns.some((t) => /just the stay/i.test(t.inbound || '') && /malibu/i.test(t.outbound || '')),
      no_proactive_yoga_meals: !ctx.turns.slice(0, 2).some((t) => /\b(?:yoga|dinner|meals)\b/i.test(t.outbound || '')),
      no_payment_before_addons: !ctx.turns.slice(0, 1).some((t) => /checkout\.stripe\.com/i.test(t.outbound || '')),
      stripe_link_sent: ctx.stripeLinkSends >= 1 || !!ctx.lateStripeSend,
      hold_created: !!ctx.booking,
      no_confirmation: !ctx.confirmationSent,
    }),
  },
  {
    id: 'B_explicit_addons_selection',
    name: 'Explicit surf add-ons selection',
    messages: ['book a stay', 'July 1-5', '1', 'wetsuit and lessons', 'deposit'],
    cleanupWindows: [{ check_in: '2026-07-01', check_out: '2026-07-05' }],
    checkIn: '2026-07-01',
    checkOut: '2026-07-05',
    checks: (ctx) => ({
      addons_requested: ctx.depositObs?.addons_requested?.includes('wetsuit')
        && ctx.depositObs?.addons_requested?.includes('surf_lesson'),
      no_side_question_explainer: !ctx.turns.some((t) => /wetsuit and lessons/i.test(t.inbound || '')
        && /Neopren|I am not adding rentals|do you want to know about wetsuits/i.test(t.outbound || '')),
      no_german_unless_guest: !ctx.turns.some((t) => /wetsuit and lessons/i.test(t.inbound || '')
        && /Neopren-Verleih|Surfkurse hinzuf/i.test(t.outbound || '')),
      deposit_prompt: ctx.turns.some((t) => /wetsuit and lessons/i.test(t.inbound || '')
        && /deposit|full/i.test(t.outbound || '')),
      quote_includes_addons: (ctx.depositObs?.quote_total_cents || 0) > 18000,
      stripe_link_sent: ctx.stripeLinkSends >= 1 || !!ctx.lateStripeSend,
      hold_created: !!ctx.booking,
      no_confirmation: !ctx.confirmationSent,
    }),
  ...
  {
    id: 'C_yoga_pending_attach',
    name: 'Yoga before deposit — DB attach proof',
    ...
    checks: (ctx) => ({
      ...
      services_pending_before_hold: ctx.turns.some((t) => /Can I add yoga/i.test(t.inbound || '')
        && (Array.isArray(t.services_pending_manual) ? t.services_pending_manual.includes('yoga') : /yoga/i.test(t.outbound || ''))),
      hold_created: !!ctx.booking,
      attached_manual_services: ctx.attachedManualServices?.includes('yoga')
        || ctx.serviceRecords?.some((r) => r.service_type === 'yoga'),
      yoga_record_source: ctx.serviceRecords?.some((r) => r.service_type === 'yoga'
        && (r.source === 'luna_guest_pending' || r.source === 'luna_guest_pending')),
      yoga_record_status: ctx.serviceRecords?.some((r) => r.service_type === 'yoga' && r.status === 'requested'),
      yoga_needs_scheduling: ctx.serviceRecords?.some((r) => r.service_type === 'yoga'
        && r.metadata?.needs_scheduling === true),
...
function buildMetaPayload(text) {
  const wamid = `wamid.stage33b.${Date.now()}.${crypto.randomBytes(4).toString('hex')}`;
  return {
    ...
    entry: [{
      changes: [{
        value: {
          ...
          contacts: [{ profile: { name: 'Stage33b Guest' }, wa_id: PROOF_PHONE_RAW }],
          messages: [{
...
async function inspectTest(sinceIso, testDef) {
  ...
  if (booking) {
    ...
    serviceRecords = (await pg.query(
      `SELECT id::text, service_type, service_date::text, status, source, metadata, created_at::text
         FROM booking_service_records WHERE booking_id = $1::uuid ORDER BY created_at ASC`,
      [booking.id],
    )).rows.map((r) => ({
      ...r,
  ...
  let stripeLinkSends = sends.filter((s) => /checkout\.stripe\.com/i.test(s.message_text || '')).length;
  ...
  const attachedManualServices = serviceRecords
    .filter((r) => r.source === 'luna_guest_pending')
...
(async () => {
  ...
  try {
    ...
    if (cmd === 'tests' || cmd === 'all') {
      ...
      sleep(20000);
      report.gates_during = envPick(GATE_NAMES);
      const testFilter = process.env.STAGE33B_TEST || '';
      const suite = testFilter
        ? TESTS.filter((t) => t.id === testFilter || t.id.startsWith(testFilter))
        : TESTS;
      for (const testDef of suite) {
    ...
    report.result = preflightOk && testFails.length === 0 && verifierFails.length === 0
      ? 'PASS'
      : (preflightOk && testFails.length <= 1 && verifierFails.length === 0 ? 'PARTIAL' : 'FAIL');
    report.failures = {
