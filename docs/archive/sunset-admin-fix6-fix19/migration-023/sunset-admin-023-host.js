...
async function snapshot(client) {
  ...
  for (const t of TABLES) {
    const c = await client.query(
      `SELECT 1 FROM information_schema.columns
        WHERE table_schema='public' AND table_name=$1 AND column_name='location_id' LIMIT 1`, [t],
    );
  ...
  return { database: db, location_id_columns, counts };
}

async function seedSardinero(client) {
  await client.query('BEGIN');
  await client.query(`INSERT INTO tenant_price_rules (
      ...
      WHERE client_slug='sunset' AND location_id='sunset-somo' AND active=true
        AND NOT EXISTS (
          SELECT 1 FROM tenant_price_rules t2
           WHERE t2.client_slug=tenant_price_rules.client_slug AND t2.location_id='sunset-sardinero'
             AND t2.item_type=tenant_price_rules.item_type AND t2.item_code=tenant_price_rules.item_code
             AND t2.unit=tenant_price_rules.unit AND t2.active=true
        )`);
  await client.query(`INSERT INTO tenant_lesson_capacity_rules (
      ...
      WHERE client_slug='sunset' AND location_id='sunset-somo' AND active=true
        AND NOT EXISTS (
          SELECT 1 FROM tenant_lesson_capacity_rules t2
           WHERE t2.client_slug=tenant_lesson_capacity_rules.client_slug AND t2.location_id='sunset-sardinero'
  ...
  await client.query(`INSERT INTO tenant_lesson_time_rules (
      ...
      WHERE client_slug='sunset' AND location_id='sunset-somo' AND active=true
        AND NOT EXISTS (
          SELECT 1 FROM tenant_lesson_time_rules t2
           WHERE t2.client_slug=tenant_lesson_time_rules.client_slug AND t2.location_id='sunset-sardinero'
...
async function main() {
  ...
  if (!url) throw new Error('WOLFHOUSE_DATABASE_URL required');
  const host = url.replace(/^postgres(ql)?:\/\//i, 'http://').split('@').pop().split('/')[0];
  const db = url.replace(/^postgres(ql)?:\/\//i, 'http://').split('/').pop().split('?')[0];
  if (!host.includes(APPROVED_HOST)) throw new Error(`host guard failed: ${host}`);
  if (db !== APPROVED_DB) throw new Error(`db guard failed: ${db}`);
  ...
  try {
    if (phase === 'preflight') {
      ...
      console.log(JSON.stringify({ ok: true, host: APPROVED_HOST, ...s, migration_needed: !s.location_id_columns.tenant_price_rules }, null, 2));
      return;
    }
    if (phase === 'migrate') {
      const s = await snapshot(client);
