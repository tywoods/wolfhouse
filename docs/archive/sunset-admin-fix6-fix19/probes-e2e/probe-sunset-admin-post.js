...
(async () => {
  ...
  const expectedSource = process.env.EXPECTED_SOURCE || 'db';
  if (!password) {
  ...
  let json;
  try { json = JSON.parse(admin.body); } catch (_) { json = { raw: admin.body.slice(0, 500) }; }

  const priceCount = Array.isArray(json.prices) ? json.prices.length : json.price_count;
  const lessonCap = json.lesson_capacity?.default_daily_cap
    ?? json.lesson_daily_cap
    ?? json.daily_lesson_cap
    ?? json.lesson_cap;
  const lessonTimesLen = Array.isArray(json.lesson_times) ? json.lesson_times.length : null;
  const changeHistoryLen = Array.isArray(json.change_history) ? json.change_history.length : null;
  ...
  const summary = {
    ...
    lesson_capacity_default_daily_cap: lessonCap,
    lesson_times_length: lessonTimesLen,
    change_history_length: changeHistoryLen,
  };
