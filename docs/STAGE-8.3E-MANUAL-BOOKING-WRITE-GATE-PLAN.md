...
**Status:** 8.3i DONE (2026-06-02). **8.3j DONE (2026-06-02) � schema mismatches fixed, fixture proof 65/65 PASS, helper production-schema-compatible.** **8.3k DONE (2026-06-02) � rollback/undo proof 59/59 PASS, final delta=0, all blockers verified.** **8.3l DONE (2026-06-02) � preview UI wired to read-only POST /staff/manual-bookings/preview; Preview Conflicts button enabled on cell selection; result panel shows valid/blocked/warning/error states; Create Manual Booking remains disabled; 122/122 verifier PASS; preview_only=true, creates_booking=false, no_write_performed=true confirmed.** No confirmed create. No staff writes. Staff writes disabled. **Stage 8.4 RE-SCOPED (2026-06-02): manual booking creation split into gated slices; a pricing/payment engine is now a hard prerequisite. A provisional `POST /staff/manual-bookings/create` stub exists DISABLED-by-default (MANUAL_BOOKING_ENABLED=false ? 403) and UNWIRED from the UI. Do not enable until the pricing/payment engine (slices 1�6) is built. See [STAGE-8.4-MANUAL-BOOKING-CREATION.md](STAGE-8.4-MANUAL-BOOKING-CREATION.md).**
...
| **8.3k** | **Rollback / undo proof** — `scripts/lib/staff-manual-booking-rollback-sql.js`; 10 blockers; CASCADE deletion; pre_beds/pre_payments capture; audit written on every attempt; `MANUAL_BOOKING_ROLLBACK_BLOCK_CODES`; `verify-staff-manual-booking-rollback-sql.js` **52/52 PASS**; proof `stage8.3k-manual-booking-rollback-proof.js` **59/59 PASS**, delta=0. Blockers proven: confirm_not_set, role, code mismatch, payload id mismatch, unsafe_payment, booking_not_found. No API route. No UI. No Azure. | Yes (test DB only, BEGIN/ROLLBACK) | **DONE** — 52/52 static PASS; 59/59 runtime PASS; delta=0. |
| **8.3l** | **UI preview modal** wired to 8.3h preview endpoint only — Preview Conflicts button, result panel, auth error handling, clear selection resets preview. c-preview-result. 122/122 verifier PASS. | No | **DONE** — preview_only=true; creates_booking=false; no_write_performed=true; Create button stays disabled. |
