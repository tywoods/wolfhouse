...
- **Stage 8.6 — Staff Ask Luna via WhatsApp allowlisted staff phones** *(near-next)*: staff can ask Luna operational questions directly via WhatsApp using their own numbers; staff phone allowlist in Staff Portal/settings controls access; Staff API owns query intent + helper logic; n8n is WhatsApp pipe only; Staff Portal Ask Luna box remains optional/secondary. First answerable queries: arrivals/departures today, who owes money, deposit/full paid status, checkout links pending, beds/rooms needing cleaning, needs-human conversations. Later queries (after add-on records): yoga payments, lessons tomorrow, board/wetsuit requests, unpaid add-ons. Token-gated, no guest message leakage into staff query mode.
