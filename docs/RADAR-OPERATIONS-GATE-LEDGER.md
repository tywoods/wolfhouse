...
**Frozen method/attempt semantics:** each `method_id` binds exact `command_template` and/or REST `method`/`path_template`/`api_version`/`body_schema`, allowed subscription/RGs/hosts, explicit `retry_policy` + `fallback_policy`, and requires **one attempt-log record per physical dispatch** (including failures, retries, and declared fallbacks). Hidden retries/fallbacks and missing failure records are defects. `complete_manifest_sha256` and per-response `response_sha256` recompute from fixtures via frozen canonical JSON + SHA-256.
