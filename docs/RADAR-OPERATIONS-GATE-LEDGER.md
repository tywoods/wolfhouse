# RADAR Slice 16G — Operations gate ledger (Staff API metric alerts source-partial)

**Status:** source partial progress only (supersedes deferred 16F; no cherry-pick) (zero live deploy / mutation / notification-delivery / alert-fire claim)
...
| `fixtures/radar-operations/slice16g-metric-alert-plan.json` | Secret-free alert plan |
| `fixtures/radar-operations/slice16g-expected-contract.json` | Frozen independent contract |
| `scripts/lib/radar-slice16g-staff-api-metric-alerts.js` | Locks + RED/GREEN + scope short-circuit + `runPreflight` + safe `runSafeDeploymentEntry` (shell-free spawn, temp ARM, no path overrides) |
| `scripts/preflight-radar-slice16g-staff-api-metric-alerts.js` | Exact sub/RG/app/AG preflight (no Azure calls) |
| `scripts/verify-radar-slice16g-staff-api-metric-alerts.js` | Offline independent 16G verifier |
| `fixtures/radar-operations/gate-matrix.json` | Updated gate matrix (G03 source-partial) |
| `npm run verify:radar-slice16g-staff-api-metric-alerts` | Gate |
...
| G03 | Actionable tenant-aware alerts | `partial` (source-partial via 16G) |
...
**ID:** `16G_staff_api_metric_alerts`
...
`16G_DRILL_metric_alert_fire_notify` — after approved deploy: confirm alerts Enabled; prove notification delivery; prove controlled alert fire; re-list metric alerts read-only; prove no container restart/deploy/DB write / AG mutation.
...
az bicep build --file infra/azure/staging-staff-api-metric-alerts/rg-staff-api-metric-alerts.bicep --outfile tmp/radar-16g-rg-staff-api-metric-alerts.json
