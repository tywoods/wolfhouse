...
| Correction | dedicated immutable `STAFF_API_INGRESS_TENANT_SLUG` preferred; `DEFAULT_CLIENT_SLUG` compat fallback; conflict fail-closed |
...
**Proves (source/deploy-config):** Dedicated `STAFF_API_INGRESS_TENANT_SLUG` with backwards-compatible `DEFAULT_CLIENT_SLUG` fallback and conflict fail-closed; Wolfhouse/Sunset staging Bicep wired explicitly; REDs for missing/conflict/spoof/OFF parity; failed Wolfhouse canary recorded as identity fail-closed (not overload shed).
