# Email mailbox adapter boundary (Slice 1A + 1B + 1C-alpha + 1C-beta + 1C-gamma + 2A + 2B + 2C + 2D + 2F-A + 2F-B + 2F-C2 + inbound-envelope + ms-normalized-page + ms-immutableid-page-transport + ms-immutableid-bounded-catchup-transport + ms-messages-delta-page-transport + inbound-batch-processor + delegated-read-authority + delegated-grant-access-session + authority-bound-inbound-operation + authority-bound-bounded-catchup-operation + authority-bound-messages-delta-page-operation + email-delta-runtime-composition-inert + email-delta-recovery-journal + inbound-diagnostic + inbound-event-store + inbound-capture + inbound-delta-state)

**Status:**
- **email-delta-retry-stable-page-attempt (corrective; opaque same-ID replay):** Fixes independent-review finding that PR414 page operation generated the page-commit operation id only after Graph and hid it on `commit_outcome_unknown`, so callers could not same-ID replay/reconcile. Authority-bound operation factory exposes **`createPageAttempt({clientId,locationId,endpointId})`**: allocates a canonical UUID **before** any Graph/grant/KV/lease/DB side effect, closes over it privately (**no** id property, enumeration, log, result, or error leakage), and returns frozen exact surface **`{run,reconcile,status}`**. Repeated `run` on the same attempt after `commit_outcome_unknown` first queries durable migration-**066** journal via recovery-store **`readPageCommitOutcome`** (authority-bound read-only; source-pinned worker `sunset-email-delta-worker`; never caller-supplied actor/worker; never infers from cursor/current state) by exact enclosed id + tenant/endpoint/fences: **committed** → sanitized committed replay with **zero** Graph/KV/grant/lease/event/cursor mutation; **claimed/ambiguous** → uncertain/evidence unavailable with **zero** refetch; **absent** after confirmed rollback → may execute once safely. Fences (requested generation/stateVersion) unknown before lease: allocate attempt before Graph; capture/fill fences after authoritative lease/status **without** changing operation id; journal exact attribution. Reconcile classification is **journal-only**. Direct `runAuthorityBoundMessagesDeltaPage` preserved only as **explicitly activation-ineligible** offline single-shot (ephemeral attempt). Runtime composition must eventually require the attempt API. **`EMAIL_AUTHORITY_BOUND_MESSAGES_DELTA_PAGE_CALLER_DURABILITY_LIFECYCLE_READY = false`** — process-crash recovery of opaque attempt handles needs a caller durability / scheduler control lifecycle that is **not** invented here; **no worker activation** until that exists. Worker id source-pin `sunset-email-delta-worker`. **No** migration067, routes, runtime wire, deploy, flags, Graph/KV/worker/scheduler. Gates: `verify:email-authority-bound-messages-delta-page-operation`, `verify:email-delta-recovery-operation-store`, `prove:email-delta-recovery-operation-store-pglite`.
