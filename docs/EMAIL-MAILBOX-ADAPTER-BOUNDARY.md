...
| `scripts/lib/email-delta-runtime-config.js` | Default-off independent flag parse + deployment/tenant/worker/migration064/KV pin validation; fail closed worker/admin true; frozen identity-free readiness; documents future exclusive txn-client adapter | Activate worker/admin; apply migration; construct Pool/Azure SDK; request-selected tenant; mute fail-closed |
| `scripts/lib/email-delta-sunset-staging-runtime-composition.js` | Sunset-staging inert composition: readiness/lifecycle only; hard-fail run/reconcile/restart; lazy #410 owner closures; Staff API resolve-only integration | Scheduler/admin route; run path; DB/KV SDK/Graph/timer/lease at factory; flag in manifests/defaults; getPool exclusive loan; close application pool |
