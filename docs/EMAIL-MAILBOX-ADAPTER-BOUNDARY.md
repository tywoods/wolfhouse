...
**Status:**
- **email-delta-recovery-journal (offline recovery-operation store; UNWIRED prerequisite):** Migration `065` empty `tenant_email_delta_recovery_operations` recovery journal + import-inert module `email-delta-recovery-operation-store`. Client-supplied `operation_id` UUID PK; composite tenant FKs to `tenant_locations` / `tenant_channel_endpoints`; **tenant-safe actor** composite FK `(client_id, actor_staff_user_id) → staff_users (client_id, id)` (via `staff_users_client_id_id_uq` from 060) — no invented actor relation. `operation_kind` exact `restart_generation|reconcile_page_commit`; requested generation/state_version safe-bigint fences; outcome `claimed|committed|not_committed|commit_outcome_unknown|conflict|evidence_unavailable`; coherent nullable result generation/version/phase; endpoint/outcome/time index; `set_updated_at` trigger. **No** mailbox/provider/cursor/message/email/subject/token/free-text/JSON columns; no seed/backfill. Factory-fixed `withTransactionClient` + `authorityVerifier` only. APIs: `getRecoveryStatus` (safe status + `recovery_blocked` on active lease), `restartGeneration` (idempotent by operationId/actor/authority/fences), `reconcilePageCommit` (separate reconciliation `operationId` + `targetOperationId`). PR408 exposes tightly capability-bound `advanceGenerationOnExclusiveClient` so journal claim + demote/insert new generation + journal completion share **ONE** outer transaction/client (no nested BEGIN/checkout/release); public `beginNextGeneration` remains behavior-compatible. Authority verify before TX and re-fence within; active lease fail closed (`not_committed`). Same operationId + identical inputs → persisted exact result zero mutation; mismatch actor/endpoint/kind/fences → `operation_id_conflict`. Two IDs same CAS → one `committed` one `conflict`. COMMIT dispatch ambiguity → exact `commit_outcome_unknown` (no retry-as-success); retry same ID after committed ack-loss returns persisted committed; rolled-back ambiguity may execute exactly once. Preserve old generation / no cursor copy. Reconciliation classifies **only durable journal evidence**; page commits are not journaled in this PR → explicit `evidence_unavailable` (never infer `not_committed` from migration 064 `cursor_operation_id` mismatch/generation/version; never mutate events/cursor). Pinned intrinsics/proxy/accessor/symbol/nonenumerable hardening; exact frozen PII-free DTO/errors; logless. **No** routes/runtime/scheduler/worker/admin activation/staff-query-api changes. Gates: `verify:email-delta-recovery-operation-store`, `prove:email-delta-recovery-operation-store-pglite`.
