# Email mailbox adapter boundary (Slice 1A + 1B + 1C-alpha + 1C-beta + 1C-gamma + 2A + 2B + 2C + 2D + 2F-A + 2F-B + 2F-C2 + inbound-envelope + ms-normalized-page + ms-immutableid-page-transport + ms-immutableid-bounded-catchup-transport + inbound-batch-processor + delegated-read-authority + delegated-grant-access-session + authority-bound-inbound-operation + inbound-diagnostic + inbound-event-store + inbound-capture)

**Status:**
- **ms-immutableid-bounded-catchup-transport (runtime-capable multi-page primitive, UNWIRED):** Thin surface over the **single** delegated-messages network owner. Sequential multi-page GET of authority-bound `/v1.0/users/{canonicalUuid}/messages` with pinned `Prefer: IdType="ImmutableId"` on **every** request; factory-fixed **maxPages=10**, **maxMessages=50** (never caller-supplied). First request is the exact single-page path (`$top=5` + fixed `$select`); subsequent requests follow **only** provider-returned `@odata.nextLink` after descriptor-safe extraction and **strict** re-validation (primitive string + bounded length; pinned URL parse; https; no userinfo/hash; exact `graph.microsoft.com` host/port; exact same `/v1.0/users/{same encoded UUID}/messages` path with path-confusion rejection; query allowlist = original `$top`/`$select` plus exactly one opaque bounded nonempty continuation param; no duplicate keys; no changed `$top`/`$select`; no extras). Rejects loop/repeated nextLink. Normalizes every row through the existing canonical mapper; cross-page canonical sort + identity dedupe use envelope-contract owners (same rules as the batch processor) — **no** consumer invocation, **no** persistence. Success returns one fresh frozen sanitized DTO `{ envelopes, pages_fetched, observed_count, unique_count, duplicate_count, truncated }` (no links/tokens/raw rows/provider errors). `truncated:true` when maxPages/maxMessages is reached while another nextLink exists (or unique identities exceed maxMessages); otherwise `false`. maxMessages selection is deterministic (canonical order; first N unique after sort; observed counts only accepted identities + their duplicates). Atomic: any page/request/parser/normalization failure throws only the existing sanitized `microsoft_graph_immutableid_page_failed` and returns **no** partial envelopes. Single-page ImmutableId and count-health factories remain byte-compatible on the same network owner. **No** route/runtime composition, DB, OAuth scope change, deploy, Azure, cron, startup, or live Graph from verifiers. Gate: `verify:email-microsoft-graph-immutableid-bounded-catchup-transport`.
