...
| `scripts/lib/email-delta-runtime-config.js` | Default-off independent flag parse + deployment/tenant/worker/migration064/KV pin validation (raw-exact KV enabled; self-contained pins; no #410/KV/state owner require); fail closed worker/admin true; frozen identity-free readiness; module-init intrinsic pins; documents future exclusive txn-client adapter | Activate worker/admin; apply migration; construct Pool/Azure SDK; request-selected tenant; load owner constructors; mute fail-closed |
