...
- **Delegated read-authority Gmail generalization owner decision (RED contract; offline / UNWIRED):** Resolve an exact Gmail endpoint only when the existing encrypted grant join proves matching client/location/endpoint ownership and the endpoint is `email` + `gmail_api` + `delegated_authorization_code` + `google_delegated_oauth` + `verified`, canonical issuer `https://accounts.google.com`, exact case-sensitive 1..255 ASCII `!`–`~` principal/resource `sub`, and `user`/`own_user`. Return that `sub` unchanged as `providerMailboxId`; UUID normalization is Microsoft-only. The existing six-field DTO already contains the closed `provider` discriminator (`microsoft_graph` or `gmail_api`), so no DTO field is added: every existing Microsoft field, order, value, and behavior remains byte-compatible. Provider-specific Graph/Gmail transport selection remains outside this repository resolver and is explicitly deferred; no transport, OAuth route, network, credentials, activation, or runtime wiring is introduced by this contract.
