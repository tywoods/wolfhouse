...
| `scripts/lib/email-authority-bound-inbound-operation.js` | **UNWIRED** composition: authority resolve → injected one-shot grant-session token → ImmutableId transport (`authority.providerMailboxId`) → batch processor once (factory-fixed consumer); identity-free counts only | Refresh/custody/HTTP duplication; caller token/provider/mailbox/consumer; authority DTO/envelope/token escape; routes/runtime wiring; migration/deploy/live |
