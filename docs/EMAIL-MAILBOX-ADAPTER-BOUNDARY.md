...
| `scripts/lib/email-microsoft-delegated-inbound-diagnostic-sunset-staging-runtime-composition.js` | Default-off Sunset composition: access-session + ImmutableId transport + authority-bound op + **module-owned** zero-touch diagnostic consumer; public `{ status:'ok', received_count, accepted_count, discarded_count }` (maps internal input/delivered/duplicate; max-5 + invariant); exports canonical `isInboundDiagnosticEnabled` | Enable without exact flag+sunset; duplicate refresh/custody; inspect/log/persist envelopes; expose processed/delivered/input_count/unique_count/duplicate_count/IDs/PII/stage/generation; cron/poller/startup; flag in manifests/defaults |
| `scripts/lib/staff-email-oauth-routes.js` (`handleInboundDiagnostic`) + top-level `staff-query-api` router | Admin POST inbound-diagnostic; router dual-gates before requireAuth/body/DB; reuses Sunset auth/access/body/binding + resolved DB UUIDs; public `{ success, status:'ok', received_count, accepted_count, discarded_count }`; 404/400/503 sanitized | Gate after auth/body; change read-health; trust caller slug identity; non-admin; leak internal count names |
