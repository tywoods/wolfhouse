...
| `scripts/lib/email-microsoft-delegated-inbound-diagnostic-sunset-staging-runtime-composition.js` | Default-off Sunset composition: access-session + ImmutableId transport + authority-bound op + **module-owned** zero-touch diagnostic consumer; public `{ status:'success', durably_processed:false, input_count, delivered_count, duplicate_count }` (same authority count names; max-5 + observed=unique+duplicate); exports canonical `isInboundDiagnosticEnabled` | Enable without exact flag+sunset; duplicate refresh/custody; inspect/log/persist envelopes; status ok / received/accepted/discarded synonyms; claim durability; expose processed status/IDs/PII/stage/generation; cron/poller/startup; flag in manifests/defaults |
| `scripts/lib/staff-email-oauth-routes.js` (`handleInboundDiagnostic`) + top-level `staff-query-api` router | Admin POST inbound-diagnostic; router dual-gates before requireAuth/body/DB; reuses Sunset auth/access/body/binding + resolved DB UUIDs; public `{ success, status:'success', durably_processed:false, input_count, delivered_count, duplicate_count }`; 404/400/503 sanitized | Gate after auth/body; change read-health; trust caller slug identity; non-admin; status ok / received/accepted/discarded public DTO |
