...
| `scripts/lib/staff-email-oauth-routes.js` (`handleInboundCapture`) + top-level `staff-query-api` router | Admin POST inbound-capture; composition gate snapshot before requireAuth/body/DB; `withPgClient` outer + `withTransactionClient: withPgClient` exclusive persist; public `{ success, status:'durable_capture_completed', observed_count, durable_identity_count, duplicate_in_batch_count, durably_captured:true }`; 404/400/503 sanitized; **activation precondition:** migration 063 ledger+schema | Gate after auth/body; change diagnostic/read-health; trust slug identity; newly-inserted counts; PII/IDs/stage; new pool owner; enable/deploy/cron |
