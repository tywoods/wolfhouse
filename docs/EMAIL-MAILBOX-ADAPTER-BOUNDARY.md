# Email mailbox adapter boundary (Slice 1A + 1B + 1C-alpha + 1C-beta + 1C-gamma + 2A + 2B + 2C + 2D + 2F-A + 2F-B + 2F-C2 + inbound-envelope + ms-normalized-page + ms-immutableid-page-transport + ms-immutableid-bounded-catchup-transport + inbound-batch-processor + delegated-read-authority + delegated-grant-access-session + authority-bound-inbound-operation + authority-bound-bounded-catchup-operation + inbound-diagnostic + inbound-event-store + inbound-capture + inbound-delta-state)

**Status:**
- **inbound-delta-state (offline durable Graph messages-delta state + page-commit; UNWIRED):** Migration `064` empty `tenant_email_inbound_delta_states` — durable per-endpoint generation state for Microsoft Graph messages-delta. Identity binds trusted `client_id` / `location_id` (tenant_locations.id UUID) / `endpoint_id` + `provider=microsoft_graph` + provider tenant UUID + authoritative mailbox UUID. **`ingestion_generation` is independent of OAuth `grant_generation`** (grant refresh/rotation must not reset delta state). `query_version` advances on rebind/query change. Phase `initial|tracking|reset_required|paused`; monotonic `state_version` CAS fencing; DB-clock lease owner/token/until (separate from grant lease). Graph `nextLink`/`deltaLink` are confidential provider capabilities — **never plaintext** in DB/log/result/error; sealed cursor envelope (v1 AES-256-GCM + wrapped DEK + version-pinned KEK) is all-null or all-present with `cursor_kind`. AAD binds client+endpoint+provider+tenant+mailbox+generation+query_version+cursor_kind so rebind/generation/query-version change fails closed. Partial unique `(client_id, endpoint_id) WHERE is_current` prevents ambiguous current generations while preserving old generations auditably. Module `email-inbound-delta-state-store` (import-inert): initialize, acquire/renew/release lease, open cursor under valid lease, **`commitPageEvents` (ONLY page-commit owner)**, mark reset-required, begin next generation, public status. `commitPageEvents`: no network/crypto inside exclusive-client transaction; caller supplies already-validated canonical envelopes/tombstones + already-sealed successor cursor; lock/CAS current state; verify client/endpoint/generation/state_version/lease/mailbox/query; insert migration-063 events via shared `SQL_INSERT_EVENT` `ON CONFLICT DO NOTHING`; advance sealed cursor+phase+state_version; COMMIT. Duplicate-only/tombstone-only pages still advance; tombstones create no synthetic events (arrival-capture only). Insert/CAS failure rolls back both. Commit-unknown returns exact sanitized uncertainty (never retries/claims rollback; replay converges). Cursor expiry/Graph 410 → `reset_required` (never timestamp fallback). Explicit reset/rebind starts `ingestion_generation+1` with verified authority only. Public status omits cursor, lease token, mailbox/tenant identities, envelopes/PII. **No** routes/runtime/cron/startup/network/send/draft. Gates: `verify:email-inbound-delta-state-store`, `prove:email-inbound-delta-state-store-pglite`.
...
| `scripts/lib/email-inbound-delta-state-store.js` | Durable Graph messages-delta state + **only** page-commit owner (`commitPageEvents`); sealed nextLink/deltaLink; DB-clock ingestion lease; generation/query_version fencing; reuses grant envelope provider + 063 `SQL_INSERT_EVENT`; public status strips secrets | Second event insert owner; plaintext cursor in DB; network/crypto inside commit txn; grant-generation coupling; routes/runtime/cron; log cursor/PII |
