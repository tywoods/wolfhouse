# Email mailbox adapter boundary (Slice 1A + 1B + 1C-alpha + 1C-beta + 1C-gamma + 2A + 2B + 2C + 2D + 2F-A + 2F-B + 2F-C2 + inbound-envelope + ms-normalized-page + ms-immutableid-page-transport + inbound-batch-processor + delegated-read-authority + delegated-grant-access-session + authority-bound-inbound-operation + inbound-diagnostic + inbound-event-store)

**Status:**
- **inbound-event-store (offline durable consumer + default-off Sunset composition; UNWIRED route):** Migration `063` empty `tenant_email_inbound_events` — persists exact canonical envelope fields plus trusted `client_id` / `location_id` / `endpoint_id` authority. **`location_id` is `tenant_locations.id` UUID** (matches delegated-read authority DTO + OAuth transactions composite FK `(client_id, id)`), **not** the text kebab `location_id` on `tenant_channel_endpoints`. Composite endpoint FK reuses `tenant_channel_endpoints (client_id, id)` from 059. DB-enforced unique identity `(provider, provider_mailbox_id, provider_message_id)`; `internet_message_id` metadata only (nullable; not identity). Module `email-inbound-event-store`: descriptor/proxy-safe db boundary; factory-fixed durable consumer; **one transaction per delivered batch** (BEGIN → `INSERT … ON CONFLICT DO NOTHING` → COMMIT; **no UPDATE**); exact frozen `{ acknowledged: true }` only after conclusive COMMIT; mid-batch failure rolls back all; commit-sent-then-reject → sanitized `commit_outcome_unknown` (no ack/compensation/internal retry/rollback claim; later replay converges); provider/mailbox mismatch rejected **before SQL**. Reuses envelope validator, batch processor, authority-bound operation, access-session, ImmutableId transport — **no** second contract/custody/network owner. Default-off import-inert Sunset composition (`email-microsoft-delegated-inbound-event-store-sunset-staging-runtime-composition`) gated by exact `LUNA_EMAIL_OAUTH_INBOUND_EVENT_STORE_ENABLED=true` **and** `LUNA_DEPLOYMENT=sunset-staging`; flag **not** in manifests/defaults; **no** route/cron/poller/startup/diagnostic behavior change. Gates: `verify:email-inbound-event-store`, `prove:email-inbound-event-store-pglite`.
...
- **authority-bound-inbound-operation (composition):** One internal operation (`createAuthorityBoundInboundOperation` / `runAuthorityBoundInbound`) that resolves through merged `resolveDelegatedReadAuthority`, obtains an access token **only** through an explicitly trusted injected one-shot grant-session capability (`runWithAccessTokenOnce` — callback-scoped; never caller input / never a returned token source), whose **private callback alone** invokes the existing ImmutableId page transport with `authority.providerMailboxId` only (double-scrub loan + transport input), then invokes the merged inbound batch processor exactly once with a **factory-fixed** consumer. Caller input is exact `{ clientId, locationId, endpointId }` only — **no** token / provider / mailbox / address / status / generation / consumer. Dependencies are trusted executable code and exact frozen/snapshotted services. Returns only frozen sanitized **identity-free** status/counts (`status`, `input_count`, `delivered_count`, `duplicate_count`). **No** envelope / PII / token / authority DTO escape / log / retention. Does **not** duplicate HTTP / refresh / custody (no lease→open→MS refresh→reseal→CAS inside this module; no second lease/SQL/refresh owner). ImmutableId transport path is exact `/v1.0/users/{canonicalUuid}/messages` (not `/me`) so token/authority mailbox mismatch fails at Graph rather than mislabeling canonical envelopes; count-health remains `/me`. Default-off; referenced only by the inbound-diagnostic Sunset runtime composition (`EMAIL_AUTHORITY_BOUND_INBOUND_OPERATION_RUNTIME_WIRED = false`). No DB migration, OAuth scope change, deploy, live I/O, or persistence.
...
| `scripts/lib/email-authority-bound-inbound-operation.js` | Composition: authority resolve → injected `runWithAccessTokenOnce` → private callback ImmutableId transport (`authority.providerMailboxId`, double-scrub) → batch processor once (factory-fixed consumer); identity-free counts only | Refresh/custody/HTTP duplication; caller token/provider/mailbox/consumer; authority DTO/envelope/token escape; direct routes; migration/deploy/live |
...
| `scripts/lib/email-inbound-event-store.js` | Durable atomic idempotent event store + factory-fixed consumer; one txn/batch insert-or-no-op; ack after COMMIT; proxy-safe db boundary; reuses envelope validator | Second envelope contract; UPDATE path; ack before commit; log PII; route/startup wiring |
| `scripts/lib/email-microsoft-delegated-inbound-event-store-sunset-staging-runtime-composition.js` | Default-off Sunset composition: access-session + ImmutableId + authority-bound op + durable event-store consumer; `durably_processed: true` on success; exact `LUNA_EMAIL_OAUTH_INBOUND_EVENT_STORE_ENABLED` | Enable without flag+sunset; change diagnostic route; flag in manifests/defaults; cron/poller/startup |
