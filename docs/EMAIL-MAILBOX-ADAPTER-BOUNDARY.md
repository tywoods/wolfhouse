# Email mailbox adapter boundary (Slice 1A + 1B + 1C-alpha + 1C-beta + 1C-gamma + 2A + 2B + 2C + 2D + 2F-A + 2F-B + 2F-C2 + inbound-envelope + ms-normalized-page + ms-immutableid-page-transport + inbound-batch-processor + delegated-read-authority + delegated-grant-access-session + authority-bound-inbound-operation)
...
| `scripts/lib/email-authority-bound-inbound-operation.js` | **UNWIRED** composition: authority resolve → injected `runWithAccessTokenOnce` → private callback ImmutableId transport (`authority.providerMailboxId`, double-scrub) → batch processor once (factory-fixed consumer); identity-free counts only | Refresh/custody/HTTP duplication; caller token/provider/mailbox/consumer; authority DTO/envelope/token escape; routes/runtime wiring; migration/deploy/live |
