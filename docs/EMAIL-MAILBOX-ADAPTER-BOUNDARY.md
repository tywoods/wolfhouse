...
| `scripts/lib/email-microsoft-delegated-inbound-diagnostic-sunset-staging-runtime-composition.js` | Default-off Sunset composition: access-session + ImmutableId transport + authority-bound op + **module-owned** zero-touch diagnostic consumer; public `{ status:'ok', input_count, unique_count, duplicate_count }` | Enable without exact flag+sunset; duplicate refresh/custody; inspect/log/persist envelopes; expose processed/delivered/IDs/PII/stage/generation; cron/poller/startup; flag in manifests/defaults |
| `scripts/lib/staff-email-oauth-routes.js` (`handleInboundDiagnostic`) | Admin POST inbound-diagnostic; reuses Sunset auth/access/body/binding + resolved DB UUIDs; 404/400/503 sanitized | Change read-health; trust caller slug identity; non-admin; leak internal terms |
