...
>
> **Next: Stage 8.4.4** — Quote preview endpoint. Read-only API endpoint (`GET /staff/quote-preview`) that accepts booking input params and returns a `calculateWolfhouseQuote()` output as JSON. No DB writes. Mirrors the existing availability-preview pattern. Gate: `MANUAL_BOOKING_ENABLED` remains false.
