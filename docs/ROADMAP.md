**8.7.4 FIX ASK LUNA BUTTON HANDLER -- PASS (2026-06-03):** `window.alAsk = alAsk` in staff-query-api.js (IIFE scope fix from 8.7.3 rehearsal). Enter key unchanged. `verify-staff-query-ui.js` 46/46 PASS (+3 regression checks). No backend/n8n/WhatsApp/Stripe/Azure. Redeploy Staff API to apply on staging. Next: re-run 8.7.2 demo Step 3 on hosted portal.
