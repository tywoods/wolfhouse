**Phase 10.3f MULTI-BED MOVE SELECTION SPEC — PASS (2026-06-04):** Docs-only design for assignment-level move when a booking has multiple `booking_beds` rows. Spec: [PHASE-10.3f-MULTI-BED-MOVE-SPEC.md](PHASE-10.3f-MULTI-BED-MOVE-SPEC.md). **Problem:** single-bed move works; multi-bed currently blocks with `requires_manual_review` / `single_bed_booking_required` — safe but too limiting. **Model:** optional/required `booking_bed_id`; `requires_selection:true` when multi-bed and no selection; UPDATE one row only; siblings unchanged. **Next:** **10.3g** API → **10.3h** UI pills → **10.3i** hosted proof → close **10.3**. **Safety:** docs only · no code · no DB · no deploy · no n8n · no WhatsApp · no Stripe.
...
| **10** Staff Operations Polish | **IN PROGRESS — 10.0–10.4 PASS; 10.3f spec PASS (2026-06-04)** | **10.0–10.2** preview · **10.3** single-bed move write + **10.3e** drawer UI · **10.3e.1** drawer layout (`efc199a`) · **10.3f** multi-bed move spec · **10.4** date-change preview hosted (`ac4c1d5` / `--0000061`). **Next:** **10.3g** multi-bed move API **or** **10.4e** date-change preview UI (recommend finish **10.3g–10.3i** to close bed-move vertical). |
