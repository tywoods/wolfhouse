**Phase 10.3e STAFF PORTAL MOVE CONTROLS — COMPLETE ENOUGH / PASS (2026-06-04):** Staff Portal booking drawer **Move bed** panel — target bed selector, **Preview move**, **Move booking**; same-date bed move only; no date-change UI; no drag/drop; no Confirm Move modal; preview when gate OFF; Move booking disabled when gate OFF. Commit **`7104815`** · verifier **`verify:staff-booking-move-ui`** **39/39 PASS**. Hosted: image `whstagingacr.azurecr.io/wh-staff-api:7104815-stage103e-move-ui-gate-off` → **`--0000058`** (gate OFF: preview PASS, move disabled, write **403** `booking_move_write_disabled`, no DB mutation, stayed **DEMO-R1-B2**) · **`--0000059`** (gate ON: `MB-WOLFHO-20260920-4f62e2` / Manual Polish Test **B2→B1**, preview `can_move:true`, UI move `moved:true`, idempotency `idempotent:true`/`moved:false`, counts unchanged) · **`--0000060`** (gate OFF cleanup, **100% traffic**, Healthy). **Proof booking after cleanup:** **DEMO-R1-B1** · dates **2026-09-20→2026-09-23** unchanged. **Safety:** staging/test only · no production DB · no n8n · no WhatsApp · no Stripe · no payment/service-record mutation · no Ask Luna change · no date change · no live guest automation · staging gate **OFF**. **Next:** **Phase 10.4 — Date-change preview** (preview-only; half-open overlap; same bed/room MVP; no mutation). Spec: [PHASE-10.3-MOVE-ROOM-BED-WRITE-SPEC.md](PHASE-10.3-MOVE-ROOM-BED-WRITE-SPEC.md).
...
| **10** Staff Operations Polish | **IN PROGRESS — 10.0–10.3e PASS (2026-06-04)** | **10.0–10.2** preview (`6d339e3` / `--0000054`). **10.3** gated move write (`0a1acbf`) · **10.3e** drawer move UI hosted (`7104815` / `--0000060`, gate OFF). **Next:** **10.4** date-change preview |
