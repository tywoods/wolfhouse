**Stage 27t GUEST AUTOMATION GATE DESIGN — DESIGN LOCK (2026-06-08):** Docs-only gate design before public/client-facing Luna guest intake. **Proven chain:** 27q/27r/27s/27s.1 · `WHATSAPP_DRY_RUN` restored · **no public guest automation wired**. **Next:** 27u orchestrator dry-run → 27v Staff Portal review → 27w allowlisted guest live-send proof → 27x limited staging → production explicit GO only. Doc: [STAGE-27T-GUEST-AUTOMATION-GATE-DESIGN.md](STAGE-27T-GUEST-AUTOMATION-GATE-DESIGN.md). Verifier: `verify:stage27t-guest-automation-gate-design`.
