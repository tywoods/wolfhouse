**8.7.11 CLEAN PAYMENT + ADD-ONS LAYOUT -- PASS (2026-06-03):** UI-only — booking drawer Payment section contained in compact left `.ctx-pay-box` (no full-width green stretch); manual booking add-ons compact grid left-aligned; **Meals** qty input added (visual-only / not priced in quote — not sent via `buildAddOns()`). Luna confirmation draft unchanged; no send button. `verify-staff-bed-calendar-ui.js` PASS (+16 checks). No backend/n8n/WhatsApp/Stripe/Azure. **Not deployed** — batch with 8.7.10 nav labels on next redeploy.

**8.7.10 CLEAN STAFF PORTAL NAV LABELS -- PASS (2026-06-03):** Nav tab **Luna** (no bot emoji); order Today → Inbox → Bed Calendar → Luna → Tour Operator → Developer Tools. Panel hero title **Luna**; `/staff/ask-luna` unchanged. `verify-staff-query-ui.js` PASS (+7d). No backend/n8n/WhatsApp/Stripe/Azure. Redeploy Staff API to apply on staging.
