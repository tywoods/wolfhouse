**Phase 10.2 MOVE ROOM/BED PREVIEW — COMPLETE ENOUGH / PASS (2026-06-03):** Slices **10.2 / 10.2a / 10.2b** complete; hosted proof **PASS** on staging. Commits: **`e5058ab`** (10.2 `POST /staff/bookings/move-preview`; static verifier `verify:staff-booking-move-preview` **35/35**) · **`1c438a9`** (10.2a SQL fix: `r.name AS room_name`; hosted `--0000053` after prior `r.room_name` 500) · **`6d339e3`** (10.2b expose `target.room_name`; verifier **41/41**). Latest hosted: commit **`6d339e3`** → image `whstagingacr.azurecr.io/wh-staff-api:6d339e3-stage102b-move-preview-room-name` (ACR `cb1g`) → revision **`wh-staging-staff-api--0000054`** (100% traffic, Healthy). **Proof booking:** `MB-WOLFHO-20260920-4f62e2` · guest **Manual Polish Test** · stay **2026-09-20→2026-09-23** · current bed **`DEMO-R1-B1`**. **Allowed preview:** target **`DEMO-R1-B2`** · `room_code:DEMO-R1` · **`room_name:Demo Dorm Room 1`** · `success:true` · `can_move:true` · `preview_only:true` · `would_mutate:false` · `conflicts:[]` · message *Move preview passed. No changes were made.* **Blocked preview:** overlapping range → `can_move:false` · conflicts include blocking booking (`MB-WOLFHO-20260801-4f10c3` on same bed) · message target bed unavailable / no changes made. **Same-day turnover boundary:** strict half-open overlap — checkout/checkin same-day boundaries allowed; adjacent turnover booking not counted as conflict; true overlap still blocks (runtime proof on `DEMO-R1-B1` Jun turnover fixture). **Safety:** preview-only · SELECT-only · no booking/booking_beds/payments/service_records mutation · source assignment unchanged · no move write route · no n8n · no WhatsApp · no Stripe · staging only · no production DB. **Next: Phase 10.3 — Move room/bed write** (**GATED** — split carefully: gated write endpoint · conflict recheck inside write · update `booking_beds` assignment safely · audit result · no payments/service mutation · no WhatsApp/n8n/Stripe · hosted proof staging/test only). Verifier: manual docs check only.
...
| **10** Staff Operations Polish | **IN PROGRESS — 10.0 + 10.1 + 10.2 COMPLETE ENOUGH / PASS** | **10.0** Bed Calendar nights + turnover + form nights (`625b766` / `--0000050`). **10.1** manual booking create UX polish (`c06b486` / `--0000051`; proof `MB-WOLFHO-20260920-4f62e2`). **10.2** move room/bed preview (`6d339e3` / `--0000054`; `POST /staff/bookings/move-preview`). **Next: 10.3** move room/bed write (**GATED** — split carefully) |
