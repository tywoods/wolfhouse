**Stage 27l HOLD + PAYMENT DRAFT DESIGN LOCK — PASS (2026-06-08):** Docs-only design for hold + payment draft after `payment_choice_ready`. **Gate:** `ready_for_hold_payment_draft` + quote `ready` + availability `available` + valid intake fields. **Hold:** 6h expiry · no proactive expiry copy · re-check availability after expiry. **Draft:** deposit €200/€100 or full quote total · not payment truth. **Plan:** 27m planner · 27n staging write · 27o Stripe test link · 27p webhook truth. **Safety:** docs only · no runtime/DB/writes/Stripe/WhatsApp/Meta/n8n. Verifier: `verify:stage27l-hold-payment-draft-design`. Doc: [STAGE-27L-HOLD-PAYMENT-DRAFT-DESIGN.md](STAGE-27L-HOLD-PAYMENT-DRAFT-DESIGN.md). **Next:** **Stage 27m** — dry-run hold/payment-draft planner.
