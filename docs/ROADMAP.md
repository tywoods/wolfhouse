**Phase 10.3a MOVE ROOM/BED WRITE SPEC — PASS (2026-06-03):** Docs-only design for gated booking move write after Phase 10.2 preview proof. Spec: [PHASE-10.3-MOVE-ROOM-BED-WRITE-SPEC.md](PHASE-10.3-MOVE-ROOM-BED-WRITE-SPEC.md). Proposed `POST /staff/bookings/move` (operator auth + `STAFF_ACTIONS_ENABLED` + `BOOKING_MOVE_ENABLED`); required `idempotency_key`; conflict recheck inside transaction; **Option A:** UPDATE single `booking_beds` row when exactly one assignment exists; half-open + same-day turnover rules match preview; audit in response + `appendAuditLog`. **MVP:** bed/room move at same dates only; multi-bed → `requires_manual_review`; date changes → **10.4**. **Split:** 10.3b API · 10.3c verifier · 10.3d hosted proof (`MB-WOLFHO-20260920-4f62e2`) · 10.3e UI optional. No code/DB/deploy/n8n/WhatsApp/Stripe. **Next: 10.3b** API handler. Verifier: manual docs check only.
...
| **10** Staff Operations Polish | **IN PROGRESS — 10.0–10.2 PASS · 10.3a spec PASS** | **10.0–10.2** complete (preview `6d339e3` / `--0000054`). **10.3a** move write spec PASS. **Next: 10.3b** gated `POST /staff/bookings/move` API (no UI) |
