...
**Status: IN PROGRESS** (2026-06-01) — 7.0–7.7 DESIGN DONE · **7.2b+7.2c+7.3b+7.7a–d+7.7f+7.7g+7.7h+7.7i+7.7j DONE · 7.7k plan DONE (design)**. **7.7k DONE (design only)**: §5a safe bed reassignment plan — allowed v1 = same-range bed→bed move only; 14 hard blockers; 7 warning-confirm cases; in-transaction half-open overlap guard + row lock (future btree_gist EXCLUDE noted); full audit + before/after rollback (conflict-on-undo → handoff); proposal→confirm→gated-write UI; sub-slices 7.7k1–7.7k8. Write NOT implemented/approved. Pilot checklist F8 DESIGN_DONE; editable calendar not required for Phase 1. **7.7m DONE (design only)**: §5b manual booking creation plan — required fields, modes A–D (unassigned/with-bed/hold/confirmed), overlap guard, audit + rollback, sub-slices 7.7m1–7.7m7; write NOT implemented; NOT required for shadow pilot Phase 1; required before spreadsheet retirement; F9 DESIGN_DONE. `lunafrontdesk.com` purchased (DNS not configured). No Azure. Next: 7.7k1 reassign SQL helper (static only) or 7.7m1 manual booking SQL helper (static only) or 7.7o booking edit/write gates plan.﻿# Wolfhouse Booking Assistant — Product Roadmap
