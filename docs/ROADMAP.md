**Stage 27p STRIPE PAYMENT TRUTH — PASS (2026-06-08):** `runGuestStripePaymentTruthApplyApproved` in `scripts/lib/luna-guest-stripe-payment-truth-apply.js`. **Reuses:** `handleStripeWebhook` (8.4.11). **Gate:** `confirm_payment_truth` + staging + test mode + session/amount match. **Output:** `ready_for_confirmation_dry_run`. **No confirmation send/WhatsApp/webhook duplicate.** Verifier: `verify:stage27p-stripe-payment-truth`. Doc: [STAGE-27P-STRIPE-PAYMENT-TRUTH.md](STAGE-27P-STRIPE-PAYMENT-TRUTH.md). **Next:** confirmation dry-run slice.
