**Stage 27n GATED HOLD/PAYMENT DRAFT WRITE — PASS (2026-06-08):** `runGuestHoldPaymentDraftWriteDryRunApproved` in `scripts/lib/luna-guest-hold-payment-draft-write.js`. **Gate:** `confirm_write:true` + non-production + 27m planner ready. **Reuses:** `upsertBookingHold` · `payments` draft · quote snapshot metadata · idempotency via bookings/payments metadata. **No Stripe link/WhatsApp/Meta/n8n.** Verifier: `verify:stage27n-hold-payment-draft-write`. Doc: [STAGE-27N-HOLD-PAYMENT-DRAFT-WRITE.md](STAGE-27N-HOLD-PAYMENT-DRAFT-WRITE.md). **Next:** **Stage 27o** — Stripe test link.
