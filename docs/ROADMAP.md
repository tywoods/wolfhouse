**Phase 10.4a DATE-CHANGE PREVIEW SPEC — PASS (2026-06-04):** Docs-only design for preview-only date change after Phase 10.3 move write + UI. Spec: [PHASE-10.4-DATE-CHANGE-PREVIEW-SPEC.md](PHASE-10.4-DATE-CHANGE-PREVIEW-SPEC.md). **Proposed:** `POST /staff/bookings/date-change-preview` — operator auth · SELECT-only · half-open overlap · same-bed MVP · single-bed only · `can_change_dates` + conflicts + nights delta + `pricing_impact.requires_reprice` (no payment mutation). **Deferred:** date-change write → **10.5**; no write route · no UI write · no guest send in 10.4. **Split:** 10.4a spec · 10.4b API · 10.4c verifier · 10.4d hosted proof · 10.5 write later. **Safety:** no code · no DB · no deploy · no n8n · no WhatsApp · no Stripe. **Next:** **10.4b** API preview endpoint.
...
| **10** Staff Operations Polish | **IN PROGRESS — 10.0–10.3e PASS; 10.4a PASS (2026-06-04)** | **10.0–10.2** preview (`6d339e3` / `--0000054`). **10.3** move write · **10.3e** drawer move UI. **10.4a** date-change preview spec PASS. **Next:** **10.4b** preview API |
