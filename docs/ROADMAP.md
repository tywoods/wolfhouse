**Phase 10.3 MOVE ROOM/BED WRITE API — COMPLETE ENOUGH / PASS (2026-06-04):** Gated write `POST /staff/bookings/move` implemented, verified, and proven on staging. Spec: [PHASE-10.3-MOVE-ROOM-BED-WRITE-SPEC.md](PHASE-10.3-MOVE-ROOM-BED-WRITE-SPEC.md). Commits: **`d9b5c36`** (10.3a spec) · **`b5c76fe`** (10.3b API, verifier **52/52**) · **`0a1acbf`** (10.3b.1 preview verifier alignment, **45/45**). Hosted: image `whstagingacr.azurecr.io/wh-staff-api:0a1acbf-stage103c-move-write-gated` (ACR `cb1h`) → revisions **`--0000055`** (gate OFF) · **`--0000056`** (gate ON) · **`--0000057`** (gate OFF cleanup, **100% traffic**, Healthy). **MVP:** operator auth · **`BOOKING_MOVE_WRITE_ENABLED=true`** required · single-bed only · same dates only (date changes → **10.4**) · conflict recheck in transaction · half-open overlap · self-booking excluded · UPDATE one `booking_beds` row in place · required `idempotency_key` · no UI move button (→ **10.3e**). **Proof booking:** `MB-WOLFHO-20260920-4f62e2` · `01039383-389e-4e71-a7d6-75b56345fdbf` · **Manual Polish Test** · **2026-09-20→2026-09-23** · moved **DEMO-R1-B1 → DEMO-R1-B2** · `booking_bed_id` **`0c70c90a-2c7d-4bb1-bdf9-12289747cf6f`** updated in place · counts unchanged · idempotent retry PASS. **Gate:** OFF **403** · ON move PASS · cleanup OFF **403** · staging gate currently **OFF**. **Safety:** no production DB · no n8n · no WhatsApp · no Stripe · no payment/service-record mutation · no Ask Luna change. **Blocked conflict proof:** skipped (no fixture). **Next:** decide **10.3e** Staff Portal Preview/Move UI vs **10.4** date-change preview.
...
| **10** Staff Operations Polish | **IN PROGRESS — 10.0–10.3 PASS (2026-06-04)** | **10.0–10.2** preview (`6d339e3` / `--0000054`). **10.3 PASS** — gated move write hosted + gate cleanup (`0a1acbf` / `--0000057`, gate OFF). **Next:** **10.3e** UI move controls vs **10.4** date-change preview |
