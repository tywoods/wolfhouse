...
Sunset staging only. Dedicated internal draft service `hermes-sunset-email-luna` (`HERMES_ROLE=sunset-email-luna`). Durable Hermes config pins `model.provider: openai-codex` and `model.default: gpt-5.6-sol`. Isolated home is a Lunabox volume or a dedicated Azure Files mount of `/opt/data` (account `lunasunsetemailst`, share `hermes-sunset-email-luna-home`) with canonical Hermes path `/opt/data/.hermes` (`HOME=/opt/data`). Codex refresh rotations persist at `/opt/data/.hermes/auth.json` on that share. No Key Vault auth.json snapshot. No copies or symlinks. No WhatsApp/Discord gateway, no Staff booking plugin, no outbound email authority.
