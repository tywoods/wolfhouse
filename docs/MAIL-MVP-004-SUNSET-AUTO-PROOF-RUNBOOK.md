...
`execute-once` requires the exact typed phrase, the Sunset target pins, the **current 100% Healthy** serving revision + image tag/digest from revision show, a fresh 64-hex nonce, and a 15-minute `--confirm-issued-at` freshness window evaluated **once at execute-once start** (target and scope unchanged). Traffic is parsed from actual ACA ingress: exactly one explicit weight-100 revision equal to `latestReady`/`latest`, then revision-show Healthy, Provisioned/Succeeded, and a documented serving running state (`Running`, or `RunningAtMaxScale` only with at least one running replica). ACA revision templates are immutable; a revision that is `latest`==`latestReady`==the sole 100%-traffic revision, Healthy/running-accepted, and has at least one running replica from the replica list belonging to that exact revision is a running deployment of that template. Desired flag state is accepted only from that successor's `az containerapp revision show` template env when the image tag+digest match the authorized original, exactly the two allowlisted flags have literal desired values, and there are no duplicate, missing, or `secretRef` values and no unrelated flag mutation. Bound proof includes revision name, image tag, digest, and replica. Unproven raw template env (`flagsSource=template`) is not enough. Graph preflight remains a separate exact ACA `exec` before flags. Process `printenv` is preferred if it succeeds once on the bound replica; a trusted WebSocket HTTP 429 (Retry-After=600) or unstable exec does **not** wait 630s/20 minutes — ACA-native immutable revision proof is then sufficient. Flag updates create a new ACA revision that can take more than 3 minutes (observed ~6 minutes even when health endpoints already 200) to become `latestReady`/100%. The supervisor waits up to **10 minutes per successor** (2s identity poll) for that same-image/digest/100%/running-replica native proof; a 3-minute bound times out before a legitimate successor can be attested. Never reuse attestation across revision/replica or desired enabled state; exact native or process proof returns immediately; timeout fail-closes (`enabled_revision_unproven`) without invoking the owner. Original authorization remains valid across that wait: the supervisor stamps its own issued-at onto a one-use expiry-bound inner capability **after** the enabled successor is attested, so the caller timestamp is not capability identity and the 15-minute operator window is not widened. After a real 003 `{status:'sent'}` the harness reads selected-operation durable approval `message_text` plus Create Draft author evidence HMAC-bound to draft body hash + request/source operation + tenant/location/conversation — conversation metadata booleans are not proof. Missing synthetic return fields must not classify a successful send as leftover.
...
1. Independently re-read serving identity from explicit 100% traffic + revision-show health. Kill-switch probe the attested-false replica first (`emergency_flags_off`, zero author/journal/provider). Flag updates create a new ACA revision; do not pin the original revision name. Accept only a successor (or same-image) revision with the authorized image tag+digest, `latest`==`latestReady`==sole 100% Healthy traffic, at least one running replica belonging to that exact revision, and both approved flags literal `true` from the immutable revision-show template env (exactly those two keys; no `secretRef`, duplicates, missing values, or unrelated env mutation). Prefer one replica-process `printenv` if it succeeds; on trusted 429 allow the ACA-native proof immediately (never nested `sh -c`, never wait 630s/20 minutes for exec). Wait up to 10 minutes per revision (2s identity poll) for successor readiness. A 3-minute bound times out before a legitimate successor can be attested. Put email channel mode `auto` only on that attested enabled successor (never on the pre-successor replica). No other flag changes. Invoke the owner only after that proof. No public endpoint or new secret.
2. Invoke the canonical 003 production auto owner **exactly once** via image-owned `scripts/prove-mail-mvp-004-auto-create-send.js` (`MAIL_MVP_004_STAFF_OWNER_PROOF=1`). The inner ACA exec does **not** run 003 itself and does **not** spawn a child in the exec cgroup. It writes a one-shot request file; the long-lived Staff API process (container main tree, outside the exec session) claims it, stamps the dispatch receipt with its pid, and runs 003. Exec emits the dispatch marker only after that Staff claim, then returns. Owner send does not retry. Empty Create Draft context. Staff API remains the only price/availability/booking authority.
3. Reconcile durable approval/journal/provider state. Classified inner JSON is preserved even if exec status is nonzero. If exec disconnects after the issued marker with no classified JSON, **do not retry the owner**; reconcile only via a short read-only replica snapshot (`MAIL_MVP_004_SNAPSHOT=reconcile`) that does **not** consume the one-use owner capability. Host polls those snapshots at a bounded interval while the Staff pid is alive. Inner writes a durable dispatch receipt at claim time and ignores ACA exec SIGHUP/SIGPIPE. Terminal receipts clear pid so a live Staff process is not mistaken for an in-flight owner. The read-only snapshot fail-closed projects worker `dispatch_reason` and `owner_status` from that receipt (enumerated snake_case tokens only; never secrets, never send authority, never retry). A saveDraft throw also projects closed `save_stage` (bounded snapshot/persist identifier) and PostgreSQL `pg_code` (SQLSTATE only; never SQL, PII, credentials, or raw messages). If the receipt process is gone and selected-operation approval/journal/provider counts plus Graph arrivals are all zero, classify `proven_no_send` and allow **replacing that no-retry dispatch marker for this proven-no-send attempt only**. A replacement send still requires a fresh operator nonce (the consumed nonce stays replay-blocked) and still refuses if a prior owner pid is alive or any count is nonzero.
4. **Always** restore both flags to `false` and endpoint automation `off` in supervisor `finally`. Restore must similarly accept only a same-image successor with 100% Healthy traffic, a running replica of that revision, immutable revision-show flags literal `false` (or one successful `printenv`), and final channel mode `off`. Restore uses the same 10-minute per-successor readiness wait as enable. No retry after the inner dispatch marker.
