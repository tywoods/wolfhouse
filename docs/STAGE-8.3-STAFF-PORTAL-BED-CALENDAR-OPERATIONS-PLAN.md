...
**Status:** 8.3 PLANNING DONE (2026-06-02). **8.3a–8.3d IMPLEMENTATION DONE (2026-06-02).** **Phase 10.7 checkpoint (2026-06-04):** Staff Portal operations **COMPLETE ENOUGH / PASS** on staging (`5011975` → `wh-staging-staff-api--0000109`) — manual booking · payments · ledger · edits · move · cancel · drawer Refresh · calendar source colors (green/blue/purple) + payment badges · Tour Operator whole-room create + A/B release remainder beds. **8.3e write-gate plan DONE** — see [`STAGE-8.3E-MANUAL-BOOKING-WRITE-GATE-PLAN.md`](STAGE-8.3E-MANUAL-BOOKING-WRITE-GATE-PLAN.md) (revised slice numbering authoritative there). See per-slice proofs below.
...
| **8.3g** | **Conflict / availability preview helper** � `scripts/lib/staff-manual-booking-availability.js`; pure JS; `previewManualBookingAvailability()`; half-open overlap; cancelled/expired exclusion; 7 blockers; 5 warnings; `verify-staff-manual-booking-availability.js` **52/52 PASS**. No DB. No API. No writes. | **DONE** |
| **8.3h** | **Preview / proposal endpoint** � `POST /staff/manual-bookings/preview`; SELECT-only; auth-gated (operator+); calls `previewManualBookingAvailability()`; returns preview_only/creates_booking/no_write_performed safety fields; `verify-staff-manual-booking-preview-api.js` **48/48 PASS**; proof 31/31 PASS. | **DONE** |
...
| **8.3s** | **Batch Azure deploy** — 8.3l+8.3q+8.3r deployed to wh-staging-staff-api. Image: wh-staff-api:1894036-8x3s-batch (ACR run cbb). Revision: --0000012 (100% traffic, Healthy). /staff/login 200; auth guard active; STAFF_ACTIONS_ENABLED=false; WHATSAPP_DRY_RUN=true; Write actions DISABLED in logs. Content proof: image built from commit 1894036 (all three panels confirmed by local proof). No DB writes. n8n untouched. | **DONE** |
| **8.3u** | **Operations UI correction** — Tour Operator + Room Release moved to new Tour Operator tab; forms use date dropdowns (no cal selection needed); bc-op-panel/bc-rr-panel removed from Bed Calendar; to-op-panel + to-rr-panel in tab-tour-operator; bcHandleCellClick td.dataset bug fixed (cell selection now works); Demo Range chip removed; booking drawer: code-only title, Room/Beds merged into Stay, duplicate section removed; 164/164 verifier PASS. No API. No DB writes. No Azure. | **DONE** |
| **8.3v** | **Azure deploy (8.3u)** — image wh-staff-api:ea2437d-8x3v-ui-corrections (ACR run cbc). Revision: --0000013 (100% traffic, Healthy). Smoke: /staff/login 200; auth-guard redirects /staff/ui; STAGING+SHADOW MODE+STAFF ACTIONS DISABLED badges confirmed. Source check: data-tab=tour-operator PASS; to-op-panel PASS; to-rr-panel PASS; data-chip=demo ABSENT PASS; bc-op-panel ABSENT PASS; td.dataset.date PASS. Write actions DISABLED in logs. STAFF_ACTIONS_ENABLED=false; WHATSAPP_DRY_RUN=true. No DB writes. n8n untouched. | **DONE** |
