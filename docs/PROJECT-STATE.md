...
**Last updated:** 2026-06-01 (**Stage 7.7c — conversation inbox UI DONE**. `buildUiHtml` in `scripts/staff-query-api.js` rebuilt as two-tab Cami dashboard: Conversations tab (default, inbox from `GET /staff/conversations`, priority pills, detail pane, Luna draft read-only, 401 surface) + Query Tools tab (existing). New: `scripts/verify-staff-conversation-ui.js` (34/34 PASS). Fixture proof: `GET /staff/ui` 200 HTML; fixture row visible; detail pane renders; 9 `api:conversation.*` audit entries; protected table delta=0; cleanup. Prior: 7.7b conversation API read endpoints DONE. `lunafrontdesk.com` domain purchased (DNS not configured, no TLS). Pilot: NO_GO.)
...
| **7** Scalable | **IN PROGRESS** (2026-06-01) — 7.0–7.7 DESIGN DONE. **7.2b DONE** · **7.2c DONE** · **7.3b DONE** · **7.7a/b/c DONE** — **7.7c Cami inbox UI**: two-tab dashboard (Conversations default + Query Tools), inbox renders from `GET /staff/conversations`, detail pane, priority pills, Luna draft read-only, 401 surface, 34-check verifier PASS, fixture proof PASS. `lunafrontdesk.com` purchased. No Azure resources. Staging/prod NOT secure. Next: 7.7d conversation detail + message thread. Plan: [PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md](PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md). | Multi-client + Azure when approved |
