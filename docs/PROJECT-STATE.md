...
**Last updated:** 2026-05-31 (**Stage 7.7 amendment — inline staff reply made explicit hard requirement** in PHASE-7.7: inline composer + edit-draft + copy/manual-send (Phase 1) + approve/send gate (deferred) + staff takeover/return-to-Luna (deferred write) + mark-replied-manually (deferred) + reply audit trail. New endpoints: GET /staff/conversations/:id/staff-state, POST /staff/conversations/:id/staff-reply/draft, mark-replied-manually, takeover, return-to-luna, approve-send. Slices renumbered: 7.7j=inline-reply-composer-proof, 7.7k=takeover-plan, 7.7l=approve-send-gate-plan, 7.7n=bed-reassign-plan, 7.7o=booking-edit-gate-plan (15 slices total). PHASE-7.6 F2/F3/F7 expanded; inline-reply added as hard no-go condition. Prior: Stage 7.7 dashboard + bed calendar plan DONE — design-only: inbox/conversation-detail/Luna-draft/booking+addon-context/handoff-queue/daily-ops views + spreadsheet-style bed calendar grid (rooms×dates, booking_beds as date-span blocks, planning-row-format color model). Bed calendar = HARD requirement before live launch unless Cami/Ale write a deferral. Read-only first; send/edit/resolve deferred behind gates. 13 implementation slices (7.7a–7.7m). Missing read endpoints listed (/staff/conversations*, /staff/bed-calendar*). PHASE-7.6 Section F expanded to F1–F8 + F7-CAL (81 gates total). Doc: [PHASE-7.7-CAMI-REVIEW-DASHBOARD-PLAN.md](PHASE-7.7-CAMI-REVIEW-DASHBOARD-PLAN.md). Nothing built. Pilot: NO_GO.)
...
| **7** Scalable | **PLANNING CLOSED / IMPLEMENTATION STARTED** (2026-05-31) — 7.0–7.7 DESIGN DONE. **7.2b migration 009 APPLIED** + **7.2c auth middleware scaffold APPLIED** + **7.3b IaC scaffold PASS** + **7.7 Cami dashboard + bed calendar plan DONE**: dashboard design (inbox/detail/Luna-draft/context/handoff/bed-calendar), bed calendar = hard launch gate, read-only first, 13 slices, missing endpoints listed; PHASE-7.6 F-section → F1–F8+F7-CAL (81 gates). No Azure resources, no dashboard built. Staging/prod NOT secure. Next: 7.7b conversation read endpoints (first build slice) or 7.3c DNS/TLS. Plan: [PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md](PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md). | Multi-client + Azure when approved |
