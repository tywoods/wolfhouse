...
| **5** Clean | **5.1 planning complete (2026-05-30)** | Targeted SoT cleanup for Wolfhouse pilot readiness — **not started for implementation**. Plan: [PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md](PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md). **5.1 conversation PG path** fully planned: `IF Conversation Exists?` → PG-primary, `Merge Session State` → PG-first, conversation upsert gate removed (conversations ≠ protected table), FK null-safety for dry-run holds. `session_state` contract documented. Implementation not started. **Next step:** approve 5.1 implementation → begin 5.1-S1 (build-script patch, no runtime until gate). |
