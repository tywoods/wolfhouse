...
| **5** Clean | **5.1 PASS. 5.2 CLOSE WITH DEFERRALS. 5.3a–5.3c STATIC DONE. 5.3d PARTIAL PASS (3× retried, hold-path design blocker).** Formula fix applied (`applyActiveBookingHoldIdGuard` enabled — `FALSE()` guard proven in exec 1241). Seed proof: `WH-53-FIXTURE-001` payment_pending + `checkout_created` payments row; all 6 staff queries correct. `Search Active Booking - Current Hold ID` now returns 0 records for fixture phone ✅. BUT `Postgres - Ensure Booking In Postgres` unreachable without Airtable hold: `Search Hold With Guest Details` returns empty → `IF - Hold Found` FALSE. **Next: Stage 5.3e — add fixture stub for `Search Hold With Guest Details` + update webhook message (email + payment link keywords) to reach ensure-promote node.** | Targeted SoT cleanup for Wolfhouse pilot readiness. Plan: [PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md](PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md). |
...
**Immediate next step: Stage 5.3e — add fixture stub for `Search Hold With Guest Details`.** The ensure-promote node (`Postgres - Ensure Booking In Postgres`) is only reachable via `Search Hold With Guest Details` (Airtable hold search) → `IF - Hold Found` TRUE. For fixture phone `+34600000155` with no Airtable hold, `IF - Hold Found` takes the FALSE branch. Fix: add an `IF - Stage53 Fixture Hold Search?` stub in `build-main-local-stripe.js` that returns a synthetic hold record for the fixture phone when `STAGE53_FIXTURE_PAYMENT=true`. Also update the webhook message to include email + "payment link" keywords (`hasContact=true`, `has_payment_link_intent=true`) so BSR routes to `payment_details_provided`.
