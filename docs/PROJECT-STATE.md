...
**Last updated:** 2026-06-01 (**Stage 7.7h — bed calendar read-only render DONE**. New Bed Calendar tab in `/staff/ui`: date range inputs, Load button, READ-ONLY BED CALENDAR label, scrollable grid (rooms/beds left, dates top, colored colspan blocks), A/D markers, read-only block detail panel. CSS classes: confirmed/hold/payment_pending/needs_review/cancelled. New verifier 30/30 PASS. Proof: GET /staff/ui 200 all checks pass; GET /staff/bed-calendar days=7 rooms=10 block confirmed; audit OK; delta=0. Prior: 7.7g bed calendar API. `lunafrontdesk.com` purchased (DNS not configured). Pilot: NO_GO.)
...
| **7** Scalable | **IN PROGRESS** (2026-06-01) — 7.0–7.7 DESIGN DONE. **7.2b/c DONE** · **7.3b DONE** · **7.7a–d + 7.7f + 7.7g + 7.7h DONE** — **7.7h**: Bed Calendar tab in `/staff/ui`; date range controls; scrollable grid (rooms/beds left, dates top, colored blocks); A/D markers; read-only detail panel; 30-check verifier PASS; proof PASS. No Azure. Staging/prod NOT secure. Next: 7.7i booking detail drawer or 7.7j reply composer. Plan: [PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md](PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md). | Multi-client + Azure when approved |
