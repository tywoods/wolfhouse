...
**Last updated:** 2026-06-01 (**Stage 7.7f — handoff queue integration DONE**. Conversations tab now has Inbox and Needs Human sub-tabs. New `GET /staff/handoffs` endpoint (auth-gated, audited, `api:handoffs.open`) returns open `staff_handoffs` rows plus `needs_human_without_handoff` reconciliation list. Handoff queue table shows priority (URGENT/HIGH pill), guest, phone, reason, status, assigned staff, booking code, opened_at, time since opened (relative; red when stale). Row click navigates to linked conversation detail. READ-ONLY HANDOFF QUEUE label; resolve-disabled notice; no resolve button. `verify-staff-conversation-ui.js` extended to 52 checks. Fixture proof: open handoffs=1; `api:handoffs.open OK hq=1`; protected table delta=0. Prior: 7.7d thread+draft panel. `lunafrontdesk.com` purchased (DNS not configured). Pilot: NO_GO.)
...
| **7** Scalable | **IN PROGRESS** (2026-06-01) — 7.0–7.7 DESIGN DONE. **7.2b/c DONE** · **7.3b DONE** · **7.7a–d + 7.7f DONE** — **7.7f**: Needs Human sub-tab + `GET /staff/handoffs` endpoint; priority/time-since-open queue; row click → conversation detail; 52-check verifier PASS; fixture proof PASS. No Azure. Staging/prod NOT secure. Next: 7.7g bed calendar or 7.7e Luna draft panel. Plan: [PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md](PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md). | Multi-client + Azure when approved |
