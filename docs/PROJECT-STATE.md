...
**Last updated:** 2026-05-30 (Stage 4 closed-month guard IMPLEMENTED — static + import-inactive verified; A5 runtime re-test pending)
...
**Stage 4 A5 closed-month guard IMPLEMENTED (2026-05-30):** Added deterministic closed-month guard to booking_flow path in build-main-local-stripe.js. Function: applyClosedMonthGuard(workflow, CLOSED_MONTHS_CONFIG). New nodes: Code - Check Closed Month (reads session.check_in/check_out, parses month names, compares against closed_months literal injected at build time); IF - Closed Month? (true→Reply - Closed Month, false→IF - Ready For Availability); Reply - Closed Month (chainLlm, claude-haiku-4-5, LLM prompt with closed_months/suggested_open_months/language context); Anthropic Chat Model (Closed Month) (model sub-node). Wiring: Determine Missing Fields → Code - Check Closed Month (no longer direct to IF-Ready). Reply - Closed Month → IF - DRY RUN? (Create Outbound Message) — same sink as Generate Next Reply. Parser Node advisory injection: added operational context note before Schema block so LLM knows closed months. Verifier: new verifyClosedMonthGuard(workflow, closedMonths) called from runVerifyTargets. Tag: stage4-cm-guard (shortened from stage4-closed-month-guard to respect varchar(24) limit on tag_entity.name). All static checks: PASS (shadow-mode OK, closed-month guard OK, payment contract OK, rooming contract OK, runner syntax OK). Import-inactive: OK — 346 nodes in n8n DB, active=false, all 4 guard nodes confirmed. A5 runtime re-test pending.