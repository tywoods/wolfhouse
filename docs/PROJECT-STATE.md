...
| **5** Clean | **5.1 PASS — PG-primary conversation memory proven (2026-05-30).** | Targeted SoT cleanup for Wolfhouse pilot readiness. Plan: [PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md](PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md). **5.1 runtime gate PASS:** A3 T1/T2 (execs 1214/1215) + A4 T1/T2 (execs 1216/1217) — no runner seed. T1 writes conversation naturally via `Postgres - Upsert Conversation Hold` (pg_ok=true, booking_not_in_pg=true). T2 reads from PG: `IF Conversation Exists?` → TRUE via PG conversation_id; `Merge Session State` old_state from PG. Protected counts Δ=0. Cleanup: 2 rows deleted. **Known gap:** session_state from T1 missing `package` — BSR lacks full booking context in T2; LLM classifies payment intent from message alone (conf≥0.85). **Deferred:** A2 (T1 no hold = no PG write → Stage 5.1b); package in session_state. **Next:** Stage 5.1b — write session on non-hold routing paths. |
