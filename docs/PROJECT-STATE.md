...
**Last updated:** 2026-06-02 (**Stage 8.3u DONE** — Operations UI correction pass. Tour Operator + Room Release moved to new top-level Tour Operator tab (tab-tour-operator); forms use date dropdowns, no calendar selection dependency. bcHandleCellClick td.dataset bug fixed — cell selection now works. Demo Range chip removed. Booking drawer: code-only title, Room/Beds merged into Stay, duplicate section removed. 164/164 verify-staff-bed-calendar-ui PASS. All 10 static verifiers PASS. 18/18 local HTML checks PASS. No API. No DB writes. No Azure. Azure proof deferred.)
