...
**Last updated:** 2026-06-01 (**Stage 7.7g — bed calendar query/API DONE**. New `GET /staff/bed-calendar?start=YYYY-MM-DD&end=YYYY-MM-DD` endpoint (auth-gated viewer minimum, audited `api:bed_calendar`). Returns `days[]`, `rooms[]` hierarchy (10 rooms/52 beds local), `blocks[]` with `start_offset`, `span_days`, `color_type`, `is_arrival`, `is_departure`. Date validation: format, end>start, max 90d (400 otherwise). New helpers: `getBedCalendarRoomsQuery`, `getBedCalendarBlocksQuery`, `getBedCalendarSummaryQuery`. Verifiers: 25+28 checks PASS. Fixture proof: booking_beds baseline=15, after seed=16, block found start_offset=0 span_days=7 color_type=confirmed, audit `api:bed_calendar OK`, cleanup delta=0. Prior: 7.7f handoff queue. `lunafrontdesk.com` purchased (DNS not configured). Pilot: NO_GO.)
...
| **7** Scalable | **IN PROGRESS** (2026-06-01) — 7.0–7.7 DESIGN DONE. **7.2b/c DONE** · **7.3b DONE** · **7.7a–d + 7.7f + 7.7g DONE** — **7.7g**: `GET /staff/bed-calendar` endpoint; rooms/days/blocks response; date-span logic; 25+28-check verifiers PASS; fixture proof PASS; delta=0. No Azure. Staging/prod NOT secure. Next: 7.7h bed calendar UI render. Plan: [PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md](PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md). | Multi-client + Azure when approved |
