...
**Last updated:** 2026-06-02 (**Stage 8.3l DONE** — manual booking preview UI wired to read-only POST /staff/manual-bookings/preview. Preview Conflicts button enabled on cell selection; result panel shows valid/blocked/warning/error states; Create Manual Booking remains disabled; 122/122 verify-staff-bed-calendar-ui PASS; preview_only=true, creates_booking=false, no_write_performed=true confirmed locally. Fixed bd.capacity schema bug in getManualBookingPreviewBedsQuery. No confirmed create. No staff writes. Staff writes disabled. Manual booking creation disabled. STAFF_ACTIONS_ENABLED=false. MANUAL_BOOKING_ENABLED=false. WHATSAPP_DRY_RUN=true. Azure proof pending.)
