...
**Last updated:** 2026-06-01 (**Stage 7.7k — safe bed reassignment plan DONE (design only, docs)**. New §5a in dashboard plan: allowed v1 = same-range bed→bed move only (no date/guest/payment/cancel/status change, no guest notify); 14 hard blockers; 7 warning-confirm cases; in-transaction half-open overlap guard with row lock (no DB EXCLUDE constraint today — future btree_gist hardening noted); full audit row spec; before/after rollback with conflict-on-undo → handoff; UI proposal→confirm→gated-write flow; 8 sub-slices 7.7k1–7.7k8. No code, no DB writes. Pilot checklist F8 → DESIGN_DONE; F8-CAL-EDIT added (editable calendar not required for Phase 1). Prior: 7.7j copy/review shadow workflow. Pilot: NO_GO.)
