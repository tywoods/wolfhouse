...
**Last updated:** 2026-06-01 (**Stage 7.7i — booking detail drawer DONE**. `GET /staff/bookings/:bookingCode/context` endpoint added — returns booking, payments, rooming, conversation, handoff, add-ons. 6 query helpers in `staff-booking-detail-queries.js`. UI `showBlockDetail` upgraded: fetches context, renders enriched drawer with Booking/Payments/Rooming/Conversation/Handoff/Add-ons sections + "Open conversation" button. Verifiers: 27/27 + 26/26 + 40/40 PASS. Proof: WH-77I-DETAIL-001 fixture → context 200 + all sections; audit `api:booking_context` OK; delta=0. Prior: 7.7h bed calendar render. `lunafrontdesk.com` purchased (DNS not configured). Pilot: NO_GO.)
...
| **7** Scalable | **IN PROGRESS** (2026-06-01) — 7.0–7.7 DESIGN DONE. **7.2b/c DONE** · **7.3b DONE** · **7.7a–d + 7.7f + 7.7g + 7.7h + 7.7i DONE** — **7.7i**: `GET /staff/bookings/:bookingCode/context`; enriched drawer (Booking/Payments/Rooming/Conversation/Handoff/Add-ons); Open conversation button; 6 query helpers; 27+26+40 verifier checks PASS; proof PASS. No Azure. Staging/prod NOT secure. Next: 7.7j copy/review workflow or 7.7k safe reassignment plan. Plan: [PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md](PHASE-7-PRODUCTION-HARDENING-PILOT-PLAN.md). | Multi-client + Azure when approved |
