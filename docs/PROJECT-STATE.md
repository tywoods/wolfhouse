...
**Last updated:** 2026-05-31 (Stage 5.4 CLOSE WITH DEFERRALS — confirmation state query semantics proven; `confirmation_sent_at` gate verified)
...
| **5** Clean | **5.1 PASS. 5.2 CLOSE WITH DEFERRALS. 5.3 CLOSE WITH DEFERRALS. 5.4 CLOSE WITH DEFERRALS (2026-05-31).** Stage 5.4: `getConfirmationNeededQuery()` gate semantics proven — `confirmation_sent_at=NULL` appears in Query F; `confirmation_sent_at IS NOT NULL` excluded; 12 assertions green; `booking_beds` unchanged. **Next: Stage 5.5 — rooming/bed assignment SoT.** | Targeted SoT cleanup for Wolfhouse pilot readiness. Plan: [PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md](PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md). |
...
**Immediate next step: Stage 5.5 — rooming/bed assignment SoT.** Stage 5.4 CLOSE WITH DEFERRALS (2026-05-31): `getConfirmationNeededQuery()` gate semantics proven — needs-confirmation fixture appears; already-confirmed fixture excluded; 12 assertions green; `booking_beds` unchanged; no workflow activation. Stage 5.3 CLOSE WITH DEFERRALS: all six staff payment queries + `payment_balances` proven; fixture ensure-promote/payment path proven (exec 1245); confirmation-needed query proven; combined smoke PASS.
