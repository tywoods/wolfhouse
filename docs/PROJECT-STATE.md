...
| **5** Clean | **5.1 static implementation committed (2026-05-30, e0b58d5). Runtime gate planned.** | Targeted SoT cleanup for Wolfhouse pilot readiness. Plan: [PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md](PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md). **5.1 static patches committed:** `IF Conversation Exists?` PG-primary (OR combinator), `Merge Session State` PG-first, `Postgres - Upsert Conversation Hold` gate removed, FK null-safety in conversation upsert SQL (`booking_not_in_pg` flag). 345-node workflow regenerated. **Runtime gate:** A3+A4 T1/T2 without runner seed. A2 deferred (Stage 5.1b — T1 no hold = no PG write). Cleanup SQL: `scripts/fixtures/stage5.1-conversations-cleanup.sql`. **Next:** activate Main, run A3-T1/T2 + A4-T1/T2, verify PG conversation written and read naturally. |
