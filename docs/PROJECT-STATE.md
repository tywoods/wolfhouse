...
**Last updated:** 2026-06-02 (**Stage 8.3r DONE** — operator room release UI skeleton. bc-rr-panel card with operator-block/release-dates/release-scope/notes sections; release type select (selected_beds/whole_room/selected_dates); Guest messaging=Disabled, Stripe=Disabled, n8n=Not triggered, Staff write=Disabled in staging; prefills release-start/end/nights/room/bed from cell selection; Release Dates and Preview Release both disabled; safety notice; 162/162 verify-staff-bed-calendar-ui PASS. No API route. No DB writes. No Azure deploy. Staff writes disabled. Azure proof deferred for batch deploy.)
