...
| **5** Clean | **Planning only (2026-05-30)** | Targeted SoT cleanup for Wolfhouse pilot readiness — **not started for implementation**. Plan: [PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md](PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md). Goals: PG-primary bookings/payments/conversations, structured add-on records, staff handoffs, `payment_balances` view. Live operation and real WhatsApp remain unapproved. **Next step:** approve 5.0 plan → begin 5.1 conversation PG path (dry-run first). |
