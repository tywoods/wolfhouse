...
| **5** Clean | **5.1 PASS — PG-primary conversation memory proven (2026-05-30). 5.1b STATIC DONE (2026-05-30). 5.1c STATIC DONE (2026-05-30) — runtime pending.** | Targeted SoT cleanup for Wolfhouse pilot readiness. Plan: [PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md](PHASE-5-SOURCE-OF-TRUTH-CLEANUP.md). **5.1 runtime gate PASS:** A3 T1/T2 (execs 1214/1215) + A4 T1/T2 (execs 1216/1217) — no runner seed. T1 writes conversation naturally via `Postgres - Upsert Conversation Hold` (pg_ok=true, booking_not_in_pg=true). T2 reads from PG: `IF Conversation Exists?` → TRUE via PG conversation_id; `Merge Session State` old_state from PG. Protected counts Δ=0. Cleanup: 2 rows deleted. **5.1b static:** `session_state` extended to include `package`, `language`, `route`, `room_type`, `room_preference`, `guest_name`, `guest_email`, `missing_fields` — each written only when non-null/non-empty (null-safe). Verifier asserts package+language presence. All static checks pass, active=false. **5.1c static:** `Postgres - Write Session State` added on `IF - Ready For Availability` FALSE branch (missing-fields path). SQL upserts `conversations` only; no FK to bookings; session_state merges existing \|\| incoming. `verifyPGSessionWrite` (7 checks) wired into `--verify-targets`. 346 nodes, active=false. **Next:** Stage 5.1c A2 runtime gate — run A2 T1+T2 without runner seed; prove PG write on missing-fields turn and natural PG read on T2. |
