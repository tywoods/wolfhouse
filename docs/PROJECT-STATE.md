...
**Last updated:** 2026-06-02 (**Stage 8.3k DONE** — manual booking rollback/undo proof. scripts/lib/staff-manual-booking-rollback-sql.js: 10-blocker CTE rollback helper; CASCADE deletion via DELETE FROM bookings (ON DELETE CASCADE handles booking_beds+payments); pre_beds+pre_payments pre-capture CTEs for reporting; MANUAL_BOOKING_ROLLBACK_BLOCK_CODES (10 codes: confirm_not_set, staff_role_insufficient, client_not_found, booking_not_found, not_manual_staff_booking, not_manual_created, confirmation_already_sent, unsafe_payment_exists, rollback_payload_code_mismatch, rollback_payload_id_mismatch); audit written on every attempt (booking_id=NULL, FK-safe); erify-staff-manual-booking-rollback-sql.js: **52/52 PASS**; proof stage8.3k-manual-booking-rollback-proof.js: **59/59 PASS** — B (happy path create→rollback: booking+beds+payments deleted, audit written, booking_id=NULL confirmed), C (blocker tests: confirm=false, role=viewer, code mismatch, payload id mismatch — all block correctly, booking preserved), D (unsafe payment blocker: checkout_created payment blocks rollback), E (double-rollback: second attempt blocked=booking_not_found); final protected table delta=0. No API route. No UI wiring. No Azure. STAFF_ACTIONS_ENABLED=false. MANUAL_BOOKING_ENABLED=false. Staff writes disabled. Prior:  2026-06-02 (**Stage 8.3j DONE** � schema-alignment fix for `buildManualBookingCreateSql()`. P1: removed `language` from `bookings` INSERT ? stored in `metadata` JSONB. P2: `inserted_payment` fixed to use `status`/`payment_kind`/`amount_due_cents`/`currency` (no `provider`/`amount_cents`/`payment_status`). P3: `audit_written` uses `workflow_name`+`message` (no `event_type`). Also fixed `$16::booking_status`, `$17::payment_status` enum casts and `$5::text IS NOT NULL` for parameter type inference. `verify-staff-manual-booking-create-sql.js`: **47/47 PASS** (7 new 8.3j schema checks added). Proof (`stage8.3i-manual-booking-create-proof.js`) updated: `patchProofSql` removed, SQL used directly; **65/65 PASS** (29 assertions A1�A29 incl. payment row + audit row + metadata.language verification); final delta=0. All other verifiers PASS. No API route. No UI wiring. No Azure. `STAFF_ACTIONS_ENABLED=false`. `MANUAL_BOOKING_ENABLED=false`. Staff writes disabled. Prior: **Stage 8.3y Azure deploy DONE** � Needs Human tab converted to same two-column conv-card layout as Inbox (filtered to `needs_human`/open handoff); `loadConvDetail(convId, targetEl)` refactored to support both Inbox and NH right panels; `handoffLabel()` reused in `renderHandoffQueue()`; raw `reason_code` no longer directly passed to `escHtml`; Booking sidebar card moved above Bot state; `kv('Pending',�)` and `kv('Last reply',�)` removed from Bot state; check-in/check-out combined as `kv('Stay', fmtDateOnly(check_in) + ' ? ' + fmtDateOnly(check_out))`; `fmtDateOnly()` helper added; "Messages" h3 title removed from thread section; `.hq-table`/`hq-tbody`/`hq-table-wrap` CSS/HTML removed; `#hq-right` flex panel added with same `inbox-two-col` layout. `verify-staff-conversation-ui.js`: **77/77 PASS** (11 new Stage 8.3y checks added). All other verifiers PASS. `node --check` PASS. No API changes. No DB writes. No write routes. No live WhatsApp/Stripe/n8n. `STAFF_ACTIONS_ENABLED=false`. `MANUAL_BOOKING_ENABLED=false`. Azure deploy pending. Prior: **Stage 8.3x Azure deploy DONE** � image `4e02763-8x3x-inbox` deployed, revision `wh-staging-staff-api--0000010` Healthy. Safety flags: `STAFF_ACTIONS_ENABLED=false`, `WHATSAPP_DRY_RUN=true`, `STAFF_AUTH_REQUIRED=true`. n8n untouched. `scripts/staff-query-api.js`: Inbox tab converted to persistent two-column layout (`.inbox-two-col` flex container); left column = conversation card list (`.conv-card` � guest name, phone, priority pill, friendly handoff label); right column = detail panel (always visible, empty state "Select a conversation to review." when nothing selected); `handoffLabel(code)` added � maps 11 raw codes to friendly labels (`date_change_requested` ? "Date change request", `payment_inquiry` ? "Payment question", etc.); `renderInbox()` replaced table rows with `.conv-card` divs; `loadInbox()` updated to reset right panel to empty state; `loadConvDetail()` header updated � raw "Stage:" removed, friendly handoff label shown; "Message thread � N messages" count title removed (now just "Messages"); "Back to inbox" button removed from HTML; `btn-back` JS handler guarded as null-safe; `.route` display removed from message bubbles; `inbox-table`/`inbox-tbody`/`inbox-table-wrap` fully removed. `scripts/verify-staff-conversation-ui.js`: 66/66 PASS (7 new Stage 8.3x checks: `.inbox-two-col`, `conv-card`/`conv-list`, `handoffLabel()`, `date_change_requested` not directly in template, "Message thread" count removed, raw stage removed, `handoffLabel()` called in `renderInbox`). All other verifiers PASS unchanged. `node --check` PASS. No API changes. No DB writes. No write routes. No live WhatsApp/Stripe/n8n. `STAFF_ACTIONS_ENABLED=false`. `MANUAL_BOOKING_ENABLED=false`. Azure deploy pending. Known manual booking schema mismatches remain (bookings.language, payments columns, workflow_events columns) � NOT fixed in this task. Prior: **Stage 8.3i � Manual booking fixture write proof DONE** (local/test DB only, BEGIN/ROLLBACK). `scripts/fixtures/stage8.3i-manual-booking-create-proof.js`: new file; proves `buildManualBookingCreateSql()` CTE logic; 9 proof cases � happy-path (booking + booking_beds + audit rows inserted in txn, rollback_payload verified), idempotency duplicate (same idempotency_key in same txn ? is_duplicate=true, no second booking row), overlap conflict (overlapping date+bed ? overlap_conflict blocker), touching boundary (half-open, no conflict), invalid payment amounts (deposit > total ? blocked), confirm=false (? confirm_not_set blocked), staff role insufficient (viewer ? blocked), invalid dates (check_out before check_in ? blocked), client not found (unknown slug ? blocked); all transactions ROLLBACKed; final delta=0 on all 9 protected tables; graceful SKIP when local DB unavailable (exit 0); 3 schema mismatches documented and patched for proof execution: P1 `language` column missing on `bookings`, P2 `inserted_payment` CTE uses wrong column names (`provider`, `amount_cents`, `payment_status`), P3 `audit_written` CTE uses `event_type` instead of `workflow_name` + missing `message NOT NULL`; `proof:stage8.3i-manual-booking-create` added to `package.json`; `node --check` PASS. No API route. No UI wiring. No Azure changes. `STAFF_ACTIONS_ENABLED=false`. `MANUAL_BOOKING_ENABLED=false`. Staff writes still disabled. Manual booking NOT enabled. Prior: **Stage 8.3h DONE**. `POST /staff/manual-bookings/preview`: auth-gated (operator/admin/owner); SELECT-only DB queries via `scripts/lib/staff-manual-booking-preview-queries.js` (beds metadata + existing assignments + client lookup); calls `previewManualBookingAvailability()`; returns `preview_only:true`, `creates_booking:false`, `no_write_performed:true`, `staff_actions_enabled:false`, `manual_booking_enabled:false`, `input_summary`, `availability` (full helper output), `next_step`; file-only audit (no workflow_events DB write); does NOT require `STAFF_ACTIONS_ENABLED` or `MANUAL_BOOKING_ENABLED`; 405 for non-POST; SQL injection guards applied. `scripts/lib/staff-manual-booking-preview-queries.js`: SELECT-only SQL helpers (beds, assignments, client); half-open overlap params; no mutation SQL. `scripts/verify-staff-manual-booking-preview-api.js`: **48/48 PASS**. `scripts/fixtures/stage8.3h-manual-booking-preview-proof.js`: **31/31 PASS** (valid preview, overlap conflict, invalid dates, no-write structural proof, SQL shape checks, live probe). `package.json`: `verify:staff-manual-booking-preview-api` added. Manual booking creation NOT enabled. `STAFF_ACTIONS_ENABLED=false`. No Azure changes. Prior: **Stage 8.3g DONE**. (static code only). `scripts/lib/staff-manual-booking-availability.js`: new file; pure JS; `previewManualBookingAvailability()` export; half-open overlap detection (existing_start < proposed_check_out AND existing_end > proposed_check_in); cancelled/expired exclusion (`NON_BLOCKING_STATUSES`); 7 blockers (`invalid_dates`, `no_selected_beds`, `bed_not_found`, `bed_inactive_or_unsellable`, `overlap_conflict`, `guest_count_exceeds_selected_beds`, and guest-count-less warning); 5 warnings (`same_day_arrival`, `next_day_arrival`, `long_stay`, `protected_room_selected`, `operator_room_selected`); no DB calls, no SQL, no pg-connect, no fetch, no fs writes, no API route; structured output: `is_valid`, `has_conflict`, `blockers`, `warnings`, `proposed_nights`, `selected_bed_count`, `selected_beds`, `conflict_beds`, `conflict_assignments`, `availability_by_bed`, `summary`. `scripts/verify-staff-manual-booking-availability.js`: **52/52 PASS** (static checks + 8 functional fixture tests A�H). `package.json`: `verify:staff-manual-booking-availability` added. Manual booking writes NOT implemented. `STAFF_ACTIONS_ENABLED=false`. No Azure changes. Prior: **Stage 8.3f � Manual booking SQL helper static proof DONE** (static code only). `scripts/lib/staff-manual-booking-create-sql.js`: new file; 15-CTE chain; `MANUAL_BOOKING_BLOCK_CODES` (14 codes); `buildManualBookingCreateSql()` returns parameterised SQL; 24-param contract; half-open overlap guard + defense-in-depth NOT EXISTS; idempotency via `metadata` JSONB interim (column migration documented for 8.3g+); audit_payload + rollback_payload CTEs; `audit_written` on every attempt; `confirmation_sent_at = NULL`; no Stripe/WhatsApp/n8n; no DB client import; no API route. `scripts/verify-staff-manual-booking-create-sql.js`: **40/40 PASS**. `package.json`: `verify:staff-manual-booking-create-sql` added. Manual booking writes NOT implemented. `STAFF_ACTIONS_ENABLED=false`. No Azure changes. Prior: **Stage 8.3e � Manual booking write gate plan DONE** (docs-only). New `docs/STAGE-8.3E-MANUAL-BOOKING-WRITE-GATE-PLAN.md`: purpose, what-it-creates (one txn, no WhatsApp/Stripe/confirmation), required fields, hard blockers, warning/second-confirm cases, preview-first flow, audit, rollback, idempotency, revised contiguous slice numbering (manual booking 8.3e�8.3o; move/cancel/operator renumbered 8.3p�8.3w), staging gates, demo-vs-spreadsheet line, sign-off table. Pilot NO_GO; `STAFF_ACTIONS_ENABLED=false`; manual booking writes NOT implemented. Prior: 8.3d preview skeleton DONE. Full form: Selected Stay (pre-filled readonly), Guest, Payment (deposit/status/no-Stripe hint), Notes, Avail placeholder, Safety notice, disabled Create+Conflicts buttons. 105 verifier PASS. Azure rev `wh-staging-staff-api--0000009`. Prior: 8.3c selection model DONE. Empty cells carry data-date/room/bed; click to select; sage/teal highlight; selection panel with Check-in/Check-out/Nights; occupied-cell warning; "Create Manual Booking � coming soon" disabled; clear button. 86 verifier checks PASS. Azure rev `wh-staging-staff-api--0000008`. Prior: 8.3b booking detail drawer cleanup DONE. 6 grouped sections: Guest/Stay/Room-Beds/Payment/Add-ons/Conv-Handoff; nights badge; Total/Paid/Remaining balance �-rows; planned actions disabled stub; no raw UUIDs. 72 verifier checks PASS. Azure proof pending. Prior: Stage 8.3a bed calendar read-only cleanup DONE. date `type="date"` inputs, shortcut chips, always-visible color legend, A/D markers ? tooltip, operator+manual block colors, cleaner labels, free-bed count. 56 verifier checks PASS. Azure proof pending next build. Prior: Stage 8.3 operations plan DONE. Planning/docs only. Created `docs/STAGE-8.3-STAFF-PORTAL-BED-CALENDAR-OPERATIONS-PLAN.md`: reframes the bed calendar as the operations workspace; product language "Staff Portal"/"Luna Front Desk" (stop saying "Cami dashboard"; Cami/Ale stay named users in docs only); roles Staff/Operator/Admin/Owner/Developer. Expands slice 8.3 into 8.3a�8.3o: read-only cleanup (date-picker dropdowns, always-show all rooms/beds, clean labels, prominent blocks, receding empties, remove inline A/D ? hover, always-visible color legend, date shortcuts), drawer cleanup (grouped cards, clean room/bed codes, deposit/balance/total, disabled planned actions), calendar cell selection model (no writes), manual booking ladder (modal proposal-only ? SQL static ? fixture write ? gated API ? gated UI; deposit manual no Stripe; overlap-guarded; audited; rollback proof), move room/bed preview (7.7k gates), cancel/date-change/count-change design (paid?handoff), tour operator booking plan+impl (operator block records, locked defaults, unique calendar color), operator room release plan+impl (split logic exists in `operator-room-release-pg-sql.js`; migrate to backend module, keep n8n inactive; never free assigned-guest dates), dashboard extras roadmap (occupancy %, housekeeping, payments, demo reset/hide). Required before Ale/Cami demo = 8.3a+8.3b read-only only; writes gated for spreadsheet replacement. All safety flags hold; no code/DB/Azure/workflow changes. Pilot NO_GO. Prior: **Stage 8.6 � demo data seeded to Azure staging. Proof 28/28 PASS.**  Scripts: `stage8-demo-seed.js`, `stage8-demo-cleanup.js`, `stage8-demo-proof.js`. 18 rows (3 convs, 7 msgs, 3 bookings, 2 booking_beds, 1 handoff, 2 payments + 2 demo rooms + 4 demo beds). `STAFF_ACTIONS_ENABLED=false`, `WHATSAPP_DRY_RUN=true`. Demo data intentionally retained for Ale/Cami walkthrough. No live WhatsApp, no Stripe, no workflow activation. n8n untouched. Pilot: NO_GO. Prior: **Stage 8.5 � demo data seed plan DONE**. Planning/docs only. Created `docs/STAGE-8.1-DASHBOARD-UX-CLEANUP-PLAN.md`: default landing = "Today / Needs Attention" (needs-human + arrivals/departures + pending payments + occupancy snapshot, composed from existing read-only data); sidebar nav = Today � Inbox � Bed Calendar � Bookings � Add-ons/Activities � Handoffs � Settings/Admin � **Developer Tools (admin/dev-only)**; Query Tools + raw intents + JSON/debug + raw client_slug moved out of default staff view (display-only role gating; no new perms/writes); bed calendar cleanup (numeric room sort R1..R10, visible blocks, sticky labels, read-only); booking drawer grouped cards (Guest/Stay/Payment/Rooming/Add-ons/Conversation/Admin-collapsed); WhatsApp-like thread + "Draft � not sent" / "Copy and send manually" composer; canonical Luna design tokens (cream/sand/sage/dusty-blue/teal + new amber-warning/soft-red-error, spacing scale, radii, button/badge/table styles); desktop-first responsive; 8.2 boundaries (visual/HTML/CSS only, maintain verifiers, add hidden-Query-Tools checks); Sunset readiness debt captured. Prior: Stage 8.0 � client-ready staging roadmap DONE. Planning/docs only. Created `docs/STAGE-8-CLIENT-READY-STAGING-ROADMAP.md`: objective = make Luna Front Desk show-ready for Ale/Cami as a polished shadow-mode staging demo while keeping all live gates closed. 8 pillars (UI polish � real staff experience � demo data � Cami workflow readiness � manual-booking roadmap � safety/ops gates � DNS polish � multi-client awareness); slices 8.0�8.13; 14-item "ready to show" checklist; explicit Stage 8 exclusions (no production launch, live WhatsApp, live Stripe, autonomous send, Sunset onboarding, multi-client platform, n8n extraction, production billing). Pilot decision remains NO_GO. No code, no Azure changes, no workflow activation. Prior: Stage 7.3f � custom domain + TLS DONE. `staff-staging.lunafrontdesk.com` bound to Azure Container App; GoDaddy CNAME + TXT verification records added; Azure managed cert `mc-wh-staging-env-staff-staging-lu-5595` provisioned (`SniEnabled`); all smoke tests PASS on clean URL: `GET /` ? ok, `/staff/login` ? 200 Luna Front Desk branding + Company wording, `/staff/ui` unauth ? redirect, `/staff/intents` unauth ? 401. Safety flags unchanged. n8n untouched. Prior: Stage 7.3e-fix � logout repaired + Company wording Root cause: `doLogout` was defined inside UI IIFE, not accessible from `onclick` global scope; fixed via `window.doLogout = function doLogout(){...}`. Labels "Client" ? "Company" in login page + query/conversations/bed-calendar panels (field IDs/values/API params/DB unchanged). All 7 verifiers PASS (31/31 login-ui, auth-api, 39/39 query-api, 29/29 query-ui, 52/52 conversation-ui, 40/40 bed-calendar-ui). Image `wh-staff-api:6fcf6ab` deployed; revision `0000003` active; smoke tests PASS over HTTPS. Safety flags unchanged. n8n untouched. Prior: Stage 7.3e Azure proof PASS � real login page deployed to Azure staging Image `wh-staff-api:dc8c86c` built/pushed to ACR (build `cb1`); Container App revision `wh-staging-staff-api--0000002` active; safety flags confirmed unchanged (STAFF_AUTH_REQUIRED=true, STAFF_ACTIONS_ENABLED=false, WHATSAPP_DRY_RUN=true, STRIPE_WEBHOOK_SKIP_VERIFY=false); smoke tests over HTTPS PASS: `GET /` ? ok, `GET /staff/login` ? 200 with Luna Front Desk branding + wolfhouse-somo + Staging/shadow mode + Staff actions disabled, `GET /staff/ui` (unauth) ? 302?login, `GET /staff/intents` (unauth) ? 401; n8n untouched; no workflow activation; no live WhatsApp/Stripe; no staff write enablement. Manual login test at `scripts/test-azure-login-proof.ps1`. Proof in `docs/PHASE-7.3D-AZURE-STAGING-DEPLOYMENT-LOGIN-PROOF.md �6`. Prior: Stage 7.3e � Luna Front Desk login page implemented (2026-06-01). `GET /staff/login` added: branded HTML form; client/email/password fields; XHR to `POST /staff/auth/login`; success?redirect to `/staff/ui`; error shown inline; "Staging / shadow mode" + "Staff actions disabled" badges; no external CDN; no OAuth. `browserLoginRedirect()`: unauthenticated browser to `/staff/ui` ? 302 to `/staff/login`; API callers still get JSON 401; `requireAuth` still enforced. Logout button in UI banner ? `doLogout()` XHR to `/staff/auth/logout` ? redirect to `/staff/login`. New verifier `scripts/verify-staff-login-ui.js`: 30/30 PASS. All existing verifiers PASS: auth-api, query-api (39/39), query-ui (29/29), conversation-ui (52/52), bed-calendar-ui (40/40). Local proof: `/staff/login` ? 200 with correct content; `/staff/intents` total=35. Prior: Stage 7.3d � Azure staging deployment + login proof DONE. Preflight doc created (`docs/PHASE-7.3C-AZURE-STAGING-DEPLOYMENT-PREFLIGHT.md`): Bicep scaffold validated (all 13 resources present, 5 safety defaults hardcoded, no real secrets), what-if command prepared, Phase A�M deployment plan defined, smoke tests defined, 26-check verifier PASS. Ty's parameter template created (`infra/azure/staging/parameters.ty-template.json`) with `<FILL_ME: ...>` placeholder tokens. Required manual inputs listed: subscription ID, region, budget, DNS provider for `lunafrontdesk.com`, staging subdomains (`staff-staging`, `n8n-staging`, `webhook-staging`), Postgres admin password, container image strategy. All static verifiers PASS (scaffold 57/57, preflight 26/26, auth API, query API, conversation UI 52/52, bed calendar 40/40). No Azure resources created. No deployment run. No DNS changed. No secrets set. Staging not live. Prior: Stage 7.7k8 � staging gate for editable bed reassignment DESIGN DONE. Pilot checklist F8-CAL-EDIT updated to DESIGN_DONE. Calendar editing NOT wired. All K8-G gates NOT_STARTED (staging not deployed). No live/staging reassignment enabled. Prior: Stage 7.7k7 � bed reassignment rollback/undo proof DONE. `scripts/lib/staff-bed-reassignment-sql.js`: added `new_bed_code` to `rollback_payload_cte` JSONB. Proof script `stage7.7k7-reassign-rollback-proof.js` (no API server, local DB only): Case A happy-path undo � move A?B (rows_updated=1) � rollback_payload contains booking_bed_id/old_bed_code/new_bed_code/dates � undo B?A (rows_updated=1, admin+override=true to bypass manual lock) � DB restored � dates unchanged � audit_event_id for both; Case B conflict-on-undo � move A?B � blocker seeded on old bed A (overlapping dates) � undo attempt blocked=true block_reason=target_bed_overlap rows_updated=0 � booking stays on B no double-booking; cleanup: all 6 protected table deltas=0 (workflow_events +3 during proof, cleaned up). 47/47 PASS. All existing verifiers PASS (38+25+32+59 checks). No UI wiring. No calendar edit. Cami/Ale sign-off required before staging/live reassignment. UI: `staff-query-api.js` title updated to "Luna Front Desk" (link to home). Prior: Stage 7.7k6 � admin-only manual/operator lock override DONE. `POST /staff/bed-calendar/reassign/confirm` extended: `manual_operator_lock_override:true` in request body; operator role ? 403 `insufficient_override_role` (blocked at API layer before SQL); admin/owner role ? bypasses `manual_operator_lock` only via `$9::boolean` param in `reassignBookingBedSql()`; all other safety blockers (overlap, confirm, cancelled, target-bed checks) remain unaffected; override fields (`manual_operator_lock_override_requested`, `manual_operator_lock_override_applied`, `override_role`) in SQL audit_payload and API response; verifier updated to 59/59 PASS; fixture proof 44/44 PASS � A: operator+override=false?409, B: operator+override=true?403, C: admin+override=false?409, D: admin+override=true?200 rows_updated=1 audit verified, E: cleanup all deltas=0. No UI wiring. No calendar edit. Prior: Stage 7.7k5 � confirmed bed reassignment write API endpoint DONE**. `POST /staff/bed-calendar/reassign/confirm`: STAFF_ACTIONS_ENABLED+STAFF_AUTH_REQUIRED dual gate; session auth, operator/admin/owner role; confirm:true body field required; reason required; booking_bed_id UUID validated; `reassignBookingBedSql()` with confirm=true inside BEGIN/COMMIT/ROLLBACK; rows_updated=1 success check; blocked?409 with block_reason; audit intent `api:bed_reassign_confirm`; 42-check proof PASS (GET?405, unauthenticated?401, viewer?403, missing confirm?400, valid?200 rows_updated=1, second POST?409 manual_operator_lock, all table deltas=0 after cleanup); verifier 48/48 PASS. No UI wiring. No calendar edit button. Prior: 7.7k4 local fixture write proof DONE. `stage7.7k4-reassign-confirm-proof.js`: 35/35 PASS. Proved: blocked=false, block_reason=null, rows_updated=1, old_bed?new_bed in DB, assignment_type='manual', workflow_events +1 audit row (via reassignBookingBedSql helper), rollback_payload contains old bed/id for undo, direct SQL undo restores old bed, all cleanup ? delta=0 on all protected tables. Note: undo via helper correctly blocked by `manual_operator_lock` (by design � requires explicit override); undo via direct SQL proven. NO API route. NO UI wiring. NO calendar writes. Fixture-only proof. Prior: 7.7k3 preview API DONE. `GET /staff/bed-calendar/reassign/preview`: operator+ auth, BEGIN/ROLLBACK (always), confirm=false ? blocked=confirm_not_set guaranteed, rows_updated=0 safety assertion, audit log written (intent: api:bed_reassign_preview), 400/404 validation. Local proof 18/18 PASS: HTTP 200, blocked=true, block_reason=confirm_not_set, rows_updated=0, booking_beds count unchanged (delta=0), protected tables untouched, audit log entry confirmed. Bug fixed in helper: missing ::text casts on jsonb params ($5,$6,$7). `verify-staff-bed-reassignment-preview-api.js`: 32/32 PASS. All suites PASS. Confirmed reassignment write NOT implemented or approved. Prior: 7.7k2 overlap verifier DONE. `scripts/verify-staff-bed-reassignment-overlap.js`: 25 checks � half-open interval (start_date < ca.end AND end_date > ca.start), current-row exclusion (id != ca.booking_bed_id), target-bed scoping (bed_id = tb.bed_id), cancelled/expired exclusion in overlap CTE, blocker pipeline (conflict_count > 0 ? b_overlap), UPDATE re-check (conflict_count = 0 in WHERE), FOR UPDATE OF bb lock, date range preserved (no start/end date overwrite in UPDATE SET), conflict_count in final SELECT; 25/25 PASS. No helper changes needed. Prior: 7.7k1 � static bed reassignment SQL helper DONE. `scripts/lib/staff-bed-reassignment-sql.js`: `reassignBookingBedSql()` CTE-based helper � client-scoped, FOR UPDATE lock, half-open overlap guard, 9 blockers (role/confirm/cancelled/needs_review/manual-lock/overlap/target-bed/client/assignment), conditional single UPDATE booking_beds (no DELETE/INSERT/payments/conversations mutations), workflow_events audit write on success, audit_payload + rollback_payload always returned. `scripts/verify-staff-bed-reassignment-sql.js`: 38/38 PASS. Full suite PASS. NOT wired to any API route or UI. Prior: 7.7k plan DONE. Pilot: NO_GO.)
