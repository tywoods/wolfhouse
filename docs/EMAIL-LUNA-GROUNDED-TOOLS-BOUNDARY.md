...
The boundary also pins every intrinsic it relies on during module initialization. Post-import monkeypatches of array iteration, reflection, property definition, freezing, Promise continuation, or other used built-ins must not execute or change output shape. Exported tool records, authority, success/failure DTOs, wrappers, and nested facts are frozen null-prototype records built with the captured property-definition intrinsic. Returned arrays remain genuine arrays (`Array.isArray`, numeric indexing, `length`, and `JSON.stringify` compatibility) but have a null prototype before freezing, so ambient executable or sensitive `Object.prototype` and `Array.prototype` properties are not observable through them.
