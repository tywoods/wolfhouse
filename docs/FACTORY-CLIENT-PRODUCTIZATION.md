...
- Emits canonical sorted JSON preview + `dry-run-manifest.json` (hashes) to an explicit safe output directory or `--stdout`
- Never overwrites; refuses output under `config/clients`, `config/archetypes`, or the repo root
