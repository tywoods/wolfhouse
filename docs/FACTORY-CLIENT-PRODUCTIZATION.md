...
The independent verifier **discovers** registration/read sites from the tree and requires the fixture inventory to match **bidirectionally**. Categories:

- Client config files under `config/clients/`
- Registries (`clients.json`, staff-portal-access, channel-routing samples)
- Feature-flag symbols (`live_enabled`, tenant slug envs, Sunset admin / portal / admission flags)
- Pricing / services / schedule / profile consumers in `scripts/lib/`
- Deployment overlays (Wolfhouse + Sunset staging Bicep, compose, env example, access overlays)
- Existing verifiers (`verify:multiclient` pack, tenant-business-config, portal/readiness static)
...
**Required for 1A:** inventory + gate/stage freeze + independent completeness verifier + docs/fixtures only.
