...
**Wolfhouse Hermes guest source order — notes first:** for any question that could concern this property's facilities, policies or how the stay works (including wifi, parking, pets, smoking, quiet hours, towels, kitchen, laundry and check-in/out), call `get_house_info` for the owner's General Notes before public search. Use relevant notes directly; do not search to replace or contradict them. Only uncovered questions or clearly outside-world questions (weather/forecast, museum hours, soft versus hard boards, neighborhood facts) may use `search_public_info` / `read_public_source`, still subject to the Staff Luna Intelligence switch. In a mixed question, resolve the property part from notes first, then research only the uncovered public part. Missing notes are not evidence of a property policy, and public pages cannot establish private house rules. Neither notes nor web replace Staff booking, quote, availability or payment tools. Source-selection instruction owners: `docker/hermes-staging/SOUL.md` and registered tool descriptions in `docker/hermes-staging/plugins/wolfhouse_staff_api/__init__.py`; public admission/privacy/budget owner remains `docker/hermes-staging/wolfhouse/luna_intelligence.py`. Tavily remains the selected live search provider; this change does not install SearXNG or alter runtime provider configuration. A failed notes lookup is not evidence that a topic is uncovered. Sunset does not expose `get_house_info`; its `docker/hermes-sunset/SOUL.md` checks its own Staff tools/config first, never another tenant's notes. This source-order rule is a model instruction contract, not a deterministic semantic classifier in the public-tool handler.
