...
**Status:** IN PROGRESS — 8.0+8.1+8.2+8.5+8.6+8.3 plan+8.3a+8.3b+8.3c+8.3d+**8.3e write-gate plan DONE (2026-06-02)**. Slices 8.3f–8.3o (manual booking impl ladder), 8.3p+ (move/cancel/operator), 8.4, 8.7–8.13 not started.
...
| **8.3f–8.3o** | Manual booking implementation ladder | code/docs | SQL static → conflict helper → preview endpoint → fixture+idempotency → gated API → rollback proof → UI preview → gated confirm → staging E2E → sign-off. All gated; disabled by default. |
