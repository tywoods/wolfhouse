...
**Status:** IN PROGRESS — 8.0+8.1+8.2+8.5+8.6+8.3 plan+8.3a+8.3b+8.3c+8.3d+**8.3e write-gate plan DONE**+8.3f+8.3g+8.3h+8.3i+**8.3j schema-align DONE (2026-06-02)**+**8.3k rollback proof DONE (2026-06-02)**+**8.3l preview UI wired DONE (2026-06-02)**+**8.3q tour operator skeleton DONE (2026-06-02)**+**8.3r operator room release skeleton DONE (2026-06-02)**+**8.3s Azure batch deploy DONE (2026-06-02): image wh-staff-api:1894036-8x3s-batch, revision --0000012, Healthy, 100% traffic, STAFF_ACTIONS_ENABLED=false, WHATSAPP_DRY_RUN=true**++**8.3u operations UI correction DONE (2026-06-02): Tour Operator tab; cell selection fix; Demo Range removed; drawer cleanup****8.3x Inbox WhatsApp-layout DONE (2026-06-02)**++**8.3v Azure deploy (8.3u) DONE (2026-06-02): image ea2437d-8x3v-ui-corrections, revision --0000013, Healthy, Write actions DISABLED****8.3y Needs Human + detail cleanup DONE (2026-06-02)**+**8.4.1 pricing/payment config plan DONE (2026-06-02)**+**8.4.2 pricing config fixture DONE (2026-06-02): wolfhouse-somo.pricing.json 63/63 PASS**+**8.4.3 quote calculator DONE (2026-06-02): scripts/lib/wolfhouse-quote-calculator.js, verifier 77/77 PASS, Formula B per-night ceil5**+**8.4.4 quote preview endpoint DONE (2026-06-02): POST /staff/quote-preview, verifier 33/33 PASS, no DB, local proof PASS**+**8.4.5 quote preview UI + form cleanup DONE (2026-06-02): package dropdown, language removed, multi-bed selection, Calculate Quote button, itemized display, drawer dedup, verifier 194/194 PASS, no DB writes, Create Manual Booking stays disabled**+**8.4.6 room type selector DONE (2026-06-02): bk-room-type select (shared/private/double), runQuotePreview reads selection, private/double supplement in quote, reset to shared, verifier 201/201 PASS, no DB writes**+**8.4.8 create manual booking + quote + draft payment DONE (2026-06-02): booking-first; calculateWolfhouseQuote() server-side; quote_snapshot in metadata; draft payment (kind from payment_choice, amount=payment_link_amount_cents); UI gated by flags+bcLastQuote; flags=false→403; proof 81300¢ booking created+cleaned; 397/397; no Stripe/migration.** +**8.4.7 add-ons selector DONE (2026-06-02): checkboxes+qty for wetsuit/soft-top/hard-board/combos/surf lessons/yoga, buildAddOns() payload, combos suppress individual rentals, bcInitAddOns() wiring, reset clears all, local proof wetsuit 3d+2 lessons=32400c, combo 4d=30900c, verifier 222/222 PASS, no DB writes**. Slices 8.3j–8.3o (remaining manual booking gated API/UI), 8.3p+ (move/cancel/operator), 8.4.6+ (create-from-quote path, Stripe), 8.7–8.13 not started.
...
- **Stage 8.4.1 DONE (2026-06-02, docs):** Wolfhouse pricing/payment config plan — [`STAGE-8.4.1-WOLFHOUSE-PRICING-PAYMENT-CONFIG-PLAN.md`](STAGE-8.4.1-WOLFHOUSE-PRICING-PAYMENT-CONFIG-PLAN.md). Known config captured in cents (packages malibu/uluwatu/waimea with seeded seasonal weekly prices, per-person scope, +€10 pppn double/private, €200/€100 deposits, 1h hold, add-ons, refund/automation rules); REQUIRED_FROM_STAFF gaps; quote contracts; payment-record/invoice model; 12-slice ladder.
- **Stage 8.4.2 DONE (2026-06-02, config + verifier):** Wolfhouse pricing config fixture created at `config/clients/wolfhouse-somo.pricing.json` (matching repo `config/clients/` pattern). All package prices (malibu/uluwatu/waimea × spring_autumn/summer/august) in cents; deposit scope=per_booking confirmed; August priority=10; room supplements; add-ons (wetsuit/soft-top/hard-board/combo/lesson/yoga) all confirmed prices in cents; hold=60min; `scripts/verify-wolfhouse-pricing-config.js` 63/63 PASS; `verify:wolfhouse-pricing-config` added to package.json. Unresolved REQUIRED_FROM_STAFF: proration formula ambiguity (per-night vs whole-stay round-up), march/november edge season months, add-on charge timing, group/operator/retreat pricing. No DB writes. No API. No UI. No Azure. Flags unchanged.
...
| **8.3** | Staff Portal bed calendar **operations plan** | docs | **DONE (2026-06-02)** — [`STAGE-8.3-STAFF-PORTAL-BED-CALENDAR-OPERATIONS-PLAN.md`](STAGE-8.3-STAFF-PORTAL-BED-CALENDAR-OPERATIONS-PLAN.md). Reframes the bed calendar as the operations workspace; product language "Staff Portal" (not "Cami dashboard"); expands into sub-slices **8.3a–8.3o**: read-only cleanup (a), drawer cleanup (b), cell selection (c), manual booking modal/SQL/fixture/API/UI ladder (d–h), move preview (i), cancel/date-change design (j), tour operator booking plan+impl (k–l), operator room release plan+impl (m–n), dashboard extras roadmap (o). All write surfaces future + gated; read-only 8.3a/8.3b are the only demo prerequisites. |
| **8.3a** | Bed calendar read-only UX cleanup | code | **DONE (2026-06-02)** — date inputs (`type="date"`), shortcut chips (Today/Week/30d/Jul–Aug/Demo range), always-visible color legend (7 statuses), operator+manual block colors, inline A/D markers → tooltip, cleaner room/bed labels, taller blocks (28px), free-bed count in summary, `bcSetRange()` helper. Verifier: 56 checks all PASS. **Azure: image `bcb3ff5-8x3a` deployed, revision 0000005, login page + safety badges confirmed.** |
| **8.3b** | Booking detail drawer cleanup | code | **DONE (2026-06-02)** — 6 named sections (Guest/Stay/Room-Beds/Payment/Add-ons/Conv-Handoff), nights badge, Total/Paid/Remaining balance rows, planned-actions disabled stub, no raw UUIDs. Verifier 72/72 PASS. Azure revision `wh-staging-staff-api--0000007` live. |
| **8.3c** | Calendar cell selection model | code | **DONE (2026-06-02)** — empty cells selectable, sage highlight, selection panel (Bed/Check-in/Check-out/Nights), occupied-cell warning, disabled "Create Manual Booking — coming soon", clear button. Single-bed range. No writes. Verifier 86/86 PASS. Azure rev `wh-staging-staff-api--0000008`. |
| **8.3d** | Manual booking preview skeleton | code | **DONE (2026-06-02)** — full form: Selected Stay (pre-filled readonly), Guest, Payment (deposit/status/no Stripe hint), Notes, Avail placeholder, Safety notice (no booking/no WhatsApp/no Stripe), disabled Create+Conflicts buttons. 105 verifier checks PASS. Azure rev `wh-staging-staff-api--0000009`. |
| **8.3e** | Manual booking write gate plan | docs | **DONE (2026-06-02)** — [`STAGE-8.3E-MANUAL-BOOKING-WRITE-GATE-PLAN.md`](STAGE-8.3E-MANUAL-BOOKING-WRITE-GATE-PLAN.md): hard blockers, warning/second-confirm cases, audit, rollback/idempotency, revised contiguous numbering (manual booking 8.3e–8.3o; move/cancel/operator 8.3p+). Pilot NO_GO; writes NOT implemented. |
| **8.3f–8.3o** | Manual booking implementation ladder | code/docs | **8.3f DONE** — `buildManualBookingCreateSql()` 15-CTE static SQL helper, 40/40 verifier PASS. **8.3g DONE** — `previewManualBookingAvailability()` pure-JS helper, 52/52 verifier PASS. **8.3h DONE** — `POST /staff/manual-bookings/preview` read-only API endpoint, 48/48 verifier PASS. **8.3i DONE** — fixture write proof with `BEGIN/ROLLBACK`, 9 test cases. Schema mismatches documented. **8.3x DONE** — Inbox WhatsApp-style two-column layout: conv cards left, detail right, `handoffLabel()` friendly text, "Message thread" count removed, raw stage removed, "Back to inbox" removed. 66/66 verifier PASS. Azure image `4e02763-8x3x-inbox` deployed; `STAFF_ACTIONS_ENABLED=false`, `WHATSAPP_DRY_RUN=true`. **8.3y DONE (Azure deployed: image `a61a5ed-8x3y-nh`, revision `wh-staging-staff-api--0000011`)** — Needs Human tab converted to same two-column conv-card layout (filtered to needs_human/open handoff); `handoffLabel()` reused; Booking sidebar above Bot state; Pending/Last reply removed from Bot state; check-in/out combined date-only Stay row (`fmtDateOnly`); "Messages" title removed from thread; `loadConvDetail(convId, targetEl)` refactored to support both Inbox and NH panels. 77/77 verifier PASS. Azure deploy pending. No writes. No schema changes. All gated; disabled by default. |
| **8.3p–8.3w** | Move/cancel/date-change, tour operator, operator release | code/docs | Reuse the 8.3e write-gate pattern (preview, conflict, role, audit, rollback, flag). |
