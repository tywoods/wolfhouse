...
- **Stage 8.4.1 DONE (2026-06-02, docs):** Wolfhouse pricing/payment config plan — [`STAGE-8.4.1-WOLFHOUSE-PRICING-PAYMENT-CONFIG-PLAN.md`](STAGE-8.4.1-WOLFHOUSE-PRICING-PAYMENT-CONFIG-PLAN.md). Known config captured in cents (packages malibu/uluwatu/waimea with seeded seasonal weekly prices, per-person scope, +€10 pppn double/private, €200/€100 deposits, 1h hold, add-ons, refund/automation rules); REQUIRED_FROM_STAFF gaps (deposit scope, group/discount, retreat, operator, add-on charge timing, multi-week); quote input/output contracts; payment-record/invoice model mapped to existing `bookings`/`payments`/`workflow_events`/`staff_handoffs`; quote-snapshot storage (v1 `bookings.metadata.quote_snapshot` → v2 `quote_snapshots`); override + confidence/handoff rules; 12-slice ladder (8.4.1–8.4.12); hard gate before `MANUAL_BOOKING_ENABLED`. No code, no endpoint, no automation.
