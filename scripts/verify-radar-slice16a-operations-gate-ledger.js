...
const sel16f = matrix.slice_16f_selection;
ok('F23b exactly one 16F selection',
  sel16f && sel16f.selected === true
  && sel16f.outcome_id === '16F_staff_api_metric_alerts'
  && sel16f.gate_id === 'G03_actionable_tenant_aware_alerts'
  && sel16f.progress_class === 'source_partial_progress_only');
ok('F23c contract selected_16f matches',
  contract.selected_16f
  && contract.selected_16f.outcome_id === '16F_staff_api_metric_alerts'
  && contract.selected_16f.gate_id === 'G03_actionable_tenant_aware_alerts');
ok('F23d 16F final controlled drill present',
  sel16f.final_controlled_drill
  && sel16f.final_controlled_drill.id === '16F_DRILL_metric_alert_fire_notify');
ok('F23e 16F acceptance criteria finite (>=4)',
  Array.isArray(sel16f.acceptance_criteria) && sel16f.acceptance_criteria.length >= 4);
...
ok('F33 doc mentions selected 16F id', /16F_staff_api_metric_alerts/.test(doc));
