...
if (apiSrc) {
  assert('school switcher markup', apiSrc.includes('id="staff-school-switch"') && apiSrc.includes('data-school="sunset-somo"') && apiSrc.includes('data-school="sunset-sardinero"'));
  assert('school switcher labels i18n', i18nSrc.includes("'school.sunsetSomo'") && i18nSrc.includes("'school.sunsetSardinero'"));
  assert('default sunset location helper', apiSrc.includes("return 'sunset-somo'") && apiSrc.includes('STAFF_PORTAL_SUNSET_LOCATION_KEY'));
  assert('schedule day endpoint', apiSrc.includes('/staff/schedule/day') && apiSrc.includes('handleSunsetScheduleDayGet'));
  assert('schedule fetch uses location suffix', apiSrc.includes('sunsetLocationQuerySuffix()'));
  assert('manual booking write includes location', apiSrc.includes('location_id: getSunsetLocation()'));
  assert('drawer detail scoped by location', fs.existsSync(path.join(ROOT, 'scripts/lib/sunset-schedule-booking-drawer.js'))
    && fs.readFileSync(path.join(ROOT, 'scripts/lib/sunset-schedule-booking-drawer.js'), 'utf8').includes('booking_not_in_active_school'));
  assert('location SQL isolation helper', fs.existsSync(path.join(ROOT, 'scripts/lib/sunset-schedule-queries.js')));
}

const locModPath = path.join(ROOT, 'scripts/lib/sunset-school-locations.js');
if (fs.existsSync(locModPath)) {
  const locMod = require(locModPath);
  assert('two sunset locations configured', Array.isArray(locMod.SUNSET_LOCATIONS) && locMod.SUNSET_LOCATIONS.length === 2);
  assert('default location is sunset-somo', locMod.DEFAULT_SUNSET_LOCATION_ID === 'sunset-somo');
  assert('somo and sardinero distinct', locMod.normalizeSunsetLocationId('sunset-sardinero') === 'sunset-sardinero'
    && locMod.normalizeSunsetLocationId('sunset-somo') === 'sunset-somo'
    && locMod.normalizeSunsetLocationId('') === 'sunset-somo');
}

const qModPath = path.join(ROOT, 'scripts/lib/sunset-schedule-queries.js');
if (fs.existsSync(qModPath)) {
  const qSrc = fs.readFileSync(qModPath, 'utf8');
  const locSql = fs.existsSync(locModPath) ? fs.readFileSync(locModPath, 'utf8') : '';
  assert('schedule queries filter by location param', qSrc.includes('sqlLocationMatch') && locSql.includes("metadata->>'location_id'") && locSql.includes('= $'));
