...
const LIVE_SIMULATOR_PATH = path.join(ROOT, 'scripts', 'lib', 'crowsnest', 'crowsnest-live-simulator.js');
...
const LIVE_SIMULATOR_VERIFY_PATH = path.join(ROOT, 'scripts', 'verify-crowsnest-live-simulator.js');
...
ok('scripts/lib/crowsnest/crowsnest-live-simulator.js exists', fs.existsSync(LIVE_SIMULATOR_PATH));
...
ok('scripts/verify-crowsnest-live-simulator.js exists', fs.existsSync(LIVE_SIMULATOR_VERIFY_PATH));
...
ok('Live Simulator Enter submits while Shift+Enter remains newline', /text\.addEventListener\('keydown'/.test(liveSimulatorHtml) && /event\.key !== 'Enter'/.test(liveSimulatorHtml) && /event\.shiftKey/.test(liveSimulatorHtml) && /form\.requestSubmit/.test(liveSimulatorHtml));
...
ok('Live Simulator visible limitation names Sunset booking-only mode and disabled external effects', /Sunset staging bookings enabled/i.test(liveSimulatorHtml) && /WhatsApp\/SMS and other external sends stay off/.test(liveSimulatorHtml) && /Payments and waiver creation stay off/.test(liveSimulatorHtml));
...
ok('location doc records current live baseline (luna-crowsnest-rg)', /luna-crowsnest-rg/.test(planDoc) && /crowsnest-internal--0000002/.test(planDoc));
...
ok('deploy plan records live revision and image SHA', /crowsnest-internal--0000002/.test(deployDoc) && /b7eaba0944ab9afe8dc05f7b6ddaeb140b7c3171/.test(deployDoc));
ok('location plan records live revision and image SHA', /crowsnest-internal--0000002/.test(planDoc) && /b7eaba0944ab9afe8dc05f7b6ddaeb140b7c3171/.test(planDoc));
...
ok('package.json has verify:crowsnest-live-simulator', pkg && pkg.scripts && typeof pkg.scripts['verify:crowsnest-live-simulator'] === 'string');
