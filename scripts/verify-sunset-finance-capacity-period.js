...
// Year view: same 5 SUP unit-days → 5/(5×365) not 5/5 = 100%
const yearSummary = computeSunsetFinanceSummary({
  now: NOW,
  timeZone: 'Europe/Madrid',
  view: { granularity: 'year', anchor: '2026-08-15', start: '2026-01-01', end: '2026-12-31' },
  bsr,
  payments: [],
  bookings: [],
  rental_stock: rentalStock,
...
const yearSup = yearSummary.redesign.capacity.by_product.find((r) =>
  (r.offering_keys || []).includes('sup_rental') || /sup/i.test(String(r.label || '')));
assert.ok(yearSup, 'year SUP row present');
assert.strictEqual(yearSup.detail, '5/1825', `year SUP detail got ${yearSup.detail}`);
assert.ok(yearSup.pct != null && yearSup.pct < 5, `year SUP pct absurdly high: ${yearSup.pct}`);
...
assert.ok(dataSrc.includes('ACCOMMODATION_SETTINGS_SQL'), 'finance data has accommodation settings SQL');
