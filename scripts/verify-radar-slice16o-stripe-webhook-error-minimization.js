 ...
 * responses (SDK load + signature verification). Real-listener proofs use
 * createStaffQueryApiHttpServer (fortress dual-gate). No live deploy / Azure
 * mutation / migration.
 */
...
const CANARY = Object.freeze({
  ...
  WHSEC: 'whsec_radar16o_CANARY_WEBHOOK_SECRET_DO_NOT_LEAK',
...
function applyMinimalStaffApiEnv(extra = {}) {
  ...
  process.env.STRIPE_SECRET_KEY = 'sk_test_radar16o_placeholder';
...
async function main() {
  ...
  try {
    await withRealStaffApiServer({
      ...
      const consoleLeakSdk = bodyContainsAny(
        consoleCalls.slice(consoleBeforeSdk).join('\n'),
        [CANARY.ERR_MSG, CANARY.EXC_CLASS, CANARY.SIG_HEADER, CANARY.WHSEC],
      );
      // continue — aggregate console check below
      void consoleLeakSdk;
    });
    ...
    await withRealStaffApiServer({
      ...
      const allBodies = [missing.body, malformed.body, parserThrow.body].join('\n');
      ...
      const consoleHit = bodyContainsAny(
        consoleCalls.slice(consoleBefore).join('\n'),
        [CANARY.ERR_MSG, CANARY.EXC_CLASS, CANARY.SIG_HEADER, CANARY.WHSEC, CANARY.BODY_MARKER],
      );
    ...
    const audits = readAuditTail(auditBefore).filter((e) =>
      ...
      && (e.reason === 'sdk_load_failed' || e.reason === 'signature_verification_failed'));
    const auditOk = audits.length >= 1
      && audits.every((e) =>
        ...
        && !bodyContainsAny(JSON.stringify(e), [
          CANARY.ERR_MSG, CANARY.SIG_HEADER, CANARY.WHSEC, CANARY.BODY_MARKER, CANARY.EXC_CLASS,
        ]));
    green('audit_allowlisted_reason_only', auditOk,
      `audits=${audits.length} sample=${JSON.stringify(audits.slice(0, 2))}`);
  } catch (err) {
    green('real_listener_missing_signature_generic_400', false, String(err && err.message));
    green('real_listener_malformed_signature_generic_400', false, String(err && err.message));
    green('real_listener_parser_throw_signature_generic_400', false, String(err && err.message));
    green('real_listener_sdk_load_failure_generic_500', false, String(err && err.message));
    green('no_canaries_in_response_bodies', false, String(err && err.message));
    green('zero_db_calls_on_public_error_paths', false, String(err && err.message));
    green('no_console_canary_leak', false, String(err && err.message));
    green('request_id_retained', false, String(err && err.message));
    green('audit_allowlisted_reason_only', false, String(err && err.message));
  } finally {
    unwrapConsole();
  }

  // Also ensure SDK path body had no canaries (re-check via contract equality already done).
  ok('C7 lib forbids error/stack keys on public bodies',
  ...
  try {
    ...
    ok('C8 HEAD on 16O branch', branch === locks.BRANCH, branch);
  } catch (err) {
    ok('C8 HEAD on 16O branch', false, String(err && err.message));
  }
