...
// Pin URL parsing to the Node implementation used by this validator.
...
function failure() {
  ...
  Object.defineProperty(error, 'name', {
    value: 'GoogleOidcVerifiedIdentityError',
  });
...
function parseStrictJson(text) {
  ...
  const whitespace = () => {
    while (/[\x20\x09\x0a\x0d]/.test(text[at] || '')) at += 1;
  };
  function string() {
    const start = at++;
    let escaped = false;
...
function validAudience(aud, azp, client) {
  if (typeof aud === 'string')
    return (
      safeEqual(aud, client) && (azp === undefined || safeEqual(azp, client))
    );
  if (
    !Array.isArray(aud) ||
    aud.length === 0 ||
    new Set(aud).size !== aud.length ||
    aud.some((item) => typeof item !== 'string') ||
    typeof azp !== 'string' ||
    !safeEqual(azp, client)
  )
    return false;
...
function validMetadata(claims) {
  ...
  for (const key of ['given_name', 'family_name'])
    if (claims[key] !== undefined && !bounded(claims[key], 256)) return false;
  if (
    claims.locale !== undefined &&
    (!bounded(claims.locale, 35) ||
      !/^[A-Za-z]{2,8}(?:-[A-Za-z0-9]{1,8})*$/.test(claims.locale))
  )
    return false;
...
function createGoogleOidcVerifiedIdentity(dependencies) {
  ...
  let verifySignature;
  try {
    const deps = exactFrozenData(dependencies, ['signatureVerifier']);
  ...
  async function verifyIdentity(input) {
    ...
    used = true;
    try {
      const data = exactFrozenData(input, [
        'idToken',
        'accessToken',
        'expectedNonce',
        'expectedClientId',
      ...
      if (
        !data ||
        !bounded(data.idToken, LIMITS.token) ||
        !bounded(data.accessToken, 8192) ||
        !/^[\x21-\x7e]+$/.test(data.accessToken) ||
        !bounded(data.expectedNonce, 512) ||
        !bounded(data.expectedClientId, 256) ||
      ...
      const header = json(encodedHeader, LIMITS.header);
      const claims = json(encodedClaims, LIMITS.claims);
      const signature = decode(encodedSignature, LIMITS.signature);
      const headerKeys = new Set(['alg', 'kid', 'typ']);
      if (
        !exactKeys(header, headerKeys) ||
        header.alg !== 'RS256' ||
        !bounded(header.kid, 256) ||
        (header.typ !== undefined && header.typ !== 'JWT')
      )
        throw failure();
      const signatureRequest = Object.freeze({
      ...
      let ack;
      // Authenticate the signed bytes before trusting any decoded claim.
      try {
        ack = await Reflect.apply(verifySignature, verifier, [
          signatureRequest,
        ]);
      } catch {
        throw failure();
      ...
      if (
        !exactFrozenData(ack, ['verified']) ||
        Object.getOwnPropertyDescriptor(ack, 'verified').value !== true
      )
        throw failure();
      ...
      if (claims.at_hash !== undefined) {
        // Bind an optional at_hash claim to the supplied OAuth access token.
        const hash = decode(claims.at_hash, 22);
        if (hash.length !== 16) throw failure();
        const expected = createHash('sha256')
          .update(data.accessToken, 'ascii')
          .digest()
          .subarray(0, 16);
      ...
      return Object.freeze({
        ...
        providerPrincipalId: claims.sub,
        mailboxAddress: claims.email,
        displayName,
      });
    } catch {
      throw failure();
