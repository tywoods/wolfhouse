  ...
  for (const entry of body.entry) {
    ...
    for (const change of changes) {
      ...
      return {
        ...
        display_phone_number: trimStr(metadata.display_phone_number) || null,
...
/**
 * Shadow attachment + optional FORTRESS 15H2 ingress authority policy.
...
function normalizeMetaWhatsAppWebhook(body, options = {}) {
  ...
  if (!inbound) {
    return finalizeNormalizedMetaWhatsApp({
      ...
      message_text: null,
      profile_name: null,
      receiving_whatsapp_number: null,
      supported: false,
  ...
  return finalizeNormalizedMetaWhatsApp({
    ...
    display_phone_number: inbound.display_phone_number,
    receiving_whatsapp_number: inbound.display_phone_number,
    from: inbound.from,
 ...
 * Propagates trusted normalized.location_id when present (FORTRESS 15H2).
 */
