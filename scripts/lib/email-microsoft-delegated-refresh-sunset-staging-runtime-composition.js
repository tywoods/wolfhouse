...
/**
 * Sunset-staging Microsoft delegated refresh-health runtime composition.
 * Explicit factory only — default-off via LUNA_EMAIL_OAUTH_REFRESH_HEALTH_ENABLED.
 * Does not enable start/callback, activation, inbound/outbound, or automation.
 *
 * @module email-microsoft-delegated-refresh-sunset-staging-runtime-composition
 */
...
const {
  ...
  SUNSET_DEPLOYMENT: ROTATION_SUNSET,
} = require('./email-delegated-grant-refresh-rotation');

const ERROR_CODE = 'MICROSOFT_DELEGATED_REFRESH_RUNTIME_COMPOSITION_INVALID';
const ERROR_MESSAGE = 'Microsoft delegated refresh runtime composition failed.';
const SUNSET_DEPLOYMENT = 'sunset-staging';
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const ENV_REFRESH_HEALTH_ENABLED = 'LUNA_EMAIL_OAUTH_REFRESH_HEALTH_ENABLED';
...
if (SECRET_SUNSET !== SUNSET_DEPLOYMENT || ROTATION_SUNSET !== SUNSET_DEPLOYMENT) {
  throw new Error('refresh_runtime_composition_sunset_deployment_mismatch');
}

function failure() {
  const error = new Error(ERROR_MESSAGE);
  Object.defineProperty(error, 'name', { value: 'MicrosoftDelegatedRefreshRuntimeCompositionError' });
...
function isRefreshHealthEnabled(env) {
  try {
    return !!env && env.LUNA_DEPLOYMENT === SUNSET_DEPLOYMENT
      && env[ENV_REFRESH_HEALTH_ENABLED] === 'true';
  } catch {
    return false;
  }
}
...
function snapshotEnvReadiness(env) {
  try {
    if (!isRefreshHealthEnabled(env)) return null;
 ...
 * @returns {Readonly<{ runRefreshHealth: Function }>}
 */
function createSunsetStagingMicrosoftDelegatedRefreshRuntime(deps) {
  try {
    ...
    return createDelegatedGrantRefreshRotationService(Object.freeze({
      deployment: SUNSET_DEPLOYMENT,
