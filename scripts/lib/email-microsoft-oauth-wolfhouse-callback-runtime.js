...
const {
  ...
  WOLFHOUSE_DEPLOYMENT: OP_WOLFHOUSE_DEPLOYMENT,
...
const {
  ...
  parseWolfhouseStaffStagingEnvelopeConfig,
...
if (OP_WOLFHOUSE_DEPLOYMENT !== WOLFHOUSE_DEPLOYMENT) {
  throw new Error('oauth_runtime_composition_wolfhouse_deployment_mismatch');
}
...
function snapshotEnvReadiness(env) {
  try {
    ...
    if (env.LUNA_DEPLOYMENT !== WOLFHOUSE_DEPLOYMENT) return null;
    if (env.WOLFHOUSE_EMAIL_MICROSOFT_OAUTH_CALLBACK_ENABLED !== 'true'
        || env.WOLFHOUSE_EMAIL_MICROSOFT_OAUTH_GRANT_CUSTODY_ENABLED !== 'true') return null;
...
function resolveEnvelopeProvider(env) {
  ...
  let composition;
  try {
    composition = createActiveWolfhouseStaffStagingEnvelopeComposition(env);
  } catch {
    return null;
  }
  if (!composition
...
function pinDependencies(dependencies) {
  ...
  let secretProvider;
  try {
    secretProvider = createWolfhouseMicrosoftOAuthClientSecretProvider({
 ...
 */
function createWolfhouseStaffStagingMicrosoftOAuthCallbackRuntime(dependencies) {
  let pinned;
  try {
  ...
  try {
    ...
    const completion = createWolfhouseMicrosoftOAuthOperationComposition(Object.freeze({
      verifiedIdentity,
    ...
    return createWolfhouseMicrosoftOAuthCallbackCompletionService(Object.freeze({
      repository,
      completion,
      env: pinned.env,
      clock: dateClock,
...
module.exports = Object.freeze({
  ...
  CRYPTO_KEYS,
  TIMERS_KEYS,
  createWolfhouseStaffStagingMicrosoftOAuthCallbackRuntime,
});
