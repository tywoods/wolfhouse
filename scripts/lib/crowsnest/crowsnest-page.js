...
(() => {
  ...
  function showAudit(who, when) { root.querySelector('[data-audit-who]').textContent = who; root.querySelector('[data-audit-when]').textContent = when; }
  async function load() { try { const result = await request(''); showEffective(result); showAudit('Verified route controller', 'Live readback'); save.disabled = false; setStatus('Registered targets loaded. Review before saving.'); } catch (error) { save.disabled = true; setStatus('Routing controls unavailable: ' + error.message, 'error'); } }
