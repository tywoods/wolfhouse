...
const pools = new Map();
function config(env = process.env) { const origin = String(env.CROWSNEST_LUNA_ROUTING_CONTROLLER_ORIGIN || '').replace(/\/$/, ''); const key = String(env.CROWSNEST_LUNA_ROUTING_CONTROLLER_HMAC_KEY || ''); const dsn = String(env.CROWSNEST_COMMS_DATABASE_URL || ''); return /^https:\/\//.test(origin) && key.length >= 32 && dsn ? { origin, key, dsn } : null; }
