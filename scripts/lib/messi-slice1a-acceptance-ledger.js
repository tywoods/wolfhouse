...
const MESSI_CLOSEOUT_FINITE_FLAG =
  'finite_evidence_integration_ledger_audit_workstream_complete';
const MESSI_CLOSEOUT_ANALOG = 'verify:messi-slice1i-finite-closeout';
/** Cross-parent gate workstream — gap manifest only; never integration proof. */
const CROSS_PARENT_WORKSTREAM_CLASS = 'cross_parent_disposition_gap_manifest_only';
...
/** Reviewed FACTORY 1E candidate + canonical tip (identity; unchanged durable provenance). */
const FACTORY_1E_MERGE_TIP = '14facf5d54be8767cf9aca4d69a880f28ea3dc2e';
const FACTORY_1E_CANDIDATE_SHA = '14facf5d54be8767cf9aca4d69a880f28ea3dc2e';
const FACTORY_1E_MASTER_BASIS = 'e8452d178ad8f4b6aadc8b59b2d3032634952471';
...
const PACKAGE_JSON_1F_SCRIPT_VALUE =
  'node scripts/verify-messi-slice1a-acceptance-ledger.js';
const PACKAGE_JSON_1G_SCRIPT_KEY = 'verify:messi-slice1g-factory-wiring';
const PACKAGE_JSON_1G_SCRIPT_VALUE =
  'node scripts/verify-messi-slice1a-acceptance-ledger.js';
const PACKAGE_JSON_1H_SCRIPT_KEY = 'verify:messi-slice1h-cross-parent-gap';
const PACKAGE_JSON_1H_SCRIPT_VALUE =
  'node scripts/verify-messi-slice1a-acceptance-ledger.js';
...
const SCOPE_FENCE = Object.freeze({
  allowed: Object.freeze([
    ...
    'cross_parent_disposition_gap_manifest',
    'finite_messi_evidence_integration_ledger_audit_closeout',
  ]),
  forbids: Object.freeze([
    ...
    'gap_manifest_as_cross_parent_integration_proof',
...
const PARENTS = Object.freeze({
  ...
  FACTORY: Object.freeze({
    ...
    master_basis: FACTORY_1E_MASTER_BASIS,
    canonical_tip: FACTORY_1E_MERGE_TIP,
    candidate_sha: FACTORY_1E_CANDIDATE_SHA,
    tip_sha_on_master: FACTORY_1E_MERGE_TIP,
    workstream_class: 'finite_offline_dry_run_packaging_closeout',
    finite_closeout_analog: 'verify:factory-slice1e-finite-closeout',
    production_readiness: 'absent',
    docs: Object.freeze([
    ...
    // Finite offline dry-run closeout is wired; parent stays incomplete without these.
    missing_proof_for_complete: Object.freeze([
      'third_tenant_live_or_prod_onboarding',
      'apply_or_disk_materialization',
      'RADAR_reopen_third_tenant_factory_clearance',
...
const MESSI_GATES = Object.freeze([
  ...
  Object.freeze({
    ...
    requirement:
      'Inventory FACTORY 1E finite offline closeout, bind exact candidate/canonical tip 14facf5d evidence blobs, run verify:factory-slice1e-finite-closeout, expose finite offline dry-run workstream completion, and keep parent partial while production apply/materialization, operated onboarding, secrets/live targets, and production readiness remain absent. Do not mark complete from labels or treat finite offline dry-run closeout as FACTORY complete / production readiness.',
  }),
  Object.freeze({
    ...
    requirement:
      'Committed proof that FOUNDATION+FORTRESS+RADAR+FACTORY compose into an integrated operated/production-ready system. Slice 1H binds a deterministic disposition gap manifest proving the four reviewed parent dispositions are wired and mutually consistent, but keeps this gate absent — a gap manifest is not end-to-end cross-parent integration proof.',
  }),
  Object.freeze({
    ...
    requirement:
      'MESSI closes only when all parent MESSI gates are complete with production-grade proof. Slice 1I records finite evidence-integration ledger/audit workstream completion and reviewed dispositions (four parents + cross-parent gap manifest) while keeping this gate absent — finite ledger closeout is not MESSI complete / production readiness.',
  }),
]);

/**
 * Deterministic gap manifest for G_CROSS_PARENT_INTEGRATION.
 * Proves four reviewed parent dispositions are wired + mutually consistent.
 * Explicitly NOT cross-parent operated/production integration proof.
 */
const CROSS_PARENT_GAP_MANIFEST = Object.freeze({
  schema_version: 1,
  purpose: 'cross_parent_disposition_gap_inventory',
  integration_proof: false,
  gate_verdict_required: 'absent',
  parent_dispositions: Object.freeze([
    Object.freeze({
      parent: 'FOUNDATION',
      gate_id: 'G_FOUNDATION_PARENT',
      verdict: 'partial',
      workstream_class: 'finite_staging_schema_migration_recovery_closeout',
      production_readiness: 'absent',
      finite_closeout_analog: 'verify:messi-slice1b-foundation-closeout',
      finite_flag: 'finite_staging_workstream_complete',
      finite_flag_value: true,
    }),
    Object.freeze({
      parent: 'FORTRESS',
      gate_id: 'G_FORTRESS_PARENT',
      verdict: 'partial',
      workstream_class: 'finite_fortress_audit_workstream_closeout',
      production_readiness: 'absent',
      finite_closeout_analog: 'verify:messi-slice1d-fortress-closeout',
      finite_flag: 'finite_audit_workstream_complete',
      finite_flag_value: true,
    }),
    Object.freeze({
      parent: 'RADAR',
      gate_id: 'G_RADAR_PARENT',
      verdict: 'partial',
      workstream_class: 'finite_milestone_closeout_staging_readiness_only',
      production_readiness: 'absent',
      finite_closeout_analog: 'verify:radar-slice16ap-finite-closeout',
      finite_flag: 'finite_staging_readiness_workstream_complete',
      finite_flag_value: true,
    }),
    Object.freeze({
      parent: 'FACTORY',
      gate_id: 'G_FACTORY_PARENT',
      verdict: 'partial',
      workstream_class: 'finite_offline_dry_run_packaging_closeout',
      production_readiness: 'absent',
      finite_closeout_analog: 'verify:factory-slice1e-finite-closeout',
      finite_flag: 'finite_offline_dry_run_workstream_complete',
      finite_flag_value: true,
    }),
  ]),
  consistency_checks: Object.freeze([
    'exactly_four_reviewed_parents',
    'all_parent_verdicts_partial',
    'all_production_readiness_absent',
    'all_finite_workstream_flags_true',
    'finite_closeout_analogs_match_parent_locks',
    'workstream_classes_match_parent_locks',
  ]),
  remaining_integration_gaps: Object.freeze([
    ...
    'operated_cross_parent_production_integration',
  ]),
});

const CROSS_PARENT_MISSING_PROOF = Object.freeze([
  'committed_cross_parent_integration_proof',
  'composed_foundation_fortress_radar_factory_production_evidence',
  'end_to_end_live_staging_integration_beyond_parent_silos',
  'operated_cross_parent_production_integration',
 ...
 */
const MESSI_REVIEWED_DISPOSITIONS = Object.freeze([
  ...CROSS_PARENT_GAP_MANIFEST.parent_dispositions.map((d) => Object.freeze({
    disposition_id: d.parent,
    gate_id: d.gate_id,
    verdict: d.verdict,
    workstream_class: d.workstream_class,
    production_readiness: d.production_readiness,
    finite_closeout_analog: d.finite_closeout_analog,
    finite_flag: d.finite_flag,
    finite_flag_value: d.finite_flag_value,
  })),
  Object.freeze({
    disposition_id: 'CROSS_PARENT_GAP_MANIFEST',
    gate_id: 'G_CROSS_PARENT_INTEGRATION',
    verdict: 'absent',
    workstream_class: CROSS_PARENT_WORKSTREAM_CLASS,
    production_readiness: 'absent',
    integration_proof: false,
    gap_manifest_bound: true,
  }),
]);

const MESSI_CLOSEOUT_MISSING_PROOF = Object.freeze([
...
/**
 * Validate locked gap manifest shape + mutual consistency of the four reviewed
 * parent dispositions. Does not raise G_CROSS_PARENT_INTEGRATION above absent.
 */
function validateCrossParentGapManifest(manifest, parentGatesById) {
  const errors = [];
  const m = manifest || {};
  if (m.schema_version !== 1) errors.push('gap_manifest_schema_version');
  if (m.purpose !== 'cross_parent_disposition_gap_inventory') {
    errors.push('gap_manifest_purpose');
  }
  if (m.integration_proof !== false) errors.push('gap_manifest_integration_proof_claimed');
  if (m.gate_verdict_required !== 'absent') {
    errors.push('gap_manifest_gate_verdict_required');
  }
  const expectedParents = ['FOUNDATION', 'FORTRESS', 'RADAR', 'FACTORY'];
  const rows = Array.isArray(m.parent_dispositions) ? m.parent_dispositions : [];
  if (rows.length !== 4) errors.push('gap_manifest_parent_count');
  const seen = new Set();
  for (const exp of CROSS_PARENT_GAP_MANIFEST.parent_dispositions) {
    const row = rows.find((r) => r && r.parent === exp.parent);
    if (!row) {
      errors.push(`missing_parent_disposition:${exp.parent}`);
      continue;
    }
    seen.add(exp.parent);
    if (!deepEqual(row, deepClone(exp))) {
      errors.push(`parent_disposition_drift:${exp.parent}`);
    }
    const live = parentGatesById && parentGatesById[exp.gate_id];
    if (!live) {
      errors.push(`missing_parent_disposition_live:${exp.parent}`);
      continue;
    }
    if (live.verdict !== exp.verdict) {
      errors.push(`parent_disposition_verdict_mismatch:${exp.parent}`);
    }
    if (live.workstream_class !== exp.workstream_class) {
      errors.push(`parent_disposition_workstream_mismatch:${exp.parent}`);
    }
    if (live.production_readiness !== exp.production_readiness) {
      errors.push(`parent_disposition_production_mismatch:${exp.parent}`);
    }
    if (live.finite_closeout_analog !== exp.finite_closeout_analog) {
      errors.push(`parent_disposition_analog_mismatch:${exp.parent}`);
    }
    if (live[exp.finite_flag] !== exp.finite_flag_value) {
      errors.push(`parent_disposition_finite_flag_mismatch:${exp.parent}`);
    }
  }
  for (const p of expectedParents) {
    if (!seen.has(p) && !errors.some((e) => e === `missing_parent_disposition:${p}`)) {
      errors.push(`missing_parent_disposition:${p}`);
    }
  }
  if (!deepEqual(
    [...(m.remaining_integration_gaps || [])],
    [...CROSS_PARENT_GAP_MANIFEST.remaining_integration_gaps],
  )) {
    errors.push('gap_manifest_remaining_gaps_drift');
  }
  if (!deepEqual(
    [...(m.consistency_checks || [])],
    [...CROSS_PARENT_GAP_MANIFEST.consistency_checks],
  )) {
    errors.push('gap_manifest_consistency_checks_drift');
  }
  return { ok: errors.length === 0, errors };
}

function buildCrossParentIntegrationGate(parentGates) {
  const byId = {};
  for (const g of parentGates || []) byId[g.id] = g;
  const manifest = deepClone(CROSS_PARENT_GAP_MANIFEST);
  const manifestCheck = validateCrossParentGapManifest(manifest, byId);
  return {
    gate: {
      id: 'G_CROSS_PARENT_INTEGRATION',
      verdict: 'absent',
...
/**
 * Validate locked reviewed-disposition inventory on G_MESSI_MILESTONE_CLOSEOUT.
 * Does not raise that gate above absent / does not set messi_complete.
 */
function validateMessiReviewedDispositions(dispositions, gatesById) {
  const errors = [];
  const rows = Array.isArray(dispositions) ? dispositions : [];
  if (rows.length !== MESSI_REVIEWED_DISPOSITIONS.length) {
    errors.push('reviewed_dispositions_count');
  }
  const seen = new Set();
  for (const exp of MESSI_REVIEWED_DISPOSITIONS) {
    const row = rows.find((r) => r && r.disposition_id === exp.disposition_id);
    if (!row) {
      errors.push(`missing_reviewed_disposition:${exp.disposition_id}`);
      continue;
    }
    seen.add(exp.disposition_id);
    if (!deepEqual(row, deepClone(exp))) {
      errors.push(`reviewed_disposition_drift:${exp.disposition_id}`);
    }
    const live = gatesById && gatesById[exp.gate_id];
    if (!live) {
      errors.push(`missing_reviewed_disposition_live:${exp.disposition_id}`);
      continue;
    }
    if (live.verdict !== exp.verdict) {
      errors.push(`reviewed_disposition_verdict_mismatch:${exp.disposition_id}`);
    }
    if (live.workstream_class !== exp.workstream_class) {
      errors.push(`reviewed_disposition_workstream_mismatch:${exp.disposition_id}`);
    }
    if (live.production_readiness !== exp.production_readiness) {
      errors.push(`reviewed_disposition_production_mismatch:${exp.disposition_id}`);
    }
    if (exp.disposition_id === 'CROSS_PARENT_GAP_MANIFEST') {
      if (!live.gap_manifest) {
        errors.push('missing_reviewed_disposition:CROSS_PARENT_GAP_MANIFEST');
      } else if (live.gap_manifest.integration_proof === true
        || exp.integration_proof !== false) {
        errors.push('reviewed_disposition_integration_proof_claimed');
      }
    } else if (exp.finite_flag) {
      if (live.finite_closeout_analog !== exp.finite_closeout_analog) {
        errors.push(`reviewed_disposition_analog_mismatch:${exp.disposition_id}`);
      }
      if (live[exp.finite_flag] !== exp.finite_flag_value) {
        errors.push(`reviewed_disposition_finite_flag_mismatch:${exp.disposition_id}`);
      }
    }
  }
  for (const exp of MESSI_REVIEWED_DISPOSITIONS) {
    if (!seen.has(exp.disposition_id)
      && !errors.some((e) => e === `missing_reviewed_disposition:${exp.disposition_id}`)) {
      errors.push(`missing_reviewed_disposition:${exp.disposition_id}`);
    }
  }
  return { ok: errors.length === 0, errors };
...
function buildMessiMilestoneCloseoutGate(parentAndCrossGates) {
  const byId = {};
  for (const g of parentAndCrossGates || []) byId[g.id] = g;
  const reviewed = deepClone(MESSI_REVIEWED_DISPOSITIONS);
  const reviewedCheck = validateMessiReviewedDispositions(reviewed, byId);
  return {
    gate: {
      id: 'G_MESSI_MILESTONE_CLOSEOUT',
      verdict: 'absent',
      ...
      [MESSI_CLOSEOUT_FINITE_FLAG]: true,
...
function classifyMessiGates(opts) {
  ...
  function pushParentGate(gateId, parentId) {
    ...
    if (!hashBindingOk || !gatesPass) {
      ...
      // audit (FORTRESS), finite staging-readiness (RADAR), and finite offline
      // dry-run (FACTORY) completion are exposed separately — never as parent
      // complete or production readiness.
      verdict = 'partial';
    }
    const gate = {
      id: gateId,
    ...
    // Finite offline dry-run completion is FACTORY-only — never production complete.
    if (parentId === 'FACTORY') {
      gate.finite_offline_dry_run_workstream_complete = parent.workstream_class
          === 'finite_offline_dry_run_packaging_closeout'
        && parent.finite_closeout_analog === 'verify:factory-slice1e-finite-closeout'
        && hashBindingOk
        && gatesPass;
    }
    gates.push(gate);
  ...
  const crossBuilt = buildCrossParentIntegrationGate(gates);
  for (const e of crossBuilt.manifestCheck.errors) errors.push(e);
  gates.push(crossBuilt.gate);

  const messiBuilt = buildMessiMilestoneCloseoutGate(gates);
  for (const e of messiBuilt.reviewedCheck.errors) errors.push(e);
  ...
  // Hard contract: FACTORY parent never complete from finite offline dry-run closeout.
  const factoryGate = gates.find((g) => g.id === 'G_FACTORY_PARENT');
  if (factoryGate) {
    const requiredMissing = [
      'third_tenant_live_or_prod_onboarding',
      'apply_or_disk_materialization',
    ...
    for (const m of requiredMissing) {
      if (!factoryGate.missing_proof.includes(m)) {
        errors.push(`factory_hidden_missing_proof:${m}`);
      }
    }
    if (factoryGate.verdict === 'complete') {
      errors.push('factory_complete_from_finite_closeout');
    }
    if (factoryGate.finite_offline_dry_run_workstream_complete === true
      && factoryGate.production_readiness !== 'absent') {
      errors.push('finite_offline_dry_run_as_production_readiness');
    }
  }

  // Hard contract: gap manifest never promotes G_CROSS_PARENT_INTEGRATION.
  const crossGate = gates.find((g) => g.id === 'G_CROSS_PARENT_INTEGRATION');
  if (crossGate) {
    if (crossGate.verdict !== 'absent') {
      errors.push('gap_manifest_promoted_to_integration_proof');
    }
    if (crossGate.workstream_class !== CROSS_PARENT_WORKSTREAM_CLASS) {
      errors.push('cross_parent_workstream_class_drift');
    }
    if (crossGate.production_readiness !== 'absent') {
      errors.push('cross_parent_production_readiness_claimed');
    }
    if (!crossGate.gap_manifest) {
      errors.push('gap_manifest_missing');
    } else {
      const byId = {};
      for (const g of gates) byId[g.id] = g;
      const mc = validateCrossParentGapManifest(crossGate.gap_manifest, byId);
      for (const e of mc.errors) errors.push(e);
      if (crossGate.gap_manifest.integration_proof === true) {
        errors.push('gap_manifest_promoted_to_integration_proof');
      }
    }
    for (const m of CROSS_PARENT_MISSING_PROOF) {
      if (!crossGate.missing_proof.includes(m)) {
        errors.push(`cross_parent_hidden_missing_proof:${m}`);
      }
    }
  }

  // Hard contract: finite ledger/audit closeout never promotes MESSI milestone.
  const messiGate = gates.find((g) => g.id === 'G_MESSI_MILESTONE_CLOSEOUT');
  if (messiGate) {
    if (messiGate.verdict !== 'absent') {
      errors.push('finite_ledger_closeout_as_MESSI_complete');
    }
    if (messiGate.workstream_class !== MESSI_CLOSEOUT_WORKSTREAM_CLASS) {
      errors.push('messi_closeout_workstream_class_drift');
    }
    if (messiGate.production_readiness !== 'absent') {
      errors.push('finite_ledger_closeout_as_MESSI_complete');
    }
    if (messiGate[MESSI_CLOSEOUT_FINITE_FLAG] !== true) {
      errors.push('messi_finite_ledger_audit_flag_missing');
    }
    if (messiGate.finite_closeout_analog !== MESSI_CLOSEOUT_ANALOG) {
      errors.push('messi_closeout_analog_drift');
    }
    if (!messiGate.reviewed_dispositions) {
      errors.push('reviewed_dispositions_missing');
    } else {
      const byId = {};
      for (const g of gates) byId[g.id] = g;
      const rc = validateMessiReviewedDispositions(messiGate.reviewed_dispositions, byId);
      for (const e of rc.errors) errors.push(e);
    }
    for (const m of MESSI_CLOSEOUT_MISSING_PROOF) {
      if (!messiGate.missing_proof.includes(m)) {
        errors.push(`messi_closeout_hidden_missing_proof:${m}`);
      }
    }
  }
 ...
 * finite_offline_dry_run_workstream_complete are included only when present
 * (FOUNDATION / FORTRESS / RADAR / FACTORY respectively).
 * gap_manifest is included only on G_CROSS_PARENT_INTEGRATION.
 * finite_evidence_integration_ledger_audit_workstream_complete +
 * reviewed_dispositions are included only on G_MESSI_MILESTONE_CLOSEOUT.
 */
function ledgerGateObject(g) {
  ...
  if (Object.prototype.hasOwnProperty.call(g, 'finite_offline_dry_run_workstream_complete')) {
    out.finite_offline_dry_run_workstream_complete = g.finite_offline_dry_run_workstream_complete;
  }
  ...
  if (Object.prototype.hasOwnProperty.call(g, 'reviewed_dispositions')) {
    out.reviewed_dispositions = deepClone(g.reviewed_dispositions);
  }
...
function unrelatedGatesMatchBase(gates) {
  ...
  for (const exp of BASE_UNRELATED_GATE_OBJECTS) {
    ...
    if (Object.prototype.hasOwnProperty.call(got, 'gap_manifest')
...
function validateLedgerFixture(ledger, classification) {
  ...
  if (!classification || !classification.gates) {
    ...
    for (const g of classification.gates) {
      ...
      if (Object.prototype.hasOwnProperty.call(g, 'finite_offline_dry_run_workstream_complete')
        && got.finite_offline_dry_run_workstream_complete
          !== g.finite_offline_dry_run_workstream_complete) {
        errors.push(`finite_offline_dry_run_workstream_complete_mismatch:${g.id}`);
      }
      if (Object.prototype.hasOwnProperty.call(g, 'gap_manifest')) {
        if (!Object.prototype.hasOwnProperty.call(got, 'gap_manifest')
          || !deepEqual(got.gap_manifest, g.gap_manifest)) {
          errors.push(`gap_manifest_mismatch:${g.id}`);
        }
      }
      if (Object.prototype.hasOwnProperty.call(g, MESSI_CLOSEOUT_FINITE_FLAG)
        && got[MESSI_CLOSEOUT_FINITE_FLAG] !== g[MESSI_CLOSEOUT_FINITE_FLAG]) {
        errors.push(`${MESSI_CLOSEOUT_FINITE_FLAG}_mismatch:${g.id}`);
      }
      if (Object.prototype.hasOwnProperty.call(g, 'reviewed_dispositions')) {
        if (!Object.prototype.hasOwnProperty.call(got, 'reviewed_dispositions')
          || !deepEqual(got.reviewed_dispositions, g.reviewed_dispositions)) {
          errors.push(`reviewed_dispositions_mismatch:${g.id}`);
        }
      }
    }
    // FOUNDATION finite staging must remain exposed; FORTRESS finite audit must
    // remain exposed; RADAR finite staging-readiness must remain exposed;
    // FACTORY finite offline dry-run must be exposed.
    ...
    const factoryGot = byId.G_FACTORY_PARENT;
    if (factoryGot
      && factoryGot.finite_offline_dry_run_workstream_complete !== true
      && classification.gates.find((x) => x.id === 'G_FACTORY_PARENT')
        ?.finite_offline_dry_run_workstream_complete === true) {
      errors.push('factory_finite_offline_dry_run_missing_in_ledger');
    }
    const crossGot = byId.G_CROSS_PARENT_INTEGRATION;
    if (crossGot) {
      if (crossGot.verdict !== 'absent') {
        errors.push('gap_manifest_promoted_to_integration_proof');
      }
      if (crossGot.gap_manifest) {
        const mc = validateCrossParentGapManifest(crossGot.gap_manifest, byId);
        for (const e of mc.errors) errors.push(e);
        if (crossGot.gap_manifest.integration_proof === true) {
          errors.push('gap_manifest_promoted_to_integration_proof');
        }
      } else {
        errors.push('gap_manifest_missing');
    ...
    const messiGot = byId.G_MESSI_MILESTONE_CLOSEOUT;
    if (messiGot) {
      if (messiGot.verdict !== 'absent') {
        errors.push('finite_ledger_closeout_as_MESSI_complete');
      }
      if (messiGot[MESSI_CLOSEOUT_FINITE_FLAG] !== true) {
        errors.push('messi_finite_ledger_audit_flag_missing');
      }
      if (messiGot.reviewed_dispositions) {
        const rc = validateMessiReviewedDispositions(messiGot.reviewed_dispositions, byId);
        for (const e of rc.errors) errors.push(e);
      } else {
        errors.push('reviewed_dispositions_missing');
      }
...
const REQUIRED_RED = Object.freeze([
  ...
  // MESSI 1G — FACTORY finite offline dry-run wiring hostiles (only two new)
  'finite_FACTORY_as_production_complete',
  // MESSI 1H — cross-parent disposition gap-manifest hostiles (only two new)
  'missing_parent_disposition',
  'gap_manifest_promoted_to_integration_proof',
...
const REQUIRED_GREEN = Object.freeze([
  ...
  'factory_finite_offline_dry_run_exposed',
  'cross_parent_gap_manifest_absent_gate',
  'messi_finite_ledger_audit_closeout_absent_gate',
...
deepFreeze(BASE_UNRELATED_PARENT_GATE_OBJECTS);
deepFreeze(BASE_UNRELATED_GATE_OBJECTS);
deepFreeze(MESSI_REVIEWED_DISPOSITIONS);
deepFreeze(MESSI_CLOSEOUT_MISSING_PROOF);
...
module.exports = deepFreeze({
  ...
  MESSI_CLOSEOUT_FINITE_FLAG,
  MESSI_CLOSEOUT_ANALOG,
  ...
  PACKAGE_JSON_1G_SCRIPT_KEY,
  PACKAGE_JSON_1G_SCRIPT_VALUE,
  PACKAGE_JSON_1H_SCRIPT_KEY,
  ...
  FACTORY_1E_MERGE_TIP,
  FACTORY_1E_CANDIDATE_SHA,
  FACTORY_1E_MASTER_BASIS,
  CROSS_PARENT_WORKSTREAM_CLASS,
  CROSS_PARENT_GAP_MANIFEST,
  CROSS_PARENT_MISSING_PROOF,
  MESSI_REVIEWED_DISPOSITIONS,
  MESSI_CLOSEOUT_MISSING_PROOF,
  deepFreeze,
  deepClone,
  deepEqual,
  thaw,
  ...
  classifyMessiGates,
  validateCrossParentGapManifest,
  buildCrossParentIntegrationGate,
  validateMessiReviewedDispositions,
  buildMessiMilestoneCloseoutGate,
