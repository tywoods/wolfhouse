...
function failure() {
  ...
  Object.defineProperty(error, 'name', {
    value: 'GoogleVerifiedGrantCustodyError',
  });
...
function snapshotConfig(input) {
  ...
  const clientId = ownData(input, 'clientId'),
    endpointId = ownData(input, 'endpointId'),
    operationId = ownData(input, 'operationId'),
    actorStaffUserId = ownData(input, 'actorStaffUserId'),
  ...
  if (
    ...
    (actorStaffUserId !== null && !UUID.test(actorStaffUserId)) ||
    !bounded(expectedNonce, 512) ||
    !bounded(expectedClientId, 256)
  )
    return null;
...
function normalizeScope(value) {
  ...
  if (
    pieces.length !== GOOGLE_PHASE_A_SCOPES.length ||
...
function snapshotSelected(input) {
  ...
  const accessToken = ownData(input, 'accessToken'),
    refreshToken = ownData(input, 'refreshToken'),
    tokenType = ownData(input, 'tokenType'),
    expiresIn = ownData(input, 'expiresIn'),
    scope = normalizeScope(ownData(input, 'scope')),
    idToken = ownData(input, 'idToken');
  if (
    !printable(accessToken, 8192) ||
    !printable(refreshToken, 8192) ||
    tokenType !== 'Bearer' ||
    !Number.isInteger(expiresIn) ||
    expiresIn < 1 ||
    expiresIn > 86400 ||
    scope === null ||
    !printable(idToken, 32768)
  )
    return null;
  return Object.freeze({
    accessToken,
...
function readIdentity(input) {
  ...
  const providerTenantId = ownData(input, 'providerTenantId'),
    providerPrincipalId = ownData(input, 'providerPrincipalId'),
    mailboxAddress = ownData(input, 'mailboxAddress'),
    displayName = ownData(input, 'displayName');
  if (
    providerTenantId !== 'https://accounts.google.com' ||
    !printable(providerPrincipalId, 255) ||
    typeof mailboxAddress !== 'string' ||
    mailboxAddress.length < 3 ||
    mailboxAddress.length > 254 ||
    mailboxAddress !== mailboxAddress.trim() ||
    !GMAIL.test(mailboxAddress) ||
...
const POLICY = Object.freeze({
  ...
  snapshotSelected,
  readIdentity,
  microsoftStageTelemetry: false,
});
function createGoogleVerifiedGrantCustodyAdapter(config, dependencies) {
  return createVerifiedGrantCustodyAdapter(config, dependencies, POLICY);
}
