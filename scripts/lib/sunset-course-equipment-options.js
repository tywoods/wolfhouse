...
function validateEquipmentOptions(value, scope = {}) {
  ...
  return value.map((row) => {
    ...
    for (const key of Object.keys(row)) {
      if (!['offering_key', 'equipment_price_cents', 'all_day_surcharge_cents'].includes(key)) {
    ...
    return {
      offering_key,
      equipment_price_cents: validateMoney(row.equipment_price_cents, 'equipment_price_cents'),
      all_day_surcharge_cents: validateMoney(row.all_day_surcharge_cents, 'all_day_surcharge_cents'),
    };
  });
}
