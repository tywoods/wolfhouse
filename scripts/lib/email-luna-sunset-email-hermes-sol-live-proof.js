...
const SQL_RESOLVE_PROOF_ACTOR = [
  ...
  '  AND NOT EXISTS (SELECT 1 FROM tenant_email_reply_approvals a',
  '    WHERE a.client_id=c.client_id AND a.conversation_id=c.id',
  '      AND a.source_inbound_event_id=(',
  '        SELECT p2.inbound_event_id FROM tenant_email_inbound_inbox_projections p2',
  '        INNER JOIN tenant_email_inbound_events ev2',
  '          ON ev2.client_id=p2.client_id AND ev2.id=p2.inbound_event_id',
  '        WHERE p2.client_id=c.client_id AND p2.conversation_id=c.id',
  '        ORDER BY ev2.received_at DESC, ev2.id DESC LIMIT 1)',
  '      AND a.state IN (\'draft\',\'approved\',\'terminal\'))',
