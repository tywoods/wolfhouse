 ...
 * Example: 5 SUP × 31 August days = 155 unit-days (not bare stock 5).
 */
...
function isAccommodationCapacityRow(row) {
  ...
  if (Array.isArray(row.offering_keys) && row.offering_keys.some((k) => isAccommodationOfferingKey(k))) {
    return true;
  }
 ...
 * Prefers occupied_nights[]; else derives from check_in → check_out.
 */
 ...
 * @param {{bed_capacity?:number}|null} [args.accommodation_settings]
...
function computeSunsetFinanceSummary(args) {
  ...
  const capacity_by_product = revenue_by_product.filter((p) => p.slot !== 'other').map((p) => {
    ...
    if (isAccommodation) {
      // No invented bed capacity: hide Alojamiento until Admin sets bed_capacity.
    ...
    // used = unit-days in period; denom = daily stock × period days (not bare stock).
