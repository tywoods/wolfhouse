...
/** Sunset-staging Key Vault IMAP secret reader using the established pinned managed identity. */
const {
...
function createSunsetImapKvSecretProvider(deps = {}) {
  ...
  return Object.freeze({
    async resolveSecret(ref) {
      ...
      if (!ALLOWED.has(name) || typeof fetchImpl !== 'function') {
        throw new Error('imap_secret_resolve_failed');
      }
      try {
        if (!credential) {
        ...
        const response = await fetchImpl(`${VAULT}/secrets/${encodeURIComponent(name)}?api-version=7.4`, {
          method: 'GET',
          headers: { authorization: ['Bearer', token.token].join(' ') },
...
module.exports = Object.freeze({ createSunsetImapKvSecretProvider });
