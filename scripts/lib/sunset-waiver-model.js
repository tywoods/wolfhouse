 ...
 *
 * No public UI routes, Luna wiring, or outbound sends in this module.
 */
...
/**
 * Staging-safe public base for waiver links.
 * Prefers env overrides; defaults to sunset-staging (never production hostname).
 */
...
/**
 * @returns {string} waiv_<url-safe random>
 */
function generateWaiverPublicId() {
  // 12 bytes → 16 base64url chars; example shape waiv_abc123… (longer than demo).
...
/**
 * Stable sha256 hex of the public token (includes waiv_ prefix).
 */
...
/**
 * Create a pending waiver request. Returns row + publicId (+ publicUrl if baseUrl/env).
 */
async function createWaiverRequest(pg, input) {
  ...
  const insertSql = `
    INSERT INTO waiver_form_requests (
      ...
      public_id, token_hash, status, form_type, form_version,
...
/**
 * Resolve a public waiv_* token to a request via token_hash.
 */
...
/**
 * Record a submission and mark the request completed (transaction-safe).
 * Duplicate submit on an already-completed request returns the existing submission (idempotent).
 */
async function recordWaiverSubmission(pg, input) {
  ...
  if (request.status === 'completed') {
    const existing = await getSubmissionByRequestId(pg, tenantId, request.id);
    return {
      ok: true,
      status: 200,
      idempotent: true,
  ...
  if (request.status !== 'pending' && request.status !== 'needs_review') {
    return { ok: false, status: 409, error: 'invalid_status', request };
  }
  ...
  try {
    // Re-lock request row for race-safe completion.
    ...
    if (locked.status === 'completed') {
      ...
      return { ok: true, status: 200, idempotent: true, request: locked, submission: existing };
    }
    ...
    if (insertRes.rows.length) {
      submission = mapSubmissionRow(insertRes.rows[0]);
    } else {
      submission = await getSubmissionByRequestId(pg, tenantId, locked.id);
    }
    ...
    return {
      ok: true,
      status: insertRes.rows.length ? 201 : 200,
      idempotent: !insertRes.rows.length,
