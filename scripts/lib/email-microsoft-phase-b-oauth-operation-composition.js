...
const SCOPE_REJECTION_STAGES = Object.freeze({
  invalid: 'token_response_scope_rejected_invalid',
  duplicate: 'token_response_scope_rejected_duplicate',
  dangerous: 'token_response_scope_rejected_dangerous',
  phase_a_mixed: 'token_response_scope_rejected_phase_a_mixed',
  unknown: 'token_response_scope_rejected_unknown',
  missing_required: 'token_response_scope_rejected_missing_required',
});
...
function validateTokenResponse(response, stageTelemetry) {
  ...
  const scopeResult = classifyAndNormalizePhaseBTokenResponseScope(scope);
  const ns = scopeResult.value;
  if (ns === null) {
    safeEmitStage(stageTelemetry, SCOPE_REJECTION_STAGES[scopeResult.rejectionCategory]);
    throw failure();
  }
