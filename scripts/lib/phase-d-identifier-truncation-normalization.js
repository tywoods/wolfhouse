...
/**
 * FOUNDATION Slice 14X — exact NAMEDATALEN truncated NOT NULL
 * provenance normalization
 *
 * Merged target-authority proof (14Q skipPostgres) + exactly one TLS
 * verify-full read-only observer session
 * (application_name=wh-sunset-identifier-truncation-normalization) that reports:
 *   - safe server_version class (PG15 required for identifier truncation apply)
 *   - baseline mismatch counts (identity + 14T + 14V rename alias; identifier truncation off)
 *   - after mismatch counts (identity + 14T + 14V + identifier truncation)
...
/** Live capability activated for Slice 14X behind exact env+argv gates. */
const PHASE_D_IDENTIFIER_TRUNCATION_NORMALIZATION_LIVE_ENABLED = true;

const ENV_IDENTIFIER_TRUNCATION_NORMALIZATION = 'SUNSET_PHASE_D_IDENTIFIER_TRUNCATION_NORMALIZATION';
const CLI_PROVE_IDENTIFIER_TRUNCATION_NORMALIZATION = '--prove-identifier-truncation-normalization';
const APPLICATION_NAME = 'wh-sunset-identifier-truncation-normalization';

const BASELINE_MISMATCH_COUNT = 12;
...
const IDENTIFIER_TRUNCATION_LOCKS = Object.freeze({
  ...AUTHORITY_LOCKS,
...
function resetIdentifierTruncationNormalizationCounters() {
...
function getIdentifierTruncationNormalizationCounters() {
...
function parseArgvPairs(argv) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (a === CLI_PROVE_IDENTIFIER_TRUNCATION_NORMALIZATION
...
function exactIdentifierTruncationNormalizationArgv() {
  return [
    CLI_PROVE_IDENTIFIER_TRUNCATION_NORMALIZATION,
    CLI_PROVE_TARGET_AUTHORITY,
    '--subscription', IDENTIFIER_TRUNCATION_LOCKS.subscriptionId,
    '--resource-group', IDENTIFIER_TRUNCATION_LOCKS.resourceGroup,
    '--container-app', IDENTIFIER_TRUNCATION_LOCKS.containerAppName,
    '--postgres-server', IDENTIFIER_TRUNCATION_LOCKS.postgresServer,
    '--database', IDENTIFIER_TRUNCATION_LOCKS.database,
    CLI_CREDENTIAL_SOURCE, CREDENTIAL_SOURCE_MANAGED_IDENTITY,
  ];
}

function identifierTruncationNormalizationEnv() {
  return {
    ...targetAuthorityEnv(),
...
function evaluateIdentifierTruncationNormalizationGates(opts) {
  const options = opts || {};
  ...
  if (PHASE_D_IDENTIFIER_TRUNCATION_NORMALIZATION_LIVE_ENABLED !== true) {
    errors.push({ code: 'identifier_truncation_normalization_capability_disabled', message: 'final-rename normalization live disabled' });
  }
  ...
  if (String(env[ENV_IDENTIFIER_TRUNCATION_NORMALIZATION] || '') !== '1') {
    errors.push({ code: 'identifier_truncation_normalization_env_required', message: `${ENV_IDENTIFIER_TRUNCATION_NORMALIZATION}=1 required` });
  }
  if (String(env[ENV_SUBSCRIPTION] || '') !== IDENTIFIER_TRUNCATION_LOCKS.subscriptionId) {
  ...
  if (!parsed.flags.has(CLI_PROVE_IDENTIFIER_TRUNCATION_NORMALIZATION)) {
    errors.push({
      code: 'identifier_truncation_normalization_flag_required',
      message: `${CLI_PROVE_IDENTIFIER_TRUNCATION_NORMALIZATION} required`,
    });
  }
...
function buildLockedTruncationPgClientConfig(user, password) {
  return {
    host: IDENTIFIER_TRUNCATION_LOCKS.postgresHost,
    port: IDENTIFIER_TRUNCATION_LOCKS.port,
    database: IDENTIFIER_TRUNCATION_LOCKS.database,
    user: String(user),
...
function summarizeCompare(cmp) {
  if (cmp.normalizationError) {
    return {
      ...
      identifierTruncationsNormalized: null,
  ...
  const finalRenamesNormalized = cmp.finalRenameNormalization
    ...
    : 0;
  const identifierTruncationsNormalized = cmp.identifierTruncationNormalization
    ? Number(cmp.identifierTruncationNormalization.normalizedCount) || 0
    : 0;
  return {
    ok: cmp.ok === true,
    match: cmp.ok === true,
    ...
    identifierTruncationsNormalized,
    ...
    finalRenameNormalization: cmp.finalRenameNormalization
      ...
      : null,
    identifierTruncationNormalization: cmp.identifierTruncationNormalization
      ? {
        applied: cmp.identifierTruncationNormalization.applied === true,
        normalizedCount: identifierTruncationsNormalized,
        reason: cmp.identifierTruncationNormalization.reason || null,
        provenance: cmp.identifierTruncationNormalization.provenance || null,
      }
      : null,
...
/**
 * Baseline gate: post-14W residual inventory is exactly 12 mismatches
 * (14T+14V+14W on; identifier truncation disabled). Stop with
 * baseline_drift_mismatch otherwise.
 */
...
async function verifyIdentifierTruncationSession(client) {
  const errors = [];
...
function extractIdentifierTruncationProvenanceSummary(truncNorm) {
  if (!truncNorm || !truncNorm.provenance) {
    return {
  ...
  const prov = truncNorm.provenance;
  return {
    ...
    lockedTuple: prov.lockedTuple || null,
  };
}

async function runIdentifierTruncationObserverCompare(client, expectedContract) {
  const session = await verifyIdentifierTruncationSession(client);
  if (!session.ok) {
    return {
  ...
  // Baseline: identity + 14T + 14V + 14W final rename ON; identifier truncation OFF.
  // Must equal post-14W residual inventory (mismatchCount === 12).
  ...
  if (!baseline.ok) {
    return {
      ...
      finalRenamesNormalized: observerBefore.finalRenamesNormalized != null
        ? observerBefore.finalRenamesNormalized
        : (rawCmp.finalRenameNormalization
          ? Number(rawCmp.finalRenameNormalization.normalizedCount) || 0
          : null),
      identifierTruncationsNormalized: null,
      remainingMismatchCount: null,
      remainingKeys: [],
  ...
  // After: identity + 14T + 14V + 14W + identifier truncation normalization.
  ...
  const renameAliasesNormalized = observerAfter.renameAliasesNormalized != null
    ...
    : 0;
  const identifierTruncationsNormalized = observerAfter.identifierTruncationsNormalized != null
    ? observerAfter.identifierTruncationsNormalized
    : 0;
  ...
  const provSummary = extractIdentifierTruncationProvenanceSummary(
    observerAfter.identifierTruncationNormalization,
  );
  const finalRenamesNormalized = normCmp.finalRenameNormalization
    ? Number(normCmp.finalRenameNormalization.normalizedCount) || 0
    : 0;
  ...
  return {
    ...
    remainingKeys,
    accountingOk: BASELINE_MISMATCH_COUNT === identifierTruncationsNormalized + remainingMismatchCount,
    productFingerprintLive,
    ...
    lockedTuple: provSummary.lockedTuple,
    errors: [],
  };
}
 ...
 */
async function executeIdentifierTruncationNormalization(opts) {
  const options = opts || {};
  const gate = evaluateIdentifierTruncationNormalizationGates(options);
  if (!gate.ok) {
    return pickSafe({
      ok: false,
  ...
  if (options.injectedObserver) {
    ...
    return pickSafe({
      ...
      code: inj.code || (inj.stopReason === 'baseline_drift_mismatch'
        ? 'baseline_drift_mismatch'
        : 'identifier_truncation_normalization_injected'),
      sameTarget: true,
      sameTargetReason: authority.sameTargetReason || 'same_exact_authority',
      ...
      renameAliasesNormalized: inj.renameAliasesNormalized != null
        ...
        : null,
      identifierTruncationsNormalized: inj.identifierTruncationsNormalized != null
        ? inj.identifierTruncationsNormalized
        : null,
      ...
      lockedTuple: inj.lockedTuple || null,
  ...
  const loaded = await loadProtectedAdminCredentialsViaManagedIdentity({
    env: options.env || identifierTruncationNormalizationEnv(),
    argv: options.argv || exactIdentifierTruncationNormalizationArgv(),
    httpRequest,
  });
  if (!loaded.ok) {
    return pickSafe({
  ...
  try {
    ...
    const cfg = buildLockedTruncationPgClientConfig(user, password);
    ...
    const obs = await runIdentifierTruncationObserverCompare(client, options.expectedContract);
    ...
    return pickSafe({
      ...
      code: after.match === true
        ? 'identifier_truncation_normalization_observer_match'
        : 'identifier_truncation_normalization_observer_drift',
      sameTarget: true,
      sameTargetReason: authority.sameTargetReason || 'same_exact_authority',
      ...
      baselineMismatchCount: obs.baselineMismatchCount,
      renameAliasesNormalized: obs.renameAliasesNormalized,
      identifierTruncationsNormalized: obs.identifierTruncationsNormalized,
      ...
      lockedTuple: obs.lockedTuple || null,
      productFingerprintLive: obs.productFingerprintLive,
      ...getIdentifierTruncationNormalizationCounters(),
      applicationName: APPLICATION_NAME,
      ...
      managedIdentityName: IDENTIFIER_TRUNCATION_LOCKS.managedIdentityName,
      keyVaultName: IDENTIFIER_TRUNCATION_LOCKS.keyVaultName,
      kvSecretName: IDENTIFIER_TRUNCATION_LOCKS.secretName,
      errors: [],
      closed: true,
      committed: true,
      rolledBack: false,
    ...
    return pickSafe({
      ok: false,
      code: e.code || 'identifier_truncation_normalization_failed',
      sameTarget: true,
      blocker: e.code || 'identifier_truncation_normalization_failed',
      liveMutation: false,
      schemaMutation: false,
      dataMutation: false,
      usedLiveHttp,
...
function createScriptedIdentifierTruncationFakeClientFactory(script) {
  const s = script || {};
  function FakeClient() {
    this._ended = false;
    this.connect = async () => {};
...
function printCliHelp() {
  return [
    'phase-d:identifier-truncation-normalization — FOUNDATION Slice 14X',
    'DEFAULT: refused (zero ARM / zero KV / zero PostgreSQL).',
    '',
    'Merged target-authority + one read-only observer session proving',
    'exact NAMEDATALEN truncated NOT NULL identifier normalization',
    '(package_price_rules_double_supplement_per_person_per_n_not_null)',
    'under azure_flexible_server_v1 + postgresql_15.',
    'Requires dual Phase D flags + TARGET_AUTHORITY + IDENTIFIER_TRUNCATION_NORMALIZATION',
    '+ managed-identity + exact locked targets.',
    '',
    'Baseline must be exactly 12 post-14W residuals (identifier truncation off).',
...
module.exports = {
  ...
  createScriptedIdentifierTruncationFakeClientFactory,
  createInjectedTargetAuthorityHttp,
  createInjectedManagedIdentityHttp,
  buildOfflineProofSunsetDatabaseUrl,
