...
/**
 * FORTRESS 15J — offline HTTP harness for the real staff-query-api listener/router
...
const ACCESS_FIXTURE = path.join(
  ROOT,
  'fixtures',
  'fortress-tenant-identity',
  'slice15j-staff-portal-access.json',
);

const BOT_TOKEN = 'fortress15j_bot_token_offline_test_01';
const SESSION_ALPHA = 'fortress15j-session-alpha';
const SESSION_MULTI = 'fortress15j-session-multi';

const SESSION_USERS = {
  [SESSION_ALPHA]: {
    staff_user_id: 'staff-alpha-1',
    email: 'fortress15j.alpha@example.test',
    role: 'operator',
    status: 'active',
  ...
  [SESSION_MULTI]: {
    ...
    email: 'fortress15j.multi@example.test',
    role: 'operator',
    status: 'active',
...
function applyHarnessEnv(overrides) {
  const base = {
    ...
    STAFF_PORTAL_ACCESS_FILE: ACCESS_FIXTURE,
  };
...
function createFortress15jOfflineListener(opts) {
  ...
  api.setFortress15jOfflineSeams({
...
async function closeHarness(harness) {
  ...
  if (harness.api && harness.api.setFortress15jOfflineSeams) {
    harness.api.setFortress15jOfflineSeams(null);
  }
