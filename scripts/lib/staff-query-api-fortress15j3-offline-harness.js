...
/**
 * FORTRESS 15J3 — offline HTTP harness for the real staff-query-api listener/router
...
const BOT_TOKEN = 'fortress15j3_bot_token_offline_test_01';
const SESSION_ALPHA = 'fortress15j3-session-alpha';
const SESSION_MULTI = 'fortress15j3-session-multi';

/** Harness-only ACL map — production staff-portal access file is never overridden. */
const HARNESS_CLIENT_ACCESS = Object.freeze({
  'fortress15j3.alpha@example.test': ['wolfhouse-somo'],
  'fortress15j3.multi@example.test': ['wolfhouse-somo', 'sunset'],
});

const SESSION_USERS = {
  [SESSION_ALPHA]: {
    staff_user_id: 'staff-alpha-1',
    email: 'fortress15j3.alpha@example.test',
    role: 'operator',
    status: 'active',
  ...
  [SESSION_MULTI]: {
    ...
    email: 'fortress15j3.multi@example.test',
    role: 'operator',
    status: 'active',
...
function fortress15j3DualGateActive() {
  return String(process.env.NODE_ENV || '').toLowerCase() === 'test'
    && String(process.env.STAFF_API_FORTRESS_OFFLINE_LISTENER || '').trim() === '1';
}

function assertFortress15j3DualGate() {
  if (!fortress15j3DualGateActive()) {
    const err = new Error(
      'FORTRESS 15J3 offline listener requires NODE_ENV=test AND STAFF_API_FORTRESS_OFFLINE_LISTENER=1',
    );
    err.code = 'FORTRESS_15J3_DUAL_GATE';
    throw err;
  }
}
...
function applyHarnessEnv(overrides) {
  const base = {
    ...
    STRIPE_SECRET_KEY: 'sk_test_15j3_offline',
...
function createFortress15j3OfflineListener(opts) {
  ...
  if (!server || typeof server.listen !== 'function') {
    const err = new Error(
      'FORTRESS 15J3 dual-gated require must export factory-built server from staff-query-api',
    );
    err.code = 'FORTRESS_15J3_DUAL_GATE';
    throw err;
  }
...
async function closeHarness(harness) {
  ...
  if (harness.api && harness.api.setFortress15j3OfflineSeams) {
    harness.api.setFortress15j3OfflineSeams(null);
  }
