...
function failure() {
  ...
  Object.defineProperty(error, 'name', {
    value: 'MicrosoftVerifiedGrantCustodyError',
  });
...
function snapshotConfig(input) {
  ...
  if (
    ...
    !uuid(endpointId) ||
    !uuid(operationId) ||
    (actorStaffUserId !== null && !uuid(actorStaffUserId)) ||
    !bounded(expectedNonce, NONCE_LIMIT) ||
    !bounded(expectedClientId, CLIENT_ID_LIMIT)
  )
    return null;
  return Object.freeze({
    clientId,
...
function readIdentity(input) {
  ...
  if (
    !bounded(providerTenantId, PRINCIPAL_LIMIT) ||
    !UUID_CANON.test(providerTenantId) ||
    !bounded(providerPrincipalId, PRINCIPAL_LIMIT) ||
