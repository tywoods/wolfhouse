 ...
 *   - package present → grouped package pebble, e.g. Uluwatu x3, reusing package colors
 *   - no package → pebble text exactly "no pebble"
...
ok('bar inner html includes the package pebble', innerFn.includes('bcCalendarPackagePebbleHtml(blk)'));
...
if (ran) {
  ...
  const uluwatu = sandbox.bcCalendarPackagePebbleHtml({
    guest_count: 3,
  ...
  ok('package pebble text is Uluwatu x3', uluwatu.includes('Uluwatu x3'), uluwatu);
  ...
  const none = sandbox.bcCalendarPackagePebbleHtml({
    guest_count: 2,
    package_code: 'no_package',
  });
  const noneText = none.replace(/<[^>]+>/g, '');
  ok('no package pebble text is exactly no pebble', noneText === 'no pebble', JSON.stringify(noneText));
  ok('missing package is not blank', noneText.length > 0 && !/no package/i.test(noneText));
  ...
  ok('bar keeps name, transfer, package, and amount together',
    inner.includes('Tom') && inner.includes('Transfer') && inner.includes('Uluwatu x3') && inner.includes('€997.00')
      && !/Balance due/.test(inner),
    inner);
