...
/**
 * run-phase-d-final-rename-normalization — FOUNDATION Slice 14W
 *
 * Default-disabled operator CLI: merged target authority + one read-only
 * observer session proving migrations 002/003/004 final NOT NULL rename
 * provenance normalization under azure_flexible_server_v1 + postgresql_15.
 *
...
function defaultRefuseOutput() {
  return {
    ...
    finalRenameNormalizationLiveEnabled: PHASE_D_FINAL_RENAME_NORMALIZATION_LIVE_ENABLED === true,
...
async function main() {
  ...
  resetFinalRenameNormalizationCounters();
  const gate = evaluateFinalRenameNormalizationGates({ env: process.env, argv });
  if (!gate.ok) {
    const out = {
      ...defaultRefuseOutput(),
  ...
  const safe = {
    ...
    renameAliasesNormalized: result.renameAliasesNormalized != null
      ...
      : null,
    finalRenamesNormalized: result.finalRenamesNormalized != null
      ? result.finalRenamesNormalized
      : null,
    ...
    migration004Sha256: result.migration004Sha256 || null,
