 ...
 * price. Shows the generic-rental create working: 201 + total, idempotent replay,
 * and 409 on same-key quantity change. In-process only — no DB, no network.
 */
...
function buildDemoPg() {
  ...
  return {
    ...
    query: async (sql, params) => {
      ...
      // Generic rental catalog: one active kayak offering at Somo.
      if (/FROM tenant_rental_offerings/i.test(q)) {
        return { rows: [{ id: 'ro-1', client_slug: 'sunset', location_id: 'sunset-somo', offering_key: 'kayak_rental', label: 'Kayak (DEMO)', group_key: 'sup', excludes: '[]', sort_order: 5, active: true }] };
      }
...
(async () => {
  ...
  console.log('\n=== DEMO: generic kayak rental create (flag ON) ===\n');
  ...
  // Safety default: with the flag OFF, the same kayak create is rejected unchanged.
  process.env.GENERIC_RENTAL_CREATE_ENABLED = '';
  const offPg = buildDemoPg();
  const flagOff = await createSunsetScheduleBooking(offPg, { clientSlug: 'sunset', locationId: 'sunset-somo', actor, body: kayakBody({ idempotency_key: 'demo-off' }) });
  console.log('\nFLAG-OFF status', flagOff.status, '→ error', flagOff.body && (flagOff.body.error || flagOff.body.reason_code), '(default-safe: generic rejected, canonical unchanged)');
  process.env.GENERIC_RENTAL_CREATE_ENABLED = 'true';

  const ok = create.status === 201 && create.body.total_cents === 5000
    && flagOff.status === 400 && (flagOff.body.error === 'invalid_rental_offering' || flagOff.body.reason_code === 'invalid_rental_offering')
  ...
  console.log('\n=== DEMO RESULT:', ok ? 'WORKS ✓ (201 €50 addon_service · replay 200 same booking · qty-change 409)' : 'unexpected — inspect above', '===\n');
  process.exit(ok ? 0 : 1);
