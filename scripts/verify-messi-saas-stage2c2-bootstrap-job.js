...
async function main() {
  ...
  ok('main_depends_on', /dependsOn:[\s\S]{0,260}(pgApp|appDb)/.test(mainSrc)
    && /dependsOn:[\s\S]{0,260}(containerAppsEnv|managedIdentity)/.test(mainSrc)
    && !/dependsOn:[\s\S]{0,260}acrPullRole/.test(mainSrc));
  ...
  ok('acr_pull_not_emitted_from_bicep',
    ...
    && /"resources"\s*:\s*\[\s*\]/.test(acrPullJson));
  ...
  if (job && mainSrc && bin()) {
    try {
      ...
      ok('compile_acr_pull_no_shared_rg_deployment', acrPullNamedDeps.length === 0,
        JSON.stringify(acrPullNamedDeps.map((r) => r.name)));
      ...
      ok('compile_acr_pull_absent_from_bicep', acrPullRoles.length === 0,
        `n=${acrPullRoles.length}`);
      const bootDep = JSON.stringify(((compiledMain.resources || [])
        .find((r) => /syntheticBootstrapJob/.test(String(r.name || ''))) || {}).dependsOn || []);
      ok('compile_bootstrap_no_bicep_acr_pull_dependson', !/acrPullRole/.test(bootDep), bootDep.slice(0, 180));
    } catch (err) {
      skipMany(['compile_job','compile_manual','compile_retry_zero','compile_parallelism','compile_completions','compile_no_ingress',
        ...
        'compile_acr_pull_no_shared_rg_deployment','compile_acr_pull_absent_from_bicep','compile_bootstrap_no_bicep_acr_pull_dependson'],
