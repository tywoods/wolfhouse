...
// Authentic RED: this production owner is intentionally absent until GREEN.
const googleJwks = require('./lib/email-google-oidc-jwks-verifier');
const { createGoogleOidcJwksVerifier } = googleJwks;
const { createGoogleOidcVerifiedIdentity } = require('./lib/email-google-oidc-id-token');

const FAILURE_CODE = 'GOOGLE_OIDC_JWKS_VERIFICATION_FAILED';
...
async function expectSanitizedFailure(action) {
  await assert.rejects(
    ...
    (error) => {
      assert.equal(error.name, 'GoogleOidcJwksVerificationError');
      assert.equal(error.code, FAILURE_CODE);
      assert.equal(error.message, 'Google OIDC signature verification failed.');
...
async function verifyWith(configuration, input = verificationRequest()) {
  ...
  const verifier = createGoogleOidcJwksVerifier(harness.dependencies);
  return { harness, result: await verifier.verifySignature(input) };
}
...
test('accepts a generated RSA signature and returns an exact sealed acknowledgement', async function acceptsSignature() {
  assert.deepEqual(Object.keys(googleJwks), ['createGoogleOidcJwksVerifier']);
  assert.equal(Object.isFrozen(googleJwks), true);
  const service = createGoogleOidcJwksVerifier(makeHarness().dependencies);
  assert.deepEqual(Reflect.ownKeys(service), ['verifySignature']);
...
test('uses only the fixed Google organizations JWKS destination', async function checksFixedDestination() {
...
test('preserves exact verifier and dependency receivers', async function checksVerifierReceiver() {
  ...
  const verifier = createGoogleOidcJwksVerifier(harness.dependencies);
  assert.deepEqual(await Reflect.apply(verifier.verifySignature, verifier, [verificationRequest()]), { verified: true });
...
test('claims single use atomically before reflecting on hostile input', async function claimsSingleUseFirst() {
  ...
  await expectSanitizedFailure(() => verifier.verifySignature(hostile));
  await expectSanitizedFailure(() => verifier.verifySignature(verificationRequest()));
...
test('snapshots caller-owned signature bytes before awaiting JWKS', async function snapshotsSignature() {
  ...
  const passingVerifier = createGoogleOidcJwksVerifier(passingHarness.dependencies);
  const validAtInvocation = Buffer.from(validSignature);
  const passing = passingVerifier.verifySignature(verificationRequest({ signature: validAtInvocation }));
  ...
  const failingVerifier = createGoogleOidcJwksVerifier(failingHarness.dependencies);
  const invalidAtInvocation = Buffer.alloc(validSignature.length);
  const failing = failingVerifier.verifySignature(verificationRequest({ signature: invalidAtInvocation }));
...
test('rejects altered signing input, signature, algorithm, and kid', async function rejectsAlteredInputs() {
  ...
  for (const input of cases) {
    const harness = makeHarness();
    await expectSanitizedFailure(() => createGoogleOidcJwksVerifier(harness.dependencies).verifySignature(input));
  }
});
...
test('requires exact frozen dependency records and methods', async function rejectsDependencyShapes() {
  ...
  for (const dependencies of variants) {
    assert.throws(() => createGoogleOidcJwksVerifier(dependencies), { code: FAILURE_CODE });
  }
});
...
test('does not start HTTPS when the deadline callback fires synchronously', async function handlesSynchronousDeadline() {
  ...
  await expectSanitizedFailure(() => createGoogleOidcJwksVerifier(harness.dependencies).verifySignature(verificationRequest()));
  assert.equal(harness.state.requestCalls, 0);
  assert.equal(harness.state.clearCalls.length, 1);
});
...
test('destroys a response delivered after timeout', async function destroysLateResponse() {
  ...
  const promise = createGoogleOidcJwksVerifier(harness.dependencies).verifySignature(verificationRequest());
...
test('safely ignores bounded unknown JWKS fields', async function ignoresSafeUnknownFields() {
  ...
  assert.deepEqual(
    await createGoogleOidcJwksVerifier(harness.dependencies).verifySignature(verificationRequest()),
    { verified: true },
  );
});
...
test('passes only selected JWK fields and exact bytes to native-shaped crypto methods', async function checksCryptoArguments() {
  ...
  const result = await createGoogleOidcJwksVerifier(harness.dependencies).verifySignature(verificationRequest());
  assert.deepEqual(result, { verified: true });
...
function validatorFor(configuration = {}) {
  const harness = makeHarness(configuration);
  const signatureVerifier = createGoogleOidcJwksVerifier(harness.dependencies);
  return createGoogleOidcVerifiedIdentity(Object.freeze({ signatureVerifier }));
}

test('verifies a generated Google RSA token including at_hash through the merged identity validator', async function checksValidatorEndToEnd() {
  const now = 1900000000;
  const nonce = 'offline-nonce';
  const client = 'offline-client';
  const accessToken = 'offline-google-access-token';
  const header = { alg: 'RS256', kid, typ: 'JWT' };
  const claims = {
    sub: 'subject',
    email: 'Owner.Case@example.com',
    email_verified: true,
    name: 'Owner Case',
    aud: client,
    nonce,
    ...
    iat: now - 1,
    nbf: now - 1,
    at_hash: nativeCrypto.createHash('sha256').update(accessToken, 'ascii').digest().subarray(0, 16).toString('base64url'),
  };
  const input = Object.freeze({ idToken: createToken(header, claims), accessToken, expectedNonce: nonce, expectedClientId: client, nowEpochSeconds: now });
  const result = await validatorFor().verifyIdentity(input);
  assert.deepEqual(result, {
    providerTenantId: 'https://accounts.google.com', providerPrincipalId: 'subject',
    mailboxAddress: 'Owner.Case@example.com', displayName: 'Owner Case',
  });
  ...
  for (const idToken of hostileTokens) {
    await assert.rejects(
      validatorFor().verifyIdentity(Object.freeze({ ...input, idToken })),
      { code: 'GOOGLE_OIDC_VERIFIED_IDENTITY_INVALID' },
    );
  }
...
async function runTests() {
  ...
  process.stdout.write(`PASS verify:email-google-oidc-jwks-verifier (${tests.length} named offline tests)\n`);
}
