...
/**
 * verify:fortress-slice15h2-meta-ingress-authority-xact-lock — FORTRESS Slice 15H2
 *
 * Offline RED/GREEN tests for Meta WhatsApp ingress authority policy (B02) with
 * transactional pg_advisory_xact_lock global wa_message_id claim.
...
const CONTRACT_PATH = path.join(FIXTURE_DIR, 'slice15h2-contract.json');
const OVERLAY_PATH = path.join(FIXTURE_DIR, 'slice15h2-b02-remediation-overlay.json');
const FINDINGS_PATH = path.join(FIXTURE_DIR, 'slice15h2-findings.md');
const EVIDENCE_PATH = path.join(FIXTURE_DIR, 'slice15h2-evidence.json');
...
function buildFakeMetaWhatsAppBody(phoneNumberId, opts = {}) {
  ...
  const waMessageId = opts.wa_message_id || 'wamid.SAMPLE_15H2_INBOUND';
  const metadata = {
...
function makeCountingPg() {
  ...
  return {
    ...
    query: async (sql, params) => {
      ...
      throw new Error('15H2 zero-downstream: unexpected pg.query');
    },
  };
}
 ...
 * Tracks BEGIN/COMMIT/ROLLBACK + pg_advisory_xact_lock; never opens a live DB.
 * Unexpected owner/demo/persistence SQL fails closed (no generic success).
 * Pass `shared` so multiple clients share one event store + xact lock wait graph
 * (required for concurrent cross-slug claim proofs).
 */
function makeHarnessPg(options = {}) {
  const shared = options.shared && typeof options.shared === 'object' ? options.shared : null;
  const events = shared && Array.isArray(shared.events)
    ? shared.events
    : (Array.isArray(options.events) ? options.events.map(cloneEventRow) : []);
  ...
  if (shared && shared.idSeq == null) shared.idSeq = options.idSeq || 1000;
  let localIdSeq = options.idSeq || 1000;
  function nextId() {
    if (shared) {
      shared.idSeq = (shared.idSeq || 1000) + 1;
      return shared.idSeq;
    }
    localIdSeq += 1;
    return localIdSeq;
  ...
  let activeTxId = null;
  let txSnapshot = null;
  const xactLockWaiters = shared && shared.xactLockWaiters instanceof Map
    ? shared.xactLockWaiters
    : new Map();
  const xactHeldLocks = shared && shared.xactHeldLocks instanceof Map
    ? shared.xactHeldLocks
    : new Map();
  const effects = {
    queries: [],
    candidate_selects: 0,
  ...
  function releaseXactLocksForTx(txId) {
    for (const [key, held] of xactHeldLocks.entries()) {
      if (held.txId === txId) {
        held.release();
        xactHeldLocks.delete(key);
      }
    }
  }

  function finishTransaction(kind) {
    if (!inTransaction) {
      throw new Error(`15H2 harness: ${kind} without BEGIN`);
    }
    releaseXactLocksForTx(activeTxId);
    if (kind === 'ROLLBACK' && txSnapshot) {
      events.length = 0;
      for (const row of txSnapshot) {
        events.push(cloneEventRow(row));
      }
    }
    inTransaction = false;
    activeTxId = null;
  ...
  async function acquireXactAdvisory(params) {
    if (!inTransaction) {
      throw new Error('15H2 harness: pg_advisory_xact_lock outside transaction');
    }
    const key = lockKey(params);
    effects.advisory_xact_locks += 1;
    const prev = xactLockWaiters.get(key) || Promise.resolve();
    let releaseCurrent;
    const gate = new Promise((resolve) => { releaseCurrent = resolve; });
    const next = prev.then(() => gate);
    xactLockWaiters.set(key, next.then(() => {}, () => {}));
    await prev;
    xactHeldLocks.set(key, { txId: activeTxId, release: releaseCurrent });
  }
  ...
  async function query(sql, params) {
    ...
    if (/^\s*BEGIN\s*$/i.test(s)) {
      if (inTransaction) {
        throw new Error('15H2 harness: nested BEGIN');
      }
      inTransaction = true;
      activeTxId = ++currentTxId;
      txSnapshot = events.map(cloneEventRow);
      effects.begins += 1;
      return { rows: [] };
    }
    if (/^\s*COMMIT\s*$/i.test(s)) {
      finishTransaction('COMMIT');
      return { rows: [] };
    }
    if (/^\s*ROLLBACK\s*$/i.test(s)) {
      finishTransaction('ROLLBACK');
      return { rows: [] };
    }

    if (/pg_advisory_lock\s*\(/i.test(s) && !/xact/i.test(s)) {
      effects.advisory_locks += 1;
      throw new Error('15H2 harness: session pg_advisory_lock is forbidden (use pg_advisory_xact_lock)');
    }
    if (/pg_advisory_unlock\s*\(/i.test(s)) {
      effects.advisory_unlocks += 1;
      throw new Error('15H2 harness: session pg_advisory_unlock is forbidden (xact lock auto-releases)');
    }
    if (/pg_advisory_xact_lock\s*\(/i.test(s)) {
      await acquireXactAdvisory(p);
    ...
    throw new Error(`15H2 harness: unexpected SQL (fail-closed): ${s.replace(/\s+/g, ' ').slice(0, 160)}`);
  }
...
async function runBlockedEntry(normalizedOrBody, opts = {}) {
  ...
  const entryInput = {
    ...
    processInbound: async () => {
      ...
      counters.owner += 1;
      counters.demo += 1;
      throw new Error('15H2 entry: processInbound must not run when blocked');
    },
  };
  if (opts.normalized) {
...
console.log('verify:fortress-slice15h2-meta-ingress-authority-xact-lock — FORTRESS Slice 15H2\n');
...
ok('contract slice 15H2 + B02',
  contract.slice === 'FORTRESS-15H2'
  && contract.boundary_id === 'B02_meta_normalize_live_client_slug'
  && contract.outcome_id === '15H2_meta_ingress_authority_xact_lock'
...
ok('overlay remediated B02 historical untouched',
  ...
  && overlay.supersedes_deferred_15h === true);
ok('findings cite B02 + default-off + activation gap + replay/error rules + xact lock',
  /B02/.test(findings)
  ...
  && /pg_advisory_xact_lock/.test(findings)
...
ok('claim SQL uses transactional xact lock (no session advisory lock/unlock)', (() => {
  const sqlSrc = fs.readFileSync(
    path.join(ROOT, 'scripts', 'lib', 'luna-guest-message-events-sql.js'),
    'utf8',
  );
  const claimBlock = sqlSrc.match(
    /async function claimGuestMessageEventInboundByWaMessageId[\s\S]{0,4000}/,
  );
  const block = claimBlock ? claimBlock[0] : '';
  const sessionLockHits = (block.match(/pg_advisory_lock\s*\(/g) || [])
    .filter((m) => !/xact/i.test(m));
  const sessionUnlockHits = block.match(/pg_advisory_unlock\s*\(/g) || [];
  return /pg_advisory_xact_lock\(hashtext\(\$1\),\s*hashtext\(\$2\)\)/.test(sqlSrc)
    && /claimGuestMessageEventInboundByWaMessageId/.test(sqlSrc)
    && /BEGIN/i.test(block)
    && /COMMIT/i.test(block)
    && /ROLLBACK/i.test(block)
    && sqlSrc.includes(WA_MESSAGE_ID_LOCK_NAMESPACE)
    && sessionLockHits.length === 0
    && sessionUnlockHits.length === 0;
})());
ok('staff Meta POST uses entry before withPgClient + structured error audit', (() => {
  ...
  return /processMetaWhatsAppWebhookPostEntry/.test(block)
    && /FORTRESS 15H2/.test(block)
    && /effective_normalized/.test(block)
    && !/await withPgClient\(\s*\(?\s*pg\s*\)?\s*=>\s*processMetaWhatsAppWebhookInbound/.test(block);
})());
ok('deferred 15H tip not an ancestor of this branch', (() => {
  const { execSync } = require('child_process');
  try {
    execSync('git merge-base --is-ancestor 72fb0413c5dcbd9fd0542a991f53dca1cbc3b0cc HEAD', {
      cwd: ROOT,
      stdio: 'pipe',
    });
    return false;
...
ok('runbook documents 15H2 policy default-off + activation gap',
  /FORTRESS 15H2|FORTRESS Slice 15H2/.test(runbook)
...
green('real_builders_propagate_location_and_idempotency', (() => {
  const body = buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
    wa_message_id: 'wamid.SAMPLE_15H2_BUILDERS',
  });
  const normalized = normalizeMetaWhatsAppWebhook(
    body,
    routingOpts({ [AUTHORITY_ENV_KEY]: '1' }),
  ...
  const expectedKey = 'luna:sunset:sunset-somo:wamid.SAMPLE_15H2_BUILDERS:ask_missing_field';
...
green('legacy_idempotency_key_unchanged_without_location', (() => {
  const body = buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
    wa_message_id: 'wamid.SAMPLE_15H2_LEGACY_KEY',
  });
  const normalized = normalizeMetaWhatsAppWebhook(body, routingOpts({}));
  const draft = sampleEligibleDraft();
  const sendKind = resolveMetaWebhookSendKind(draft.next_action);
  const draftInput = buildDraftInputFromNormalized(normalized);
  const sendBody = buildMetaWebhookSendBody(normalized, draft, sendKind);
  const legacyKey = `luna:${DEFAULT_CLIENT_SLUG}:wamid.SAMPLE_15H2_LEGACY_KEY:ask_missing_field`;
...
async function runEntryAndZeroDownstream() {
  const body = buildFakeMetaWhatsAppBody('UNKNOWN_META_PHONE_NUMBER_ID_SAMPLE', {
    wa_message_id: 'wamid.SAMPLE_15H2_BLOCK',
  });
  const normalized = normalizeMetaWhatsAppWebhook(
    body,
    routingOpts({ [AUTHORITY_ENV_KEY]: '1' }),
  ...
  const missingBody = buildFakeMetaWhatsAppBody(undefined, {
    wa_message_id: 'wamid.SAMPLE_15H2_MISSING',
  });
...

  {
    const conflictNormalized = applyMetaWhatsAppIngressAuthority({
      client_slug: DEFAULT_CLIENT_SLUG,
      phone_number_id: 'WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE',
      wa_message_id: 'wamid.SAMPLE_15H2_CONFLICT',
      from: '34600000001',
      tenant_channel_shadow: {
        routing_config_enabled: true,
        channel_resolution_blocked: false,
  ...
  const guestHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_KNOWN',
      text: 'Hola, quiero reservar',
    }),
    env: { [AUTHORITY_ENV_KEY]: '1' },
  });
  const expectedKnownKeyPrefix = 'luna:sunset:sunset-somo:wamid.SAMPLE_15H2_KNOWN:';
  ...
  const offHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_OFF',
      text: 'Hola',
    }),
    env: {},
  });
  ...
  const absentHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_ABSENT',
      text: 'Hola',
    }),
    env: { [AUTHORITY_ENV_KEY]: '1' },
  ...
  const conflictHarness = makeHarnessPg({
    events: [{
      ...
      wa_message_id: 'wamid.SAMPLE_15H2_REPLAY_CONFLICT',
      channel: 'whatsapp',
      direction: 'inbound',
  ...
  const locConflictHarness = makeHarnessPg({
    events: [{
      id: 'evt-loc-1',
      client_slug: 'sunset',
      wa_message_id: 'wamid.SAMPLE_15H2_REPLAY_LOC',
      channel: 'whatsapp',
      direction: 'inbound',
      from_phone: '34600000001',
      to_phone_number_id: 'WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE',
  ...
  const fillHarness = makeHarnessPg({
    events: [{
      id: 'evt-fill-1',
      client_slug: 'sunset',
      wa_message_id: 'wamid.SAMPLE_15H2_REPLAY_FILL',
      channel: 'whatsapp',
      direction: 'inbound',
      from_phone: '34600000001',
      to_phone_number_id: 'WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE',
      ...
      handoff_required: false,
      send_attempted: true,
      send_idempotency_key: 'luna:sunset:wamid.SAMPLE_15H2_REPLAY_FILL:ask_missing_field',
      send_status: 'blocked',
  ...
  const errHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_ERR',
      text: 'Hola',
    }),
    env: { [AUTHORITY_ENV_KEY]: '1' },
  ...
  const demoHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_DEMO',
  ...
  const ownerHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_OWNER',
  ...
  const gateHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_WOLFHOUSE_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_GATE',
      text: 'Hola',
      from: '34999888777',
    }),
    env: {
  ...
  const missingTableHttp = await runHttpWithPgHarness({
    body: buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
      wa_message_id: 'wamid.SAMPLE_15H2_NOTABLE',
      text: 'Hola',
    }),
    env: { [AUTHORITY_ENV_KEY]: '1' },
  ...
  const terminalBody = buildFakeMetaWhatsAppBody('WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE', {
    wa_message_id: 'wamid.SAMPLE_15H2_TERMINAL',
    text: 'Hola',
  });
  ...
  const unprocessedHarness = makeHarnessPg({
    events: [{
      id: 'evt-unproc-1',
      client_slug: 'sunset',
      wa_message_id: 'wamid.SAMPLE_15H2_UNPROCESSED',
      channel: 'whatsapp',
      direction: 'inbound',
      from_phone: '34600000001',
      to_phone_number_id: 'WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE',
    ...
    events: [
      {
        id: 'a1',
        client_slug: 'wolfhouse-somo',
        wa_message_id: 'wamid.SAMPLE_15H2_AMBIG',
  ...
  // Concurrent cross-slug claim: two clients share event store + xact lock wait graph.
  console.log('\n── Concurrency / isolation proofs ──');
  const concurShared = {
    events: [],
    xactLockWaiters: new Map(),
    xactHeldLocks: new Map(),
    idSeq: 1000,
  };
  const concurA = makeHarnessPg({ shared: concurShared });
  const concurB = makeHarnessPg({ shared: concurShared });
  const concurWamid = 'wamid.SAMPLE_15H2_CONCUR';
  ...
  const [claimA, claimB] = await Promise.all([
    claimGuestMessageEventInboundByWaMessageId(concurA, seedSunset),
    claimGuestMessageEventInboundByWaMessageId(concurB, seedWolf),
  ]);
  const concurInserted = [claimA, claimB].filter((c) => c.inserted === true).length;
  const concurBegins = concurA.effects.begins + concurB.effects.begins;
  const concurCommits = concurA.effects.commits + concurB.effects.commits;
  const concurXactLocks = concurA.effects.advisory_xact_locks + concurB.effects.advisory_xact_locks;
  const concurInserts = concurA.effects.inserts + concurB.effects.inserts;
  const concurSessionLocks = concurA.effects.advisory_locks + concurB.effects.advisory_locks;
  const concurSessionUnlocks = concurA.effects.advisory_unlocks + concurB.effects.advisory_unlocks;
  ok('concurrent cross-slug claims cannot both insert',
    concurInserted === 1
    && concurInserts === 1
    && concurShared.events.length === 1
    && concurBegins >= 2
    && concurCommits >= 2
    && concurXactLocks >= 2
    && concurSessionLocks === 0
    && concurSessionUnlocks === 0
    && claimA.rows.length === 1
    && claimB.rows.length === 1
    && claimA.rows[0].id === claimB.rows[0].id
    && concurShared.events[0].wa_message_id === concurWamid);

  ok('transaction failure mid-claim rolls back without partial commit', await (async () => {
    const failHarness = makeHarnessPg({
      throwOnQuery: (sql, _params, fx) => {
        if (/\bINSERT\s+INTO\s+guest_message_events\b/i.test(sql) && fx.begins >= 1) {
          return Object.assign(new Error('forced_insert_failure'), { code: 'XX000' });
        }
        return null;
      },
    });
    const failSeed = buildInboundEventSeed({
      client_slug: 'sunset',
      location_id: 'sunset-somo',
      wa_message_id: 'wamid.SAMPLE_15H2_TX_FAIL',
      from: '34600000001',
      phone_number_id: 'WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE',
      message_type: 'text',
      message_text: 'fail',
    ...
    try {
      await claimGuestMessageEventInboundByWaMessageId(failHarness, failSeed);
    } catch (err) {
      insertThrew = /forced_insert_failure/.test(String(err && err.message));
    }
    const insertOk = insertThrew
      && failHarness.effects.begins >= 1
      && failHarness.effects.rollbacks >= 1
      && failHarness.effects.commits === 0
      && failHarness.effects.inserts === 0
      && failHarness.events.length === 0
      && failHarness.effects.advisory_unlocks === 0;

    const lookupHarness = makeHarnessPg({
      throwOnQuery: (sql, _params, fx) => {
        if (
          /FROM\s+guest_message_events/i.test(sql)
          && /wa_message_id/i.test(sql)
          && /^\s*SELECT/i.test(sql)
          && fx.begins >= 1
        ) {
          return Object.assign(new Error('forced_lookup_failure'), { code: 'XX000' });
        }
        return null;
      },
    });
    const lookupSeed = buildInboundEventSeed({
      client_slug: 'sunset',
      location_id: 'sunset-somo',
      wa_message_id: 'wamid.SAMPLE_15H2_TX_LOOKUP_FAIL',
      from: '34600000001',
      phone_number_id: 'WHATSAPP_PHONE_NUMBER_ID_SUNSET_SOMO_SAMPLE',
      message_type: 'text',
      message_text: 'lookup fail',
    }, { object: 'whatsapp_business_account' });
    let lookupThrew = false;
    try {
      await claimGuestMessageEventInboundByWaMessageId(lookupHarness, lookupSeed);
    } catch (err) {
      lookupThrew = /forced_lookup_failure/.test(String(err && err.message));
    }
    const lookupOk = lookupThrew
      && lookupHarness.effects.begins >= 1
      && lookupHarness.effects.rollbacks >= 1
      && lookupHarness.effects.commits === 0
      && lookupHarness.effects.inserts === 0
      && lookupHarness.events.length === 0;

    return insertOk && lookupOk;
  })());

  ok('SQL helpers use xact lock transaction (no session lock, no UNIQUE wa_message_id required)', (() => {
    const sqlSrc = fs.readFileSync(
      path.join(ROOT, 'scripts', 'lib', 'luna-guest-message-events-sql.js'),
      'utf8',
    );
...
ok('package.json has 15H2 verify script',
  pkg.scripts['verify:fortress-slice15h2-meta-ingress-authority-xact-lock']
  === 'node scripts/verify-fortress-slice15h2-meta-ingress-authority-xact-lock.js');
...
runEntryAndZeroDownstream().then(() => {
  ...
  ok('evidence slice + activation gap + verifier script',
    committedEvidence.slice === 'FORTRESS-15H2'
    ...
    && committedEvidence.verifier_script
      === 'scripts/verify-fortress-slice15h2-meta-ingress-authority-xact-lock.js'
    && /pg_advisory_xact_lock|xact_lock/i.test(
      JSON.stringify(committedEvidence.real_path_proof || {}),
    ));
  ...
  console.log('OK fortress-slice15h2-meta-ingress-authority-xact-lock');
}).catch((err) => {
  console.error(err);
  process.exit(1);
});
