...
(async () => {
  ...
  const deps={runtimeEnv:env,sendJSON(_r,s,b){replies.push([s,b]);},sendHTML(_r,s,b){replies.push([s,b]);},assertStaffClientAccess(){return true;},authorizeAuthenticatedStaffRoute(x){assert.equal(x.clientSlug,'sunset');return {ok:true};},withPgClient(fn){return fn({query(sql,p){queryArgs=[sql,p];return {rows:[{client_id:U[0],location_id:U[1],endpoint_id:U[2]}]};}});},createStart(pg){assert.equal(typeof pg.query,'function');return {start(v){startInput=v;return {authorizationUrl:'https://accounts.google.com/auth',expiresAt:'2026-08-12T00:10:00.000Z'};}};},createCallbackRuntime(pg,stageTelemetry){assert.equal(typeof pg.query,'function');assert.equal(arguments.length,2,'runtime authority must come from consumed state plus server-owned telemetry');assert.ok(Object.isFrozen(stageTelemetry));assert.deepEqual(Reflect.ownKeys(stageTelemetry),['emit']);runtimeClientId='state-first';return {completeCallback(v){callbackCalls.push(v);return typeof nextOutput === 'function' ? nextOutput() : nextOutput;}};}};
