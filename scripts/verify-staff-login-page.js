...
assert('favicon asset exists', fs.existsSync(path.join(ROOT, 'config', 'staff-portal', 'luna-favicon.png')) && fs.statSync(path.join(ROOT, 'config', 'staff-portal', 'luna-favicon.png')).size > 500);
assert('login HTML has favicon link', wolfhouseHtml.includes('/staff/assets/luna-favicon.png') && wolfhouseHtml.includes('rel="icon"'));
assert('login HTML has apple-touch-icon', wolfhouseHtml.includes('apple-touch-icon'));
assert('API serves favicon route', apiSrc.includes('/staff/assets/luna-favicon.png') && apiSrc.includes('handleStaffPortalFavicon'));
assert('API serves root /favicon.ico', apiSrc.includes("pathname === '/favicon.ico'") || apiSrc.includes('pathname === \'/favicon.ico\''));
assert('UI HTML head includes favicon', apiSrc.includes('buildUiHtml') && /luna-favicon\.png\?v=1/.test(apiSrc));
