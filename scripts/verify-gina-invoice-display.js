...
check('P5', guest.includes('>Payment Link<'), 'button label');
...
check('M3c', /id="bc-generate-payment-link-btn"[^>]*>Payment Link<\//.test(html), 'overview balance control uses the same Payment Link copy as each guest');
check('L1', apiSrc.includes('#bc-overview-invoice,#bc-running-invoice{overflow:hidden')
  && apiSrc.includes('.bc-inline-payment-link{display:inline-flex')
  && apiSrc.includes('.bc-balance-link-slot{'), 'invoice card clips link rows and the masked link cannot grow past the card');
...
check('I1', i18n.includes("'drawer.invoice.createLink': 'Payment Link'"), 'EN Payment Link');
check('I1a', i18n.includes("'drawer.invoice.copyLink': 'Copy'"), 'EN copy aria label');
check('I2', i18n.includes("'drawer.invoice.unnamedGuest': 'Unnamed guest {number}'"), 'EN unnamed guest');
check('I3', es.includes('"drawer.invoice.createLink": "Enlace de pago"'), 'ES payment link differs from EN');
check('I3a', es.includes('"drawer.invoice.copyLink": "Copiar"'), 'ES copy aria label');
check('I4', i18n.includes("'drawer.invoice.createLink': 'Link di pagamento'"), 'IT payment link differs from EN');
let unknown = null;
...
async function verifyInlinePaymentLinkResults() {
  ...
  check('UX1', perGuestBtn.outerHTML.includes('<a href="https://pay.example/guest-inline"')
    && perGuestBtn.outerHTML.includes('>Payment Link</a>')
    && !perGuestBtn.outerHTML.includes('>https://pay.example/guest-inline<'), 'per-guest link is masked; full URL stays in href only');
  check('UX1a', perGuestBtn.outerHTML.includes('btn-bc-copy-link-icon')
    && perGuestBtn.outerHTML.includes('aria-label="Copy"')
    && perGuestBtn.outerHTML.includes('bc-copy-link-svg')
    && !perGuestBtn.outerHTML.includes('>Copy<'), 'per-guest Copy is an icon with aria-label Copy');
  check('UX1b', perGuestBtn.outerHTML.includes('data-url="https://pay.example/guest-inline"')
    && perGuestBtn.outerHTML.includes('btn-bc-copy-link-icon')
    && !perGuestBtn.outerHTML.includes('bc-create-guest-payment-link-btn'), 'per-guest success replaces its Payment Link control in place');
  ...
  check('UX7', balanceResult.innerHTML.includes('<a href="https://pay.example/balance-inline"')
    && balanceResult.innerHTML.includes('>Payment Link</a>')
    && !balanceResult.innerHTML.includes('>https://pay.example/balance-inline<'), 'balance link is masked; full URL stays in href only');
  check('UX7a', balanceResult.innerHTML.includes('btn-bc-copy-link-icon')
    && balanceResult.innerHTML.includes('aria-label="Copy"')
    && balanceResult.innerHTML.includes('bc-copy-link-svg')
    && !balanceResult.innerHTML.includes('>Copy<'), 'balance Copy is an icon with aria-label Copy');
