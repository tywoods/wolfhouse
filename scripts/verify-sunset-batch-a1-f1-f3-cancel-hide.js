...
ok('drawer cancelled offers Restore + Hide', /ps-drawer-restore-booking/.test(drawerView) && /hideBooking|Hide booking/.test(drawerView));
ok('bookings hide/unhide controls', /data-bookings-hide/.test(bookingsUi) && /data-bookings-unhide/.test(bookingsUi));
ok('bookings restore control', /data-bookings-restore/.test(bookingsUi));
ok('bookings refund needs cancel i18n key wired', /refundNeedsCancel/.test(bookingsUi) || /refundNeedsCancel/.test(fs.readFileSync(require('path').join(__dirname, 'lib/staff-portal-i18n.js'), 'utf8')));
