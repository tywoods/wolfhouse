...
function createController(deps = {}) {
  ...
  function rootContract(candidateFile = caddy) { const root = f.readFileSync(rootCaddy, 'utf8'); if (!/^[a-f0-9]{64}$/.test(String(expectedRootHash || '')) || digest(root) !== expectedRootHash) throw Object.assign(new Error('root Caddyfile drift'), { code: 'root_caddy_contract_failed' }); const importLine = `import ${caddy}`; if (root.split(/\r?\n/).filter((x) => x.trim() === importLine).length !== 1 || root.indexOf(importLine) > root.search(/reverse_proxy\s+\/whatsapp\/\*/)) throw Object.assign(new Error('root import missing or unordered'), { code: 'root_caddy_contract_failed' }); return root.replace(importLine, `import ${candidateFile}`); }
  async function state() { rootContract(); const text = f.readFileSync(caddy, 'utf8'); const route = parseRoute(text); const effective = await effectiveRoute(fetchImpl); if (!route || !effective || route.target_luna !== effective.target_luna) throw Object.assign(new Error(), { code: 'route_readback_failed' }); return route; }
  async function mutate(body) {
    ...
    try {
      ...
      if (!j) {
        rootContract(); const beforeText = f.readFileSync(caddy, 'utf8'); const before = parseRoute(beforeText); const beforeEffective = await effectiveRoute(fetchImpl);
        ...
        const validationFile = path.join(stateDir, `validation-${body.operation_id}.Caddyfile`); try { atomicWrite(f, validationFile, rootContract(candidateFile), 0o600); await run(caddyBin, ['validate', '--config', validationFile, '--adapter', 'caddyfile']); } catch (_) { try { f.unlinkSync(candidateFile); } catch (__){ } return { status: 422, body: { ok: false, code: 'candidate_validation_failed' } }; } finally { try { f.unlinkSync(validationFile); fsyncDir(f, validationFile); } catch (_) {} }
  ...
  async function rollback(journal) {
    ...
    try { const validationFile = path.join(stateDir, `rollback-${journal.operation_id}.Caddyfile`); atomicWrite(f, validationFile, rootContract(), 0o600); try { await run(caddyBin, ['validate', '--config', validationFile, '--adapter', 'caddyfile']); } finally { try { f.unlinkSync(validationFile); fsyncDir(f, validationFile); } catch (_) {} } await run('/usr/bin/sudo', ['-n', '/usr/bin/systemctl', 'reload', 'caddy']); const after = await effectiveRoute(fetchImpl); if (!after || after.target_luna !== journal.original_target) throw new Error(); saveJournal(journal, 'rolled_back'); checkpoint('after-rollback-before-final-ledger'); return { status: 503, body: { ok: false, code: 'mutation_rolled_back', events: [{ type: 'rollback', details: { restored: true } }] } };
