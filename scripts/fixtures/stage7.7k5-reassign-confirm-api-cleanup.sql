-- Stage 7.7k5 fixture cleanup — safe to run multiple times
...
BEGIN
  ...
  WHERE client_id = v_client_id AND booking_code = 'WH-77K5-REASSIGN-001' LIMIT 1;
  ...
  DELETE FROM bookings WHERE client_id = v_client_id AND booking_code = 'WH-77K5-REASSIGN-001';
  GET DIAGNOSTICS v_del_bk = ROW_COUNT;

  RAISE NOTICE 'Stage 7.7k5 cleanup: booking_beds=%, workflow_events=%, bookings=%',
