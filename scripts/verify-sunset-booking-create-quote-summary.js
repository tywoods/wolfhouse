...
assert('owners', /function schedulePortalRenderCreateIntentSummary/.test(portalSrc)
  ...
  && /function schedulePortalSyncCreateFooter/.test(portalSrc) && /function schedulePortalInvalidateCreateQuoteIntent/.test(portalSrc)
...
function sandbox(opts) {
  opts = opts || {};
  const log = [], nodes = {}; let summaryN = 0, refreshN = 0, qn = 0;
  function N(id, x) {
    nodes[id] = Object.assign({ id, value:'', checked:false, disabled:false, textContent:'', innerHTML:'', style:{display:'none'},
      ...
      querySelector(){ return null; }, querySelectorAll(){ return []; } }, x || {});
    return nodes[id];
  }
  ...
  const ctx = {
    ...
    fetch(url, req) {
      ...
      if (String(url).includes('/bookings/quote')) {
        const n = ++qn, delay = typeof opts.quoteDelay === 'function' ? opts.quoteDelay(n) : (opts.quoteDelay || 0);
        const custom = typeof opts.quoteOutcome === 'function' ? opts.quoteOutcome(n, entry) : null, uncoop = !!opts.uncooperative;
...
(async function run() {
  ...
  const badBody=(cents)=>({ok:true,body:cents===undefined?{success:true,quote_provenance:{source:'x'}}:{success:true,total_cents:cents,quote_provenance:{source:'x'}}});
  const creates=(c)=>c._log.filter((e)=>String(e.url).includes('/bookings?')&&e.opts&&e.opts.method==='POST');
  const hasProv=(c)=>c._log.some((e)=>e.body&&e.body.quote_provenance);
  for (const row of [['neg',-500],['frac',10.5],['null',null],['miss'],['empty',''],['ws',' '],['false',false],['str','13500'],['nan',NaN],['inf',Infinity]]) {
    const b=sandbox({ debounceMs:5, quoteDelay:5, quoteOutcome:()=>badBody(row[0]==='miss'?undefined:row[1]) });
    b._setPayload(baseCourse()); await b.schedulePortalRunPreviewQuote();
    assert(row[0]+' total', /Quote unavailable/.test(Q(b)) && !/€\d/.test(Q(b)) && b.schedulePortalQuoteState==null);
  }
  const z=sandbox({ debounceMs:5, quoteDelay:5, quoteOutcome:()=>({ok:true,body:{success:true,total_cents:0,quote_provenance:{source:'z',quote_fingerprint:'fpz'}}}) });
  z._setPayload(baseCourse()); await z.schedulePortalRunPreviewQuote();
  assert('zero total', /€0\.00/.test(Q(z)) && z.schedulePortalQuoteState && z.schedulePortalQuoteState.total_cents===0
    && z.schedulePortalQuoteState.quote_provenance && z.schedulePortalQuoteState.quote_provenance.source==='z');
  for (const row of [['sub-str','13500'],['sub-null',null],['sub-frac',10.5]]) {
    const c=sandbox({ debounceMs:5, quoteDelay:5, quoteOutcome:()=>badBody(row[1]) });
    c._setPayload(baseCourse({ guest_name:'Sub' })); const k0=c.schedulePortalEnsureIdempotencyKey(baseCourse({ guest_name:'Sub' }));
    c.submitScheduleManualBooking(); await sleep(50);
    assert(row[0], c._qn()>=1 && creates(c).length===0 && !hasProv(c) && c.schedulePortalQuoteState==null && !c.schedulePortalSubmitInFlight && c.schedulePortalSubmitIdemKey===k0);
  }
  const errS = sandbox({ debounceMs:5, quoteDelay:5, quoteOutcome:(n)=>n===1?{ok:false,status:409,body:{success:false,reason_code:'quote_stale'}}
    :n===2?{ok:false,status:503,body:{success:false}}:n===3?{ok:false,status:500,body:{success:false}}:{ok:true,body:{success:true,total_cents:5000}} });
  ...
  guest._resetCounts(); guest.el('ps-create-guest').value='Ann'; (guest.el('ps-create-guest')._ls.input||[]).forEach((fn)=>fn());
  ...
  assert('shared strict owner', (portalSrc.match(/function schedulePortalStrictQuoteTotalCents/g)||[]).length===1
    && /schedulePortalStrictQuoteTotalCents\(data\)/.test(extractFn(portalSrc,'schedulePortalFetchQuote')||'')
    && /schedulePortalStrictQuoteTotalCents\(result\.body\)/.test(extractFn(portalSrc,'schedulePortalRenderCreateQuotePreview')||''));
  ...
  const softH = portalSrc.replace(/function schedulePortalStrictQuoteTotalCents\(body\) \{[\s\S]*?\n\}/,
    'function schedulePortalStrictQuoteTotalCents(body){ return body&&Object.prototype.hasOwnProperty.call(body,"total_cents")?body.total_cents:null; }');
  const rMoney = sandMut(softH,{ quoteDelay:5, quoteOutcome:()=>({ok:true,body:{success:true,total_cents:-500}}) });
  await rMoney.schedulePortalRunPreviewQuote();
  const moneySrc=(extractFn(portalSrc,'schedulePortalRenderCreateQuotePreview')||'')+(extractFn(portalSrc,'schedulePortalFetchQuote')||'')+(extractFn(portalSrc,'schedulePortalStrictQuoteTotalCents')||'');
  assert('RED money+no sum', /€-5\.00/.test(Q(rMoney)) && /total_cents/.test(moneySrc) && /schedulePortalStrictQuoteTotalCents/.test(moneySrc) && !/amount_cents|rentals\.reduce|sumCents/.test(moneySrc));
  const noBump = portalSrc.replace(/schedulePortalQuoteAbort = null;\n  \}\n  schedulePortalQuoteGen \+= 1;\n  schedulePortalShowQuoteChecking/,'schedulePortalQuoteAbort = null;\n  }\n  schedulePortalShowQuoteChecking');
  ...
  const coerce = portalSrc.replace(/function schedulePortalStrictQuoteTotalCents\(body\) \{[\s\S]*?\n\}/,
    'function schedulePortalStrictQuoteTotalCents(body){ var raw=body&&body.total_cents; var t=typeof raw==="number"?raw:Number(raw); if(!Number.isFinite(t)||Math.floor(t)!==t||t<0||t>Number.MAX_SAFE_INTEGER)return null; return t; }');
  ...
  const noFetchGate = portalSrc.replace(
    ...
    'if (applyState && myGen === schedulePortalQuoteGen) {\n      schedulePortalQuoteState = { quote_provenance: data.quote_provenance || null, total_cents: data.total_cents != null ? Number(data.total_cents) : null, fetched_at: Date.now(), gen: myGen };\n    }');
  assert('RED fetch mut', noFetchGate !== portalSrc);
  const rLeak = sandMut(noFetchGate,{ quoteDelay:5, quoteOutcome:()=>badBody(null) });
  rLeak._setPayload(baseCourse({ guest_name:'Leak' })); rLeak.submitScheduleManualBooking(); await sleep(50);
  const leakC = creates(rLeak);
  assert('RED fetch leak', leakC.length===1 && leakC[0].body && leakC[0].body.quote_provenance && leakC[0].body.quote_provenance.source==='x');
