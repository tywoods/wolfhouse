...
const T = {
  ...
  'schedule.create.summary.sessions': 'Sessions', 'schedule.create.summary.surfers': 'Surfers',
  ...
  'admin.period.2_days': '2 day', 'admin.period.1_day': '1 day',
};
...
function sandbox(opts) {
  ...
  const cOpts = opts.courseOptions !== undefined ? opts.courseOptions
    : [{ value: 'c1', textContent: 'Beginner', getAttribute: (k) => (k === 'data-label' ? 'Beginner' : null) }];
...
function baseCourse(extra) {
  return Object.assign({ guest_name: 'R', date_from: '2026-08-20', date_to: '2026-08-20', payment_status: 'unpaid',
    components: { course: { course_id: 'c1', course_label: 'G', quantity: 1 } }, rentals: [] }, extra || {});
}
(async function run() {
  ...
  leak.schedulePortalRenderCreateIntentSummary();
  assert('no key leak', /Group course/.test(S(leak)) && !LEAK.test(S(leak)));
  const priv = sandbox();
  priv._setPayload({ guest_name: '', guest_phone: 'X', notes: 'N', date_from: '2026-08-21', date_to: '2026-08-23', payment_status: 'unpaid',
    components: { private_lesson: { enabled: true, quantity: 2, surfer_count: 3, sessions: [
  ...
  await race([60, 5], 'race B-then-A');
  await race([5, 40], 'race A-then-B');
  const uncoop = sandbox({ debounceMs: 5, uncooperative: true, quoteDelay: (n) => (n === 1 ? 50 : 5),
    quoteOutcome: (n) => ({ ok: true, body: { success: true, total_cents: n === 1 ? 11100 : 22200 } }) });
  ...
  for (const [name, cents, bad] of [['neg', -500, /€-5|Quoted total:\s*—/], ['frac', 10.5, /€0\.10/]]) {
    const b = sandbox({ debounceMs: 5, quoteDelay: 5, quoteOutcome: () => ({ ok: true, body: { success: true, total_cents: cents } }) });
    b._setPayload(baseCourse()); await b.schedulePortalRunPreviewQuote();
    assert(name + ' total', /Quote unavailable/.test(Q(b)) && !bad.test(Q(b)));
  }
  ...
  assert('guest only', guest._counts().summary === 1 && guest._counts().refresh === 0 && guest._qn() === 0 && /€77/.test(Q(guest)) && !/Checking price/.test(Q(guest)));
    ...
    o = o || {}; o.portalSrc = src; o.payload = o.payload || baseCourse();
    const c = sandbox(o); return c;
  }
  ...
  const rGuard = sandMut(stripGen, { quoteDelay: (n) => (n === 1 ? 50 : 5), quoteOutcome: (n) => ({ ok: true, body: { success: true, total_cents: n === 1 ? 11100 : 22200 } }) });
  ...
  assert('RED stale', /€99\.00/.test(Q(rStale)) && !/Checking price/.test(Q(rStale)));
  ...
  rKey._setPayload({ guest_name: '', date_from: '2026-08-20', date_to: '2026-08-20', payment_status: 'unpaid',
    components: { course: { course_id: 'c1', tier_key: '1_week', quantity: 1 } }, rentals: [] });
  rKey.schedulePortalRenderCreateIntentSummary();
  assert('RED raw keys', /c1|1_week/.test(S(rKey)));
  const stripCanon = portalSrc.replace(
    /if \(!Number\.isFinite\(total\) \|\| Math\.floor\(total\) !== total \|\| total < 0 \|\| total > Number\.MAX_SAFE_INTEGER\) \{[\s\S]*?return;\n  \}/,
    'if (false) { return; }');
  ...
  assert('RED money+no sum', /€-5\.00/.test(Q(rMoney)) && /total_cents/.test(extractFn(portalSrc, 'schedulePortalRenderCreateQuotePreview') || '')
    && !/amount_cents|rentals\.reduce|sumCents/.test(extractFn(portalSrc, 'schedulePortalRenderCreateQuotePreview') || ''));
