 ...
 * Offline hostile When private multi-session editor. No network/DB/deploy.
 * Run: node scripts/verify-sunset-booking-create-when-sessions.js
 */
const fs = require('fs');
...
console.log('[1] DOM story order + unique IDs');
assert('sessions in When', when.includes('id="ps-create-private-lesson-sessions"') && when.includes('id="ps-create-add-session"') && when.includes('id="ps-create-private-lesson-qty"'));
assert('sessions not in What', !what.includes('id="ps-create-private-lesson-sessions"') && !what.includes('id="ps-create-add-session"'));
assert('surfers in What', what.includes('id="ps-create-private-lesson-surfers"') && what.includes('id="ps-create-private-lesson-fields"'));
assert('date-range in When', when.includes('id="ps-create-date-range"'));
const counts = {}; let m; const re = /\bid="(ps-create-[^"]+)"/g;
while ((m = re.exec(modal))) counts[m[1]] = (counts[m[1]] || 0) + 1;
assert('no duplicate ps-create ids', Object.keys(counts).filter((k) => counts[k] > 1).length === 0);
['ps-create-private-lesson-sessions', 'ps-create-add-session', 'ps-create-private-lesson-qty', 'ps-create-private-lesson-surfers', 'ps-create-date-from'].forEach((id) => assert(id + ' once', counts[id] === 1));
...
[
  ...
  assert('EN ' + k, typeof en[k] === 'string' && en[k].trim());
  assert('ES ' + k, typeof es[k] === 'string' && es[k].trim() && es[k] !== en[k] && es[k] !== k);
  assert('IT ' + k, typeof it[k] === 'string' && it[k].trim() && it[k] !== en[k] && it[k] !== k);
});
...
function build(opts) {
  ...
  function radio(id, name, checked) {
    ...
    Object.defineProperty(node, 'checked', {
      ...
      set(v) { this._checked = !!v; if (v) Object.keys(nodes).forEach((k) => { const n = nodes[k]; if (n && n.name === name && n !== this) n._checked = false; }); },
    });
    nodes[id] = node;
  }
  ...
  ['ps-create-course-fields', 'ps-create-course-tier-wrap', 'ps-create-course-qty-wrap', 'ps-create-private-lesson-fields', 'ps-create-addon-fullday-field', 'ps-create-fullday-card', 'ps-create-fullday-rows', 'ps-create-fullday-summary', 'ps-create-fullday-price-hint', 'ps-create-activity-empty-hint', 'ps-create-rentals', 'ps-create-msg'].forEach((id) => box(id, id.includes('hint') || id === 'ps-create-rentals' ? '' : 'none'));
  const privateWhen = listen({ classList: cl('portal-schedule-create-private-when'), style: { display: 'none' }, dataset: {} });
  const dateRange = listen({ id: 'ps-create-date-range', style: { display: '' }, dataset: {}, classList: cl(), nextElementSibling: privateWhen });
  nodes['ps-create-date-range'] = dateRange;
  input('ps-create-date-from', opts.dateFrom || '2026-07-20');
  input('ps-create-date-to', opts.dateTo || '2026-07-22');
  ...
  function parseRows(html, wrap) {
    const rows = [];
    const parts = String(html).split('portal-schedule-private-session-row');
    for (let i = 1; i < parts.length; i += 1) {
      ...
      const removeBtn = hasRemove ? listen({
        classList: cl('portal-schedule-session-remove'),
        getAttribute(k) {
          if (k === 'data-session-remove') return String(removeIdx != null ? removeIdx : rows.length);
          const mm = chunk.match(new RegExp(k + '="([^"]*)"'));
          return mm ? mm[1] : null;
        },
      }) : null;
      const row = listen({
        classList: cl('portal-schedule-private-session-row'), dataset: { wired: '' },
        _date: dateEl, _start: startEl, _end: endEl, _removeBtn: removeBtn, parentNode: wrap,
        querySelector(sel) {
          if (sel === '.ps-pl-session-date') return dateEl;
  ...
  const sb = {
    ...
    scheduleUpdateCreateTotalPreview() { quote.n += 1; },
    ...
    _nodes: nodes, _quote: quote, _privateWhen: privateWhen, _gear: opts.rentals || [],
  };
  const names = [
    'schedulePrivateLessonDefaultEnd', 'scheduleReadPrivateLessonSessionsFromDom', 'scheduleWirePrivateLessonSessionRow',
    'scheduleSyncPrivateLessonSessions', 'scheduleUpdatePrivateLessonDateRangeFromSessions',
    'scheduleAddPrivateLessonSession', 'scheduleRemovePrivateLessonSession',
  ...
  const code = names.map((n) => extractFn(portalSrc, n) || extractFn(apiSrc, n)).filter(Boolean);
  assert('fns extractable', code.length >= 12, String(code.length));
  vm.createContext(sb);
...
function pick(sb, id) { const n = sb.el(id); n.checked = true; sb.scheduleOnCreateComponentChange(id); }
...
console.log('\n[5] Count sync + month boundary');
{
  ...
  setRow(sb, 1, { date: '2026-07-21', start: '10:00', end: '11:30' });
  setRow(sb, 2, { date: '2026-07-22', start: '14:00', end: '15:30' });
  const snap = rows(sb);
  sb.el('ps-create-private-lesson-qty').value = '2'; sb.scheduleSyncPrivateLessonSessions();
  const after = rows(sb);
  ...
  setRow(sb, 1, { date: '2026-12-31', start: '10:00', end: '12:00' });
  sb.scheduleAddPrivateLessonSession();
  assert('Dec31+1 → 2027-01-01', rows(sb).length === 3 && rows(sb)[2].date === '2027-01-01');
}

console.log('\n[6] Outer min/max + payload order (no silent drop)');
{
  const sb = build();
  pick(sb, 'ps-create-comp-private-lesson');
  sb.el('ps-create-private-lesson-qty').value = '3'; sb.scheduleSyncPrivateLessonSessions();
  setRow(sb, 0, { date: '2026-07-22', start: '10:00', end: '12:00' });
  setRow(sb, 1, { date: '2026-07-20', start: '11:00', end: '13:00' });
  setRow(sb, 2, { date: '2026-07-21', start: '09:00', end: '11:00' });
  ...
  const sess = p.components.private_lesson.sessions;
  assert('payload keeps blank-date row in order', sess.length === 3 && sess[0].date === '2026-07-22' && sess[1].date === '' && sess[2].date === '2026-07-21' && p.components.private_lesson.quantity === 3);
}

console.log('\n[7] Validation before quote/create');
{
  const sb = build({ today: '2026-07-20' });
  assert('validate helper', typeof sb.schedulePortalValidatePrivateLessonCreate === 'function');
  const V = (pl) => sb.schedulePortalValidatePrivateLessonCreate(pl);
  assert('missing date', V({ quantity: 1, sessions: [{ date: '', start: '10:00', end: '12:00' }] }).ok === false);
  assert('missing start', V({ quantity: 1, sessions: [{ date: '2026-07-20', start: '', end: '12:00' }] }).ok === false);
  assert('end<=start', V({ quantity: 1, sessions: [{ date: '2026-07-20', start: '12:00', end: '10:00' }] }).ok === false);
  assert('past', V({ quantity: 1, sessions: [{ date: '2026-07-10', start: '10:00', end: '12:00' }] }).ok === false);
  assert('impossible', V({ quantity: 1, sessions: [{ date: '2026-02-31', start: '10:00', end: '12:00' }] }).ok === false);
  assert('duplicate', V({ quantity: 2, sessions: [{ date: '2026-07-20', start: '10:00', end: '12:00' }, { date: '2026-07-20', start: '10:00', end: '12:00' }] }).ok === false);
  assert('max', V({ quantity: 31, sessions: Array.from({ length: 31 }, () => ({ date: '2026-07-20', start: '10:00', end: '12:00' })) }).ok === false);
  assert('good', V({ quantity: 2, sessions: [{ date: '2026-07-20', start: '10:00', end: '12:00' }, { date: '2026-07-21', start: '10:00', end: '12:00' }] }).ok === true);
  assert('submit uses validate', /function submitScheduleManualBooking[\s\S]*schedulePortalValidatePrivateLessonCreate/.test(portalSrc));
}
...
console.log('\n[8] Same-open preserve + fresh open clear');
{
  ...
  pick(sb, 'ps-create-comp-private-lesson');
  sb.scheduleAddPrivateLessonSession();
  setRow(sb, 0, { date: '2026-07-22', start: '09:00', end: '11:00' });
  setRow(sb, 1, { date: '2026-07-23', start: '14:00', end: '16:00' });
  const preserved = rows(sb);
  pick(sb, 'ps-create-comp-course');
...
console.log('\n[9] One field change → one quote schedule');
{
  const sb = build();
  pick(sb, 'ps-create-comp-private-lesson');
  sb._quote.n = 0;
...
assert('payload canonical', /components\.private_lesson\s*=\s*\{[\s\S]*sessions:\s*plSessions/.test(apiSrc));
assert('date range hidden private', /dateRange\.style\.display\s*=\s*privateOn\s*\?\s*'none'/.test(apiSrc));
assert('session owner in portal', /function scheduleSyncPrivateLessonSessions/.test(portalSrc));

console.log('\n' + '─'.repeat(48));
console.log('Results: ' + pass + ' passed, ' + fail + ' failed');
if (fail > 0) { console.error('verify:sunset-booking-create-when-sessions — FAILED'); process.exit(1); }
console.log('verify:sunset-booking-create-when-sessions — ALL CHECKS PASSED');
process.exit(0);
