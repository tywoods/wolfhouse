 ...
 * (header + AsyncLocalStorage only). Completion logging is owned by 16N
 * (allowlisted staff_api_request_completed); 16J still proves header+ALS.
...
async function main() {
  ...
  try {
    ...
    const completionLines = httpConsoleSlice.filter((line) =>
      line.includes('staff_api_request_completed'));
    const leakyConsole = httpConsoleSlice.some((line) => {
      // Allowlisted 16N completion records may include request_id/duration_ms.
      if (line.includes('staff_api_request_completed')) {
    ...
    // 16N owns completion emission; 16J proves records are allowlisted (or absent).
    ...
    for (const line of completionLines) {
      ...
      try {
        ...
        if (obj.event !== 'staff_api_request_completed') { completionOk = false; break; }
        const keys = Object.keys(obj);
        const allowed = new Set([
          'event', 'request_id', 'tenant_slug', 'method', 'route', 'status_code', 'duration_ms',
        ]);
  ...
  ok('C11 drill + delivery/search/retention remain open (completion owned by 16N)',
