...
function req(envelope=env(),language='en'){const evidence=createEmailLunaDraftPolicyEvidence({client_id:ids.client_id,location_id:ids.location_id,conversation_id:ids.conversation_id,endpoint_id:ids.endpoint_id,language,identity:'matched',intent:'catalog_question',intent_support:'supported',requested_location_id:ids.location_id,explicit_human_request:false,attachment_interpretation_required:false,unsafe_transactional_request:false,required_facts:['catalog'],grounded_results:{catalog:{fact:'catalog',status:'found',client_id:ids.client_id,location_id:ids.location_id,item:'board_rental',label:'Board rental',currency:'EUR',amount_cents:2000,active:true}}});return {envelope,evidence,decision:decideEmailLunaDraftPolicy({envelope,evidence})};}
