...
function req(envelope=env()){const evidence=createEmailLunaDraftPolicyEvidence({client_id:ids.client_id,location_id:ids.location_id,conversation_id:ids.conversation_id,identity:'matched',intent:'catalog_question',intent_support:'supported',requested_location_id:ids.location_id,explicit_human_request:false,unsafe_transactional_request:false,required_facts:['catalog'],grounded_results:{catalog:{fact:'catalog',status:'found',client_id:ids.client_id,location_id:ids.location_id,item:'board',label:'Board rental',currency:'EUR',amount_cents:2000,active:true}}});return {envelope,evidence,decision:decideEmailLunaDraftPolicy({envelope,evidence})};}
