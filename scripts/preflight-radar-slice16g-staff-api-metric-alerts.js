...
/**
 * preflight-radar-slice16g-staff-api-metric-alerts
 *
 * Exact subscription/RG/app/action-group short-circuit preflight for RADAR 16G.
 * NEVER deploys. NEVER calls Azure mutating APIs.
 * Fail-closed on every unknown/positional argv. Explicitly rejects --live.
 * Delegates scope checks to module runPreflight (pinned Bicep blob; no deploy).
 *
 * Usage:
 *   node scripts/preflight-radar-slice16g-staff-api-metric-alerts.js \
...
const {
  ...
  SEVERITY,
  runPreflight,
} = require('./lib/radar-slice16g-staff-api-metric-alerts');
...
function main() {
  ...
  if (args.help) {
    console.log('Usage: node scripts/preflight-radar-slice16g-staff-api-metric-alerts.js --resource-group <rg>');
  ...
  const report = {
    ...
    templateGitBlob: pf.templateGitBlob,
    templateSha256: pf.templateSha256,
    azureCalls: azureCalls.count,
    note: 'Preflight only — pinned Bicep blob verified; no Azure calls, no deploy; AG referenced by name only',
  };

  console.log(JSON.stringify(report, null, 2));
  console.log('RADAR 16G preflight: PASS (no live calls)');
}
