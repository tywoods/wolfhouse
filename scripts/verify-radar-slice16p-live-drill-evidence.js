...
function validateGateMatrix(matrix) {
  ...
  for (const gid of locks.SOURCE_PARTIAL_GATES) {
    if (gid === 'G01_correlation_structured_logs') continue; // 16S legitimately partial_live_proven
    const g = (matrix.gates || []).find((x) => x.id === gid);
    if (!g) {
      errors.push(`${gid} missing`);
      continue;
    ...
    if (g.verdict !== 'partial') errors.push(`${gid} verdict=${g.verdict}`);
    if (g.progress_class === 'partial_live_proven') {
      errors.push(`${gid} falsely partial_live_proven`);
    }
    if (g.verdict === 'proven') errors.push(`${gid} falsely proven`);
  }

  // No gate may be proven
...
ok('C3 HEAD on 16P branch (tip may be 16W/16X)', currentBranch() === locks.BRANCH || currentBranch() === 'radar/slice-16w-readiness-shutdown-lifecycle' || currentBranch() === 'radar/slice-16x-g02-live-evidence' || currentBranch() === 'radar/slice-16y-shutdown-completion-log' || currentBranch() === 'radar/slice-16z-g02-live-sigterm-evidence' || currentBranch() === 'radar/slice-16aa-g02-live-sigint-evidence' || currentBranch() === 'radar/slice-16ab-g02-readyz503-evidence', currentBranch());
...
ok('C6 top-level contract owns 16P (tip may advance to 16W/16X)',
  (topContract.slice === locks.SLICE || topContract.slice === 'RADAR-16W' || topContract.slice === 'RADAR-16X' || topContract.slice === 'RADAR-16Y' || topContract.slice === 'RADAR-16Z' || (topContract.slice === 'RADAR-16AA' || topContract.slice === 'RADAR-16AB' || topContract.slice === 'RADAR-16AC' || topContract.slice === 'RADAR-16AD' || topContract.slice === 'RADAR-16AE'))
