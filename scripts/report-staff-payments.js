/**
 * Stage 6.4a — Staff payments batch report.
 *
 * Runs all payment-category intents from staff-query-registry.js in one pass
 * and prints a structured, section-per-intent report.
 *
 * Usage:
 *   node scripts/report-staff-payments.js [--client <slug>]
 *
 * Optional flags:
 *   --client   Client slug (default: wolfhouse-somo)
 *
 * Safety constraints:
 *   - Only payment-category intents from staff-query-registry.js are executed
 ...
 *
 * Stage 6.4a is read-only. Write/action intents are Stage 6.5.
 */
...
// Payment intents have no parameterised args in Stage 6.4a.
// Kept as empty map so param resolution logic is consistent with other reports.
const PARAM_FLAG = {};
...
async function main() {
  ...
  // ── Load payment entries from registry ─────────────────────────────────────
  const entries = getEntriesByCategory('payments');
  if (entries.length === 0) {
    console.error('No payment entries found in registry.');
    process.exit(1);
  }

  // ── Report header ──────────────────────────────────────────────────────────
  console.log('\n══════════════════════════════════════════════════════════════════');
  console.log('  STAFF PAYMENTS REPORT');
  ...
  for (const entry of entries) {
    ...
    if (missingRequired.length > 0) {
      console.log(`  [skip] requires: ${missingRequired.join(', ')}`);
      console.log('');
      continue;
    }
