...
check('A2', /if not _wh_allow_quote:\s*\n\s*reply_to = None/.test(patches), 'send() clears reply_to unless wolfhouse_quote_reply');
...
check('A6', /initial_reply_to_id=\(\s*\n\s*None\s*\n\s*if str\(getattr\(source\.platform/.test(patches), 'gateway.run skips stream initial_reply_to_id for WhatsApp');
