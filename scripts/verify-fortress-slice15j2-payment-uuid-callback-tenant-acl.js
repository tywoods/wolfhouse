...
/**
 * verify:fortress-slice15j2-payment-uuid-callback-tenant-acl — FORTRESS Slice 15J2
 *
...
const CONTRACT_PATH = path.join(FIXTURE_DIR, 'slice15j2-contract.json');
const OVERLAY_PATH = path.join(FIXTURE_DIR, 'slice15j2-b15-remediation-overlay.json');
const FINDINGS_PATH = path.join(FIXTURE_DIR, 'slice15j2-findings.md');
const EVIDENCE_PATH = path.join(FIXTURE_DIR, 'slice15j2-evidence.json');
...
console.log('verify:fortress-slice15j2-payment-uuid-callback-tenant-acl — FORTRESS Slice 15J2\n');
...
const harnessSrc = readText(path.join(ROOT, 'scripts/lib/staff-query-api-fortress15j2-offline-harness.js'));
...
ok('F1 contract 15J2 + master_basis',
  contract.slice === 'FORTRESS-15J2'
  && contract.outcome_id === '15J2_payment_uuid_callback_tenant_acl_safe_harness'
...
ok('F10 dual-gate offline listener seam (router export + fail-closed inject)',
...
console.log('\n── Secret-free ──');
for (const rel of [
  'fixtures/fortress-tenant-identity/slice15j2-contract.json',
  'fixtures/fortress-tenant-identity/slice15j2-b15-remediation-overlay.json',
  'fixtures/fortress-tenant-identity/slice15j2-findings.md',
  'fixtures/fortress-tenant-identity/slice15j2-evidence.json',
  'scripts/lib/payment-uuid-callback-tenant-acl.js',
  'scripts/lib/staff-query-api-fortress15j2-offline-harness.js',
  'scripts/verify-fortress-slice15j2-payment-uuid-callback-tenant-acl.js',
]) {
...
(async () => {
  ...
  {
    const harnessForeign = createFortress15j2OfflineListener({
      payments: paymentsDb,
      bookings: bookingsDb,
    });
    const harnessMissing = createFortress15j2OfflineListener({
      payments: paymentsDb,
      bookings: bookingsDb,
    });
  ...
  {
    const { makeTrackingPg } = require('./lib/staff-query-api-fortress15j2-offline-harness');
    const pg = makeTrackingPg({ payments: [], bookings: [] });
    const aclCalls = [];
    const out = await gateStaffPaymentUuidCallbackTenantAcl({
      paymentId: PAY_ALPHA,
      user: { email: 'fortress15j2.alpha@example.test' },
  ...
  {
    const pg = require('./lib/staff-query-api-fortress15j2-offline-harness').makeTrackingPg({
      payments: paymentsDb,
      bookings: bookingsDb,
    });
  ...
  // ── Production-mode seam attempts must be inert (fail closed) ──
  {
    ...
    try {
      ...
      apiProd.setFortress15j2OfflineSeams({
        ...
        resolveSessionUser: () => ({ email: 'evil@example.test' }),
      });
  ...
  if (!committedEvidence) {
    ok('evidence readable', false, 'missing slice15j2-evidence.json');
  } else {
    ok('evidence slice + master_basis',
      committedEvidence.slice === 'FORTRESS-15J2'
  ...
  console.log(`\n── verify:fortress-slice15j2-payment-uuid-callback-tenant-acl ${fail ? 'FAILED' : 'PASSED'} (pass=${pass} fail=${fail}) ──\n`);
