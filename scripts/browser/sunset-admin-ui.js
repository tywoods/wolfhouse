  ...
  if (!beaches.length && !editingNew) {
    ...
    beaches.forEach(function(b){
      ...
      if (adminEditTarget === 'beach:' + key) {
        ...
        html += '<div class="portal-admin-beach-id"><strong>' + escHtml(b.display_name || key) + '</strong></div>';
...
function adminReadBeachPayload(beachKey){
  ...
  if (!beachKey && body.display_name) {
    body.beach_key = body.display_name
      .toLowerCase()
...
function adminSelectSubTab(key, opts){
  // Bookings is a top-level tab now — not an Admin subtab.
  var next = (key === 'pricing' || key === 'luna-staff' || key === 'email') ? key : 'finance';
...
function wireAdminTab(){
  ...
  root.addEventListener('click', function(ev){
    ...
    if (action === 'save-new-beach' || action === 'save-beach'){
      ...
      if (action === 'save-new-beach' && !beachBody.beach_key){ adminShowMessage('error', adminBeachText('admin.beaches.nameInvalid', 'Beach name must contain at least one letter or number.')); return; }
