...
var SCHEDULE_DAY_COCKPIT_CSS = [
  ...
  // Mobile chrome: date first, roomy navigation, then view + primary action.
  // Only the existing control bar changes; desktop layout and callbacks stay intact.
  '  .ck-bar{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-template-areas:"date date" "nav nav" "range create" "legend legend";gap:10px 8px;padding:16px 12px 12px;align-items:center;}',
  '  .ck-bar>.ck-seg{grid-area:nav;min-width:0;}',
  '  .ck-bar .ck-seg button{min-width:44px;min-height:44px;flex:1 1 0;padding:8px 6px;font-size:13px;}',
  '  .ck-bar__right{display:contents;}',
  '  .ck-bar .ck-seg--range{grid-area:range;min-width:0;}',
  '  .ck-bar .ck-icon-btn{grid-area:date;align-self:start;justify-self:end;}',
  '  .ck-bar .ck-cta{grid-area:create;min-width:0;padding:10px 8px;}',
  '  .ck-bar .ck-date{grid-area:date;padding-right:54px;align-self:start;}',
  '  .ck-bar .ck-date b{font-size:22px;line-height:1.2;letter-spacing:-.02em;}',
  '  .ck-bar .ck-date span{font-size:12px;line-height:1.4;margin-top:5px;color:var(--ck-ink-2);overflow-wrap:anywhere;}',
  '  .ck-bar .ck-legend{grid-area:legend;font-size:11px;}',
  '  [data-theme="dark"] .ck-bar>.ck-seg button[aria-pressed="true"],[data-theme="dark"] .ck-bar .ck-cta{color:var(--ck-now-bg);}',
  '  [data-theme="dark"] .ck-bar .ck-cta:hover{background:var(--ck-olive);box-shadow:inset 0 0 0 1px var(--ck-ink-2);}',
  '  .ck-icon-btn{width:44px;height:44px;min-width:44px;min-height:44px;}',
  '  .ck-cta,.ck-cta--ghost,.ck-cta--sm{min-height:44px;padding:10px 16px;white-space:nowrap;line-height:1.2;}',
  '  .ck-date{flex:1 1 100%;min-width:0;}',
  '  .ck-legend{flex:1 1 auto;margin-left:0;}',
  '  .ck-now{align-items:flex-start;gap:14px;}',
  '  .ck-seats{flex:1 1 auto;margin-left:0;justify-content:flex-end;gap:14px;min-width:0;}',
  '  .ck-seats__label{white-space:nowrap;font-size:12px;line-height:1.25;min-width:4.75rem;}',
  '  .ck-ring{width:52px;height:52px;flex-shrink:0;}',
  '  .ck-ring i{width:40px;height:40px;font-size:13px;}',
  '  .ck-ribbon{overflow-x:auto;overflow-y:visible;-webkit-overflow-scrolling:touch;margin:0;padding-bottom:2px;scrollbar-width:none;-ms-overflow-style:none;}',
  '  .ck-ribbon::-webkit-scrollbar{display:none;}',
  '  .ck-block{min-width:76px;justify-content:flex-start;padding:0 8px;font-size:10px;}',
  '}',
