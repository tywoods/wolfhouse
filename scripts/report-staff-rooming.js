/**
 * Stage 6.4b — Staff rooming batch report.
 *
 * Runs all rooming-category intents from staff-query-registry.js in one pass
 * and prints a structured, section-per-intent report.
 *
 * Usage:
 *   node scripts/report-staff-rooming.js [--client <slug>] [flags...]
 *
 * Optional flags:
 *   --client   Client slug (default: wolfhouse-somo)
 *   --date     Cutoff date for rooming.arrivals (default: TODAY)
 *   --start    Range start date for rooming.occupied_beds (YYYY-MM-DD)
 *   --end      Range end date for rooming.occupied_beds (YYYY-MM-DD)
 *
 * Safety constraints:
 *   - Only rooming-category intents from staff-query-registry.js are executed
 ...
 *
 * Stage 6.4b is read-only. Write/action intents are Stage 6.5.
 */
...
async function main() {
  ...
  // ── Load rooming entries from registry ─────────────────────────────────────
  const entries = getEntriesByCategory('rooming');
  if (entries.length === 0) {
    console.error('No rooming entries found in registry.');
    process.exit(1);
  }

  // ── Report header ──────────────────────────────────────────────────────────
  console.log('\n══════════════════════════════════════════════════════════════════');
  console.log('  STAFF ROOMING REPORT');
  console.log(`  Client:  ${clientSlug}`);
  console.log(`  Intents: ${entries.length}`);
  console.log(`  Date:    ${new Date().toISOString()}`);
  if (flags['--date'])  console.log(`  --date:  ${flags['--date']}`);
  if (flags['--start']) console.log(`  --start: ${flags['--start']}`);
  if (flags['--end'])   console.log(`  --end:   ${flags['--end']}`);
  console.log('══════════════════════════════════════════════════════════════════\n');
  ...
  for (const entry of entries) {
    ...
    if (missingRequired.length > 0) {
      const hintMap = { start_date: '--start YYYY-MM-DD', end_date: '--end YYYY-MM-DD', date: '--date YYYY-MM-DD' };
