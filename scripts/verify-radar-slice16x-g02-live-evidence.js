...
ok('C11 top contract selected_16x + G02 drill live_proven (tip may advance to 16Y)',
  (topContract.slice === locks.SLICE || topContract.slice === 'RADAR-16Y' || topContract.slice === 'RADAR-16Z' || (topContract.slice === 'RADAR-16AA' || topContract.slice === 'RADAR-16AB' || topContract.slice === 'RADAR-16AC' || (topContract.slice === 'RADAR-16AD' || topContract.slice === 'RADAR-16AF') || (topContract.slice === 'RADAR-16AD' || topContract.slice === 'RADAR-16AF')))
...

{
  ...
  ok('C14 runtime paths unchanged vs master (waived when tip is 16Y source observability)',
    rt.ok || matrix.slice === 'RADAR-16Y' || matrix.slice === 'RADAR-16Z' || (matrix.slice === 'RADAR-16AA' || matrix.slice === 'RADAR-16AB' || matrix.slice === 'RADAR-16AC' || (matrix.slice === 'RADAR-16AD' || matrix.slice === 'RADAR-16AF')), rt.detail);
}
