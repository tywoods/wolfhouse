...
function validateGateMatrix(matrix) {
  ...
  if (matrix.slice !== locks.SLICE && matrix.slice !== 'RADAR-16W' && matrix.slice !== 'RADAR-16Y' && matrix.slice !== 'RADAR-16Z' && matrix.slice !== 'RADAR-16AA' && matrix.slice !== 'RADAR-16AB' && matrix.slice !== 'RADAR-16AC' && matrix.slice !== 'RADAR-16AD') {
...
ok('C3 HEAD on 16X branch (tip may advance to 16Y/16Z/16AA)', currentBranch() === locks.BRANCH || currentBranch() === 'radar/slice-16y-shutdown-completion-log' || currentBranch() === 'radar/slice-16z-g02-live-sigterm-evidence' || currentBranch() === 'radar/slice-16aa-g02-live-sigint-evidence' || currentBranch() === 'radar/slice-16ab-g02-readyz503-evidence' || currentBranch() === 'radar/slice-16ac-organic-restart-alert-evidence'
...
ok('C11 top contract selected_16x + G02 drill live_proven (tip may advance to 16Y)',
  (topContract.slice === locks.SLICE || topContract.slice === 'RADAR-16Y' || topContract.slice === 'RADAR-16Z' || (topContract.slice === 'RADAR-16AA' || topContract.slice === 'RADAR-16AB' || topContract.slice === 'RADAR-16AC' || (topContract.slice === 'RADAR-16AD' || (topContract.slice === 'RADAR-16AF' || (topContract.slice === 'RADAR-16AG' || topContract.slice === 'RADAR-16AH'))) || (topContract.slice === 'RADAR-16AD' || (topContract.slice === 'RADAR-16AF' || (topContract.slice === 'RADAR-16AG' || topContract.slice === 'RADAR-16AH')))))
...

{
  ...
  ok('C14 runtime paths unchanged vs master (waived when tip is 16Y source observability)',
    rt.ok || matrix.slice === 'RADAR-16Y' || matrix.slice === 'RADAR-16Z' || (matrix.slice === 'RADAR-16AA' || matrix.slice === 'RADAR-16AB' || matrix.slice === 'RADAR-16AC' || (matrix.slice === 'RADAR-16AD' || (matrix.slice === 'RADAR-16AF' || (matrix.slice === 'RADAR-16AG' || matrix.slice === 'RADAR-16AH')))), rt.detail);
}
