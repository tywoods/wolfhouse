'use strict';
/**
 * Deploy Luna robustness gate to staff-staging (ACR build + Container App update).
 * Usage: node scripts/deploy-staging-luna-robustness.js [deploy|verify]
 */
...
function deploy() {
  ...
  console.error('[deploy] acr build (this may take several minutes)...');
  ...
  console.error('[deploy] containerapp update + staging profile flags...');
  az([
...
function verify() {
  ...
  console.log(JSON.stringify({
    ...
    unified_planner: env.LUNA_GUEST_UNIFIED_PLANNER_MODE,
    frontdesk: env.LUNA_GUEST_FRONTDESK_PLANNER_ACTIVE,
