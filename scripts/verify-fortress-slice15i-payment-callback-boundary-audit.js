...
// When Slice 15J3 remediation overlay exists, assert remediating patterns instead of
// re-asserting the pre-fix vulnerable handler shape against live code.
const remediation15j3OverlayPath = path.join(FIXTURE_DIR, 'slice15j3-b15-remediation-overlay.json');
const has15j3Remediation = fs.existsSync(remediation15j3OverlayPath);
const overlay15j3 = has15j3Remediation ? readJson(remediation15j3OverlayPath) : null;
const aclLibSrc = has15j3Remediation
  ? readText(path.join(ROOT, 'scripts/lib/payment-uuid-callback-tenant-acl.js'))
  : '';

red('AC15I_B15_STAFF_PAYMENT_UUID_NO_ACL',
  has15j3Remediation
    ? (overlay15j3.status === 'remediated'
      && overlay15j3.historical_audit_unchanged === true
