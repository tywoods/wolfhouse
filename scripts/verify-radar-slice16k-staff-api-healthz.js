...
async function main() {
  ...
  try {
    ...
    // Successor slices may own HEAD while 16K source remains frozen on master basis.
    const allowed = new Set([
      locks.BRANCH,
      'radar/slice-16l-capacity-pressure-alerts',
      'radar/slice-16m-stripe-event-claim',
      'radar/slice-16n-request-completion-log',
    ]);
    ok('C11 HEAD on 16K branch or successor 16L/16M/16N', allowed.has(branch), branch);
  } catch (err) {
    ok('C11 HEAD on 16K branch or successor 16L/16M/16N', false, String(err && err.message));
  }
