...
/**
 * run-phase-d-pgcrypto-compatibility-normalization — FOUNDATION Slice 14AB
 *
 * Default-disabled operator CLI: merged target authority + one read-only
 * observer session proving Azure PG15 pgcrypto 1.3 presentation normalization.
 */
...
function defaultRefuseOutput() {
  return {
    ...
    pgcryptoCompatibilityNormalizationLiveEnabled: true,
...
async function main() {
  ...
  resetPgcryptoCompatibilityNormalizationCounters();
  const gate = evaluatePgcryptoCompatibilityNormalizationGates({ env: process.env, argv });
  if (!gate.ok) {
    const out = {
      ...defaultRefuseOutput(),
  ...
  const safe = {
    ...
    baselineMismatchCount: result.baselineMismatchCount != null
      ...
      : null,
    pgcryptoCompatibilitiesNormalized: result.pgcryptoCompatibilitiesNormalized != null
      ? result.pgcryptoCompatibilitiesNormalized
      : null,
