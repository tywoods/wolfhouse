#!/usr/bin/env bash
# Offline verifier: temp repo + linked worktree; D1/D2/D2.1 + hostile enum/swap/chmod.
set -euo pipefail

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
HELPER=$ROOT/scripts/lib/normalize-repo-readability.sh
START=$ROOT/scripts/captain-git-start.sh; DONE=$ROOT/scripts/captain-git-done.sh
PASS=0 FAIL=0
...
grep -q 'mktemp -d' "$HELPER" && grep -q 'print0' "$HELPER" && grep -q 'ls-files -z' "$HELPER" \
  && ! grep -qE 'done < <\(' "$HELPER" && ! grep -qE 'print0 2>/dev/null' "$HELPER" \
  && ok "enum uses temp files; no process-sub/masked find" || bad "enum uses temp files; no process-sub/masked find"
grep -q '/proc/self/fd/' "$HELPER" && grep -q 'exec {fd}' "$HELPER" \
  && ok "chmod via open FD /proc/self/fd" || bad "chmod via open FD /proc/self/fd"
...
OBJ_B=$(mode_of "$OBJ"); TR_B=$(mode_of "$MAIN/tracked file.txt")
echo "  setup object=$OBJ_B tracked=$TR_B secret=$(mode_of "$MAIN/.env.secret")"
...
echo "escape-test" >"$TMP/escape-file"; chmod 0600 "$TMP/escape-file"; NRR_ESCAPE=0
...
# Hostile: find / git enum failure must fail
_nrr_find() { return 1; }
set +e; normalize_repo_readability "$MAIN" >/dev/null 2>&1; FRC=$?; set -e
unset -f _nrr_find; _nrr_find() { find "$@"; }
(( FRC != 0 )) && ok "hostile: find enum failure returns nonzero" \
  || bad "hostile: find enum failure returns nonzero" "rc=$FRC"
_nrr_git() { [[ $* == *ls-files* ]] && return 1; git "$@"; }
set +e; normalize_repo_readability "$MAIN" >/dev/null 2>&1; GRC=$?; set -e
unset -f _nrr_git; _nrr_git() { git "$@"; }
(( GRC != 0 )) && ok "hostile: git ls-files failure returns nonzero" \
  || bad "hostile: git ls-files failure returns nonzero" "rc=$GRC"

# Hostile: swap between discovery/open must not chmod outside 0600 secret
OUTSIDE=$TMP/outside-secret; printf 'OUTSIDE\n' >"$OUTSIDE"; chmod 0600 "$OUTSIDE"
_nrr_hook_before_open() {
  [[ $1 == *'tracked file.txt' ]] || return 0
  rm -f -- "$1"; ln -s -- "$OUTSIDE" "$1"
}
printf 'tracked body\n' >"$MAIN/tracked file.txt"; chmod 0600 "$MAIN/tracked file.txt"
set +e; normalize_repo_readability "$MAIN" >/dev/null 2>&1; SRC=$?; set -e
unset -f _nrr_hook_before_open
[[ $(mode_of "$OUTSIDE") == 600 ]] && ok "hostile: swap does not chmod outside secret" \
  || bad "hostile: swap does not chmod outside secret" "mode=$(mode_of "$OUTSIDE")"
(( SRC != 0 )) && ok "hostile: swap path fails closed" || bad "hostile: swap path fails closed" "rc=$SRC"
rm -f -- "$MAIN/tracked file.txt"; printf 'tracked body\n' >"$MAIN/tracked file.txt"

# Hostile: chmod failure must fail
_nrr_chmod() { return 1; }; chmod 0600 "$MAIN/tracked file.txt"
set +e; normalize_repo_readability "$MAIN" >/dev/null 2>&1; CRC=$?; set -e
unset -f _nrr_chmod; _nrr_chmod() { chmod "$@"; }
(( CRC != 0 )) && ok "hostile: chmod failure returns nonzero" \
  || bad "hostile: chmod failure returns nonzero" "rc=$CRC"
echo; echo "summary: pass=$PASS fail=$FAIL"; (( FAIL == 0 ))
