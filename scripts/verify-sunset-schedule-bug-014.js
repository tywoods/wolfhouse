...
const firstOpenCall = wireSrc.match(/scheduleOpenOrStartConversationFromBooking\(([^)]*)\)/);
assert.ok(
  firstOpenCall && /linkedSource/.test(firstOpenCall[1]),
  'linked-conv click must use the source that found the conversation',
);

const findLinkedSrc = extractFunctionSource(apiSrc, 'scheduleFindLinkedConversation');
const resolveConvIdSrc = extractFunctionSource(apiSrc, 'scheduleResolveConversationId');
const openOrStartSrc = extractFunctionSource(apiSrc, 'scheduleOpenOrStartConversationFromBooking');
const startConvSrc = extractFunctionSource(apiSrc, 'scheduleStartConversationFromBooking');
assert.ok(findLinkedSrc && resolveConvIdSrc && openOrStartSrc && startConvSrc, 'click-path helpers extractable');
...
function runWireClick(row, group, ctx, conversations) {
  const dom = {};
  function makeEl(id, extras) {
    const o = Object.assign({
      id,
  ...
  const sandbox = {
    ...
    el: (id) => dom[id] || null,
    portalT,
    scheduleConversationsCache: conversations.slice(),
    openInboxToConversation: (id) => { opened.push(id); },
    getClient: () => 'sunset',
    ...
    fetch(url, opts) {
      ...
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ success: true, conversation_id: 'dup-created' }),
      });
    },
  };
  vm.createContext(sandbox);
...
${wireSrc}
this.scheduleWireDrawerConversation = scheduleWireDrawerConversation;
this.scheduleOpenOrStartConversationFromBooking = scheduleOpenOrStartConversationFromBooking;
`, sandbox);

  sandbox.scheduleWireDrawerConversation(row, group, ctx);
  const btn = dom['ps-drawer-conversation-btn'];
...
const existingConv = {
  conversation_id: 'conv-existing-ada',
  phone: listPhone,
  booking_code: 'SUN-ADA-1',
};
const click = runWireClick(
  { booking_id: 'b1', guest_name: 'Ada' },
  { booking_id: 'b1', guest_name: 'Ada' },
  { booking_id: 'b1', guest_name: 'Ada', phone: listPhone },
  [existingConv],
);
assert.strictEqual(click.disabled, false, 'ctx-linked conversation enables the button');
assert.strictEqual(click.label, 'Open conversation', 'existing ctx conversation shows Open, not Start');
assert.deepStrictEqual(
  click.opened,
  ['conv-existing-ada'],
  'click must open the existing ctx-linked conversation',
);
assert.strictEqual(
  click.fetches.length,
  0,
  'click must not POST create-conversation when ctx already linked the thread',
);
assert.ok(
  click.fetches.every((f) => !String(f.url || '').includes('create-conversation')),
  'no duplicate conversation create on phone-less group click',
);
...
console.log('PASS BUG-014 drawer phone empty-state (list/ctx only; no invent; staff: ignored; click opens existing)');
