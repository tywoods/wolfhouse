...
/** Coerce digit-only JSON strings to numbers on a deep clone (verifier-owned). */
function coerceNumericStrings(value) {
  if (typeof value === 'string' && /^-?\d+(\.\d+)?$/.test(value)) return Number(value);
  if (Array.isArray(value)) return value.map(coerceNumericStrings);
  if (value && typeof value === 'object') {
    const out = {};
    for (const k of Object.keys(value)) out[k] = coerceNumericStrings(value[k]);
    return out;
  }
  return value;
}
...
ok('generator_behavior_changed false', contract.generator_behavior_changed === false);
...
ok('findings cite integration / isolation / portable',
  /integration/i.test(findings)
  && /isolation/i.test(findings)
  && /portable|portability/i.test(findings));
ok('findings cite verifier-owned temp / no generator disk writes',
  /verifier-owned|verifier owned/i.test(findings)
  && /temp/i.test(findings)
  && (/no generator disk|never.*generator.*disk|generator to write disk|without generator disk/i.test(findings)));
...
try {
  for (const archetype of slice1d.ARCHETYPE_IDS) {
    ...
    for (const f of packed.envelope.files) {
      ...
      byKind[f.kind] = { abs, obj: f.kind === 'manifest' ? null : JSON.parse(f.content), content: f.content };
    }
    ...
    if (archetype === 'surf_house') {
      ...
      // Raw seed strings: calculator must accept config and return structured result.
      const rawQuote = calculateWolfhouseQuote({
        client_slug: pricingDisk.client_slug,
        check_in: '2026-04-01',
        check_out: '2026-04-08',
        guest_count: 1,
        package_code: pricingDisk.packages[0].code,
        payment_choice: 'deposit',
      }, pricingDisk);
      green('surf_house_quote_calculator_accepts_temp_pricing_shape',
        rawQuote && typeof rawQuote === 'object' && Array.isArray(rawQuote.blockers));

      // Coerced numeric clone (verifier-owned) proves calculator math path on shape.
      // package_none pricing uses the Malibu weekly reference internally, so the
      // clone exposes a malibu-coded package entry without mutating golden bytes.
      const coerced = coerceNumericStrings(JSON.parse(JSON.stringify(pricingDisk)));
      coerced.packages = coerced.packages.map((pkg, idx) => (
        idx === 0 ? { ...pkg, code: 'malibu' } : pkg
      ));
      const mathQuote = calculateWolfhouseQuote({
        client_slug: coerced.client_slug,
        check_in: '2026-04-01',
        check_out: '2026-04-08',
        guest_count: 1,
        package_code: 'malibu',
      ...
      green('surf_house_quote_calculator_math_on_coerced_temp_clone',
        mathQuote.success === true && mathQuote.nights === 7,
        mathQuote.blockers && mathQuote.blockers.join(','));

      const noneQuote = calculateWolfhouseQuote({
        client_slug: coerced.client_slug,
        check_in: '2026-04-01',
      ...
      green('surf_house_short_stay_package_none_on_coerced_temp',
        noneQuote.success === true && noneQuote.nights === 3,
        noneQuote.blockers && noneQuote.blockers.join(','));

      const combo = resolveGuestAddonComboPricing({
        serviceType: 'wetsuit',
        quantity: 1,
        pricing: { amount_due_cents: 1000, payment_required: true },
        existingRecords: [{ id: 9, service_type: 'surfboard', status: 'confirmed', quantity: 2 }],
      });
      green('surf_house_guest_addon_combo_consumer',
        combo.combo_applied === true && combo.amount_due_cents === 0);

      // Schedule surface from baseline.
    ...
    if (archetype === 'surf_school_shop') {
      ...
      // Seed placeholders are strings — flatten correctly skips non-numbers.
      const flatSeed = flattenOfferingPrices(lessons, 'lesson', 'EUR');
      green('surf_school_flatten_skips_string_seed_prices',
        Array.isArray(flatSeed) && flatSeed.length === 0);

      const coercedBaseline = coerceNumericStrings(JSON.parse(JSON.stringify(baseline)));
      const flatNum = flattenOfferingPrices(
        coercedBaseline.catalog.lessons.offerings,
        'lesson',
        'EUR',
      );
      const flatRent = flattenOfferingPrices(
        coercedBaseline.catalog.rentals.offerings,
        'rental',
        'EUR',
      );
      green('surf_school_flatten_coerced_temp_clone_yields_prices',
        flatNum.length > 0 && flatRent.length > 0,
  ...
  // Ensure generator was not asked to write — only verifier wrote under consumerTemp.
...
// ── Source fences (1D must not mutate generator/templates) ──────────────────
console.log('\n── Source fences ──');
{
...
// Tip scope vs master for factory-touched paths on this branch working tree.
{
  ...
  // Also include untracked factory files we care about.
  ...
  ok('1D tip paths within allowlist (no generator/template/product edits)',
    tipCheck.ok,
    tipCheck.bad.join(','));
  red('no_archetype_template_edits_in_tip',
    !factoryTouched.some((p) => p.startsWith('config/archetypes/')));
  red('no_generator_or_cli_edits_in_tip',
...
console.log('\n── Nested prior factory gates ──');
if (process.env.FACTORY_1D_LEDGER_PROBE === '1') {
  ok('ledger probe skips nested re-entry', true);
} else {
  const nestedEnv = {
    ...process.env,
    FACTORY_1C_LEDGER_PROBE: '1',
    FACTORY_1B_LEDGER_PROBE: '1',
    FACTORY_1A_SKIP_NESTED_1B: '1',
    FACTORY_1A_SKIP_NESTED_1C: '1',
    FACTORY_1A_SKIP_NESTED_1D: '1',
  };
...
if (process.env.FACTORY_1D_SKIP_LEGACY_GATES === '1') {
  ok('legacy gates skipped via FACTORY_1D_SKIP_LEGACY_GATES', true);
} else {
  ...
  // Full verify:multiclient includes staff-tenant-scope (retained RED).
