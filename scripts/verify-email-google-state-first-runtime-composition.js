...
function harness(spec={}) {
  ...
  const db=freeze({query(sql,values){calls.query.push({sql,values,receiver:this});
    if(/UPDATE tenant_email_google_oauth_transactions/.test(sql))return Promise.resolve(freeze({rows:freeze([freeze({operation_id:OPERATION,location_id:LOCATION,endpoint_id:ENDPOINT,staff_user_id:STAFF,code_verifier:VERIFIER,nonce:NONCE})])}));
