...
const KEYS = freeze(['tenantSlug','clientId','locationKey','applicationClientId','redirectUri','callbackEnabled']);
...
function input() { return freeze({tenantSlug:'sunset',clientId:CLIENT,authSessionId:SESSION,query:`state=${STATE}&code=${encodeURIComponent(CODE)}`}); }
...
function harness(spec={}) {
  ...
  const db=freeze({query(sql,values){calls.query.push({sql,values,receiver:this});return Promise.resolve(freeze({rows:freeze([freeze({id:ENDPOINT,client_id:CLIENT,location_id:LOCATION,channel:'email',provider:'gmail_api',secret_ref:REF,active:true})])}));}});
  ...
  const consumed=freeze({operationId:OPERATION,locationId:LOCATION,endpointId:ENDPOINT,staffUserId:STAFF,codeVerifier:VERIFIER,nonce:NONCE});
  const repository=freeze({consume(dto){calls.consume.push({dto,receiver:this});return consumed;}});
  ...
  const installer=freeze({async installVerifiedGrant(value){calls.install.push({value,receiver:this});return freeze({status:'installed'});}});
...
test('authentic consume, SQL authority, token/JWKS/profile, crypto, envelope and installer complete once',async()=>{const owner=require(OWNER_PATH),h=harness(),r=owner.createGoogleStateFirstRuntimeComposition(config(),h.dependencies);const out=await r.completeCallback(input());assert.deepEqual(out,{status:'received'});assert.equal(Object.isFrozen(out),true);assert.equal(h.calls.consume.length,1);assert.deepEqual(h.calls.consume[0].dto,{clientId:CLIENT,authSessionId:SESSION,stateHash:nativeCrypto.createHash('sha256').update(STATE,'ascii').digest('hex'),consumedAt:NOW_ISO});assert.equal(h.calls.consume[0].receiver,h.owners.repository);assert.equal(h.calls.query.length,1);assert.deepEqual(h.calls.query[0].values,[ENDPOINT,CLIENT,LOCATION]);assert.equal(h.calls.query[0].receiver,h.owners.db);assert.deepEqual(h.calls.secret.map(x=>x.value),[{secretRef:REF}]);assert.equal(h.calls.secret[0].receiver,h.owners.secretProvider);assert.equal(h.calls.requests.length,3);assert.deepEqual(h.calls.requests.map(x=>[x.options.hostname,x.options.path]),[['oauth2.googleapis.com','/token'],['www.googleapis.com','/oauth2/v3/certs'],['www.googleapis.com','/gmail/v1/users/me/profile']]);assert.deepEqual(h.calls.crypto,['createPublicKey','verify']);assert.equal(h.calls.seal.length,1);assert.equal(h.calls.install.length,1);assert.equal(h.calls.install[0].receiver,h.owners.installer);assert.equal(h.calls.open,0);assert.equal(h.calls.rewrap,0);const installed=h.calls.install[0].value;assert.deepEqual(Reflect.ownKeys(installed),['clientId','endpointId','operationId','actorStaffUserId','identity','envelope']);for(const leak of [SECRET,ACCESS,REFRESH,CODE,REF])assert.equal(JSON.stringify(installed).includes(leak),false);});
