'use strict';
/** Stage 27test-j — deploy proof. Temp — do not commit. */
const https = require('https');
const { execSync, spawnSync } = require('child_process');

const HOST = 'staff-staging.lunafrontdesk.com';
const COMMIT = 'b9974cb';
const IMAGE_TAG = `${COMMIT}-stage27test-j-proof-hygiene`;
...
function parseGolden(out) {
  ...
  const staleReplay = /idempotent_replay|non_booking_lane|stale/i.test(out);
  return {
    result: resultMatch ? resultMatch[1].toUpperCase() : null,
    passed: passMatch ? Number(passMatch[1]) : null,
    failed: failMatch ? Number(failMatch[1]) : null,
    total: totalMatch ? Number(totalMatch[1]) : null,
    stale_artifact_hits: staleReplay,
...
(async () => {
  ...
  const bannedTerms = ['n8n', 'meta webhook', 'stripe.com', 'payment link', 'checkout.session'];
  const bannedHits = bannedTerms.filter((t) => new RegExp(t, 'i').test(batchOut + goldenOut));
  ...
  const out = {
    ...
    safety: {
      ...
      no_stripe_in_output: !/stripe\.com|payment link/i.test(batchOut + goldenOut),
  ...
  const batchOk = batch.result === 'PASS'
    && batch.passed === 26
    && batch.partial === 0
    && batch.failed === 0;
  ...
  const safetyOk = out.safety.review_dry_run
    && out.safety.review_sends_whatsapp_false
    && out.safety.review_live_blocked
    && out.safety.no_stripe_in_output
    && out.safety.no_banned_terms;

  if (deployOk && batchOk && goldenOk && safetyOk) out.result = 'PASS';
  else if (deployOk && (batchOk || goldenOk)) out.result = 'PARTIAL';
