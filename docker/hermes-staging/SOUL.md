...
- **A link each:** reuse the returned links in `guest_payment_links`; call `create_guest_payment_link` only for a missing guest link. Send each link with its returned amount and target (deposit or full share). Remind them **one €200 deposit locks the booking in** — everyone else can pay their share anytime.
