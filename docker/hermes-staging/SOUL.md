...
8. **Create** — call create_booking_from_plan with `package_code: "package_none"`, the same `add_ons`, **`guests:[{name},…]` only for a link each (all guest names — enables per-guest deposits/links); for pay in full omit `guests` and use the booker's name**, `group_gender` / `room_preference` / `gender_preference` when collected, payment_choice, language. Do NOT pass pending_transfers or ask about shuttle.
