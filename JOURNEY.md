...
_Last updated: 2026-08-05 03:40 UTC by Captain_
...
- **Email 2F-C2 runtime composition INTEGRATED to master (NOT deployed)** (Deckhand built; Captain integrated, Earthling directed) — default-off Sunset-staging Azure KV envelope factory: MI clientId `0e05fbe3…` + trusted host `luna-sunset-staging-kv` + KEK version `fde9704b…` hard-pinned, RSA-OAEP-256, import-inert, `runtime_activation:false`, no startup/OAuth/refresh wiring. Serial cherry-pick onto master (my schedule ship preserved); gates runtime-comp 31 + provider 92 + custodian 80 + oauth-contract 118 + migration-integrity + **npm audit 0 vulns** (`@azure/identity 4.13.1`, `keyvault-keys 4.10.2`). `1b6fa65b`. **No deploy, migration 059 not applied, email OFF.**

## 🍳 On the stove (in progress)
- **L3 / email 2F-C DONE + 2F-C2 integrated (email still OFF):** Earthling created the KEK in `luna-sunset-staging-kv` (`luna-email-grant-kek/fde9704b…`) + granted the Sunset identity Crypto User. 2F-C2 default-off runtime-composition factory is **on master (`1b6fa65b`) but NOT deployed**. **Next (when owner/Earthling calls it):** Azure pre-deploy gate (clientId `0e05fbe3…`/principalId `5338388f…`/KEK enabled/Crypto User present) → build+deploy → controlled staging wrap/unwrap custody proof — each behind its own gate. Then refresh-exchange, OAuth callback, activation. No guest mail before all green.
...
## 🚀 Live where
- **Sunset · staff-api** (staging) — deployed `2b25d077` rev 0000473 · 08-04 22:12 · Captain _(schedule guest-collapse rule)_. **origin/master ahead at `1b6fa65b`** — 2F-C2 email factory integrated but deliberately NOT deployed (email off). Prior tonight: 0471 bookings+Restore, 0472 finance fail-soft + item-name audit.
