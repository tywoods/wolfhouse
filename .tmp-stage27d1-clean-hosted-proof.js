...
async function dbSafety(proofStart, bookingId, paymentDraftId) {
  ...
  const dupBookings = await pg.query(
    `SELECT COUNT(*)::int AS n FROM bookings b
      ...
      WHERE cl.slug = $1 AND b.phone = $2 AND b.check_in = $3::date AND b.check_out = $4::date`,
    [CLIENT, GUEST_PHONE, CHECK_IN, CHECK_OUT],
  );
  let pay = { rows: [] };
  if (paymentDraftId) {
    pay = await pg.query(
      'SELECT status::text, checkout_url, stripe_checkout_session_id FROM payments WHERE id = $1::uuid',
      [paymentDraftId],
    );
  }
  await pg.end();
  return {
    ...
    bookings_same_window: dupBookings.rows[0].n,
...
(async () => {
  const proof = {
    ...
    commit: COMMIT,
    image_tag: IMAGE_TAG,
    clean_phone: GUEST_PHONE,
    clean_dates: `${CHECK_IN} to ${CHECK_OUT}`,
    revision: null,
    turn3_run1: null,
    turn3_run2: null,
    calendar: null,
  ...
  try {
    ...
    proof.db_safety_run2 = await dbSafety(proofStart, t3a.booking_id, t3a.payment_draft_id);
    ...
    const checks = [
      ...
      ['write_created', t3a.write_status === 'created'],
      ['assign_created', t3a.assignment_write_status === 'created'],
      ['calendar_visible_expected', t3a.calendar_visible_expected === true],
      ['assigned_bed', !!t3a.assigned_bed_label],
      ['assigned_room', !!t3a.assigned_room_label],
      ['calendar_match', proof.calendar.matched === true],
      ['calendar_dates', proof.calendar.matches.some((m) => String(m.check_in || '').includes('2026-08-18'))],
      ['harness2_exit', run2.exit === 0],
      ['write_reused', t3b.write_status === 'reused_existing'],
      ['assign_reused', t3b.assignment_write_status === 'reused_existing'],
      ['same_booking', t3a.booking_id && t3b.booking_id && t3a.booking_id === t3b.booking_id],
      ['no_dup_bookings', proof.db_safety_run2.bookings_same_window <= 1],
      ['no_whatsapp', proof.db_safety.guest_message_sends === 0],
      ['no_stripe', !(proof.db_safety.payment && (proof.db_safety.payment.stripe_checkout_session_id || proof.db_safety.payment.checkout_url))],
    ...
    proof.result = proof.failures.length === 0 ? 'PASS'
      : (proof.failures.length <= 2 ? 'PARTIAL' : 'FAIL');
  } catch (err) {
