...
(async () => {
  ...
  await c.connect();
  const keys = await c.query('SELECT label, "apiKey" FROM user_api_keys ORDER BY "createdAt" DESC LIMIT 10');
  const wf = await c.query('SELECT id, name, active FROM workflow_entity WHERE id = $1', ['stage27demoJReview01']);
  console.log(JSON.stringify({ keys: keys.rows.map((r) => ({ label: r.label, prefix: r.apiKey.slice(0, 24) })), wf: wf.rows }, null, 2));
  await c.end();
