...
const IDEM_KEY = 'phase-10-3i-multi-bed-move-b1-to-r2b1';
...
(async () => {
  ...
  const mb = move.body || {};
  const ib = idem.body || {};
  const available = (mt.targets || []).filter((t) => t.available);
  ...
  const uiGateOn = {
    ...
    hasMoveTargets: /\/staff\/bookings\/move-targets/.test(uiHtml),
    ...
    noGateOffBanner: !/Move controls are disabled/.test(uiHtml),
  };

  const targetsOk =
    moveTargets.status === 200 &&
    mt.success === true &&
    mt.preview_only === true &&
    available.some((t) => t.bed_id === TARGET_BED_ID) &&
    !available.some((t) => t.bed_id === sourceBefore?.bed_id);
  ...
  const siblingOk =
    ...
    siblingAfter?.booking_bed_id === SIBLING_BOOKING_BED_ID;
  ...
  const ok = Object.values(uiGateOn).every(Boolean) && targetsOk && moveOk && idemOk && siblingOk && sourceMovedOk && dbCountsOk;

  console.log(JSON.stringify({
    gateOnRevision: 'wh-staging-staff-api--0000067',
    ...
    moveTargets: {
      ...
      available_bed_codes: available.map((t) => t.bed_code),
      body: mt,
    },
    move: { status: move.status, body: mb },
    idempotency: { status: idem.status, body: ib },
    checks: { targetsOk, moveOk, idemOk, siblingOk, sourceMovedOk, dbCountsOk },
