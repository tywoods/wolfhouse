'use strict';
/** Stage 28h.3 — deploy c454a94 reset-new-booking fix. Temp — do not commit. */
const { execSync } = require('child_process');

const COMMIT = 'c454a94';
const IMAGE_TAG = `${COMMIT}-stage28h3-reset-new-booking`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 's28h3-reset-new-booking';
...
try {
  ...
  console.log(JSON.stringify({
    ...
    healthz: healthz(),
    image: IMAGE,
    retest: 'Send: no no I want to create another booking',
  }, null, 2));
} catch (e) {
