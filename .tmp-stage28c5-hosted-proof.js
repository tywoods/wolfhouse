'use strict';
/** Stage 28c.5 — priced dates handset proof (July 24–31). Temp — do not commit. */
const https = require('https');
const { Client } = require('pg');
const { execSync } = require('child_process');

const COMMIT = '8fd0d39';
const IMAGE_TAG = `${COMMIT}-stage28c4-meta-staffapi-booking-write`;
const IMAGE = `whstagingacr.azurecr.io/wh-staff-api:${IMAGE_TAG}`;
const REV_SUFFIX = 's28c4-meta-booking';
...
const CHOSEN_DATES = { check_in: CHECK_IN, check_out: CHECK_OUT, season: 'summer', free_beds: 4 };
const TURNS = [
...
function deploy() {
  if (process.env.SKIP_DEPLOY !== '0') {
    console.error('[deploy] using existing 8fd0d39-stage28c4 image (SKIP_DEPLOY default)');
        ...
        https.get({
          ...
          calRes.on('end', () => {
            resolve({
              ...
              calendar_snippet: bookingCode && calBuf.includes(bookingCode) ? 'found' : 'not_found',
            });
          });
        });
      });
...
function fieldsFromReview(review, odr) {
  ...
  return {
    ...
    quote_blockers: quote && quote.quote_handoff_reasons,
...
(async () => {
  ...
  const out = {
    stage: '28c.5-meta-staffapi-handset-priced-dates',
    ...
    architecture: 'Meta → Staff API open-demo execute (8fd0d39)',
  ...
  try {
    ...
    const imageOk = String(out.revision.image || '').includes('8fd0d39-stage28c4-meta-staffapi-booking-write');
    out.preflight = {
      ...
      beds_ok: out.availability.free_beds >= 4,
    ...
    console.error(`\n=== PROOF WINDOW OPEN — dates ${CHECK_IN} → ${CHECK_OUT} (${CHOSEN_DATES.season}) ===`);
    ...
    out.checks = {
      ...
      turn2_no_quote_handoff: out.turn2.review_fields?.quote_handoff_required !== true,
    ...
    const core = ['turn3_write_created', 'turn3_assign_created', 'turn3_booking_code'];
