'use strict';
/** Phase 19c.1 — check-in day preview hosted proof. Temp — do not commit. */
...
(async () => {
  ...
  const revOk = rev.properties.healthState === 'Healthy' && rev.properties.trafficWeight === 100
    && String(rev.properties.template?.containers?.[0]?.image || '').includes('a04b103');

  let result = 'PARTIAL';
  if (aPass && bPass && health.status === 200 && revOk) result = 'PASS';
  if (resA.body.sends_whatsapp || resB.body.sends_whatsapp
    || resA.body.creates_booking || resB.body.creates_booking) result = 'FAIL';

  const out = {
    phase: '19c.1',
    result,
    commit: 'a04b103',
    image: 'whstagingacr.azurecr.io/wh-staff-api:a04b103-stage19c1-checkin-preview',
    acr_run: 'cb3f',
    revision: {
      name: rev.name,
